...
uint16_t calc_crc16_long_table(uint8_t *data, uint16_t len) {
    ...
    while (len--) {
        nTemp = *data++ ^ wCRCWord;
        wCRCWord >>= 8;
        wCRCWord ^= crc16_table[(nTemp & 0xFF)];
    }
    return wCRCWord;
}
...
/**
 * @brief
 * https://www.cnblogs.com/JerryMouseLi/p/12592565.html#22-%E6%9F%A5%E7%9F%AD%E8%A1%A8%E6%B3%95%E8%AE%A1%E7%AE%97crc16%E6%A0%A1%E9%AA%8C
 *
 */
static uint16_t crcTlb[] = {0x0000, 0xCC01, 0xD801, 0x1400, 0xF001, 0x3C00,
...
uint16_t calc_crc16_short_table(uint8_t *data, uint16_t len) {
    ...
    for (i = 0; i < len; i++) {
        ch = data[i];
        crc = (uint16_t)(crcTlb[(ch ^ crc) & 0x0F] ^ (crc >> 4));
        crc = (uint16_t)(crcTlb[((ch >> 4) ^ crc) & 0x0F] ^ (crc >> 4));
    }
    return crc;
}