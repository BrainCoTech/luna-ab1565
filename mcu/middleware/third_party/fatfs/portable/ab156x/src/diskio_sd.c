...
/* Read Sector(s)                                                        */
/*-----------------------------------------------------------------------*/
uint32_t sd_read_time;
uint32_t sd_write_time;
DRESULT sd_disk_read(
    BYTE pdrv,		/* Physical drive nmuber to identify the drive */
    BYTE *buff,		/* Data buffer to store read data */
    DWORD sector,	/* Sector address in LBA */
...
)
{
    ...
    while (retry < 3) {
        ...
        if (HAL_SD_STATUS_OK == status) {
            ...
            hal_gpt_get_duration_count(start_count, end_count, &duration);
            sd_read_time = duration;
            return RES_OK;
        } else {
            if (retry == 0) {
                ...
                if (HAL_SD_STATUS_OK == status) {
                    ...
                    sd_read_time = 0xFFFFFFFF;  //SD is init error
                    return RES_ERROR;
                }
            } else {
                retry++;
    ...
    sd_read_time = 0xDEADBEAF;   //write retry is overtime
    return RES_ERROR;
}
...
)
{
    ...
    while (retry < 3) {
        ...
        if (HAL_SD_STATUS_OK == status) {
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &end_count);
            hal_gpt_get_duration_count(start_count, end_count, &duration);
            sd_write_time = duration;
            return RES_OK;
        } else {
            status = hal_sd_init(fatfs_sd_port, &fatfs_sd_config);
            if (HAL_SD_STATUS_OK == status) {
                sd_write_time = 0xFFFFFFFF;  //SD is init error
                retry++;
            } else {
                return RES_ERROR;
            }
    ...
    sd_write_time = 0xDEADBEAF;   //write retry is overtime
    return RES_ERROR;
}
#endif
