...
#if defined(ANDLINK_ENABLE) || defined(MTK_JOYLINK_ENABLE)
fota_progress_update_cb         g_fota_progress_update_cb = NULL;
fota_download_len_update_cb     g_fota_download_len_update_cb = NULL;
...
void fota_download_register_download_len_update_cb(fota_download_len_update_cb download_len_update_cb)
{
    if (download_len_update_cb != NULL) {
        g_fota_download_len_update_cb = download_len_update_cb;
    }
}
#endif
...
#if defined(ANDLINK_ENABLE) || defined(MTK_JOYLINK_ENABLE)
        if (g_fota_download_len_update_cb != NULL) {
            g_fota_download_len_update_cb(count);
        }
