...
#endif
#else /* MTK_FOTA_VIA_RACE_CMD */
#ifdef FOTA_FS_ENABLE
#error "ERR: Both MTK_FOTA_VIA_RACE_CMD and FOTA_FS_ENABLE are defined!"
#endif
...
fota_ret_t fota_trigger_update(void)
{
    ...
    if (ret < HAL_FLASH_STATUS_OK) {
        FOTA_LOG_MSGID_E("[FOTA_SDK]Hal flash initialize fail, ret = %d", 1, ret);
    ...
    if (ret < HAL_FLASH_STATUS_OK) {
        FOTA_LOG_MSGID_E("[FOTA_SDK]erase 4k block fail, ret = %d", 1, ret);
        return FOTA_TRIGGER_FAIL;
    }

    /* write pattern into 512 bytes ahead of the reserved partition end address */
    FOTA_LOG_MSGID_I("[FOTA_SDK]flag address is 0x%x", 1, FOTA_TRIGGER_FLAG_ADDRESS);
    ret = hal_flash_write(FOTA_TRIGGER_FLAG_ADDRESS, (const uint8_t *)&pattern, sizeof(uint32_t));
    if (ret < HAL_FLASH_STATUS_OK) {
        FOTA_LOG_MSGID_I("[FOTA_SDK]Trigger update result is %d", 1, ret);
        return FOTA_TRIGGER_FAIL;
    } else {
        FOTA_LOG_MSGID_I("[FOTA_SDK]Trigger update result is %d", 1, ret);
...
fota_ret_t fota_trigger_update(void)
{
    ...
    if (FR_OK != res) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to create fota trigger file, error =", 0, (uint32_t)(res));
        result = FOTA_TRIGGER_FAIL;
    }
    res = f_write(&fdst, (const uint8_t *)&pattern, sizeof(uint32_t) - 1, &length_written);
    if (FR_OK != res) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to trigger flag to file, error = %d", 1, (uint32_t)res);
        result = FOTA_TRIGGER_FAIL;
    }

    res = f_close(&fdst);
    if (FR_OK != res) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to close fota trigger file!", 0);
        result = FOTA_TRIGGER_FAIL;
    }
...
bool readAndClearBlFotaUpdateInfo(bl_fota_update_info_t *pInfo)
{
    ...
    if (FR_OK != res) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to open trigger file, error =%d", 1, (uint32_t)(res));
        result = false;
    }

    res = f_read(&fdst, (void *)pInfo, sizeof(bl_fota_update_info_t), &length_read);
    if ((FR_OK != res) || (length_read != sizeof(bl_fota_update_info_t))) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to read update info res =%d, length_read=%d", 2, (uint32_t)(res), length_read);
        result = false;
    }

    res = f_close(&fdst);
    if (FR_OK != res) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to close update info file!", 0);
        result = false;
    }

    res = f_unlink(_T(FOTA_TRIGGER_FLAG_FILE));
    if (FR_OK != res) {
        FOTA_LOG_MSGID_E("[FOTA] Failed to delete update info file!", 0);
        result = false;
    }
