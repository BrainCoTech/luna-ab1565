...
atci_status_t atci_cmd_hdlr_gpio(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    const char *current_state[8] = {"1ma ", \
                                    "2ma ", \
                                    "4ma", \
                                    "8ma", \
                                    "12ma", \
    ...
    const char *current_state[4] = {"2ma ", \
                                    "4ma ", \
                                    "6ma", \
                                    "8ma"
                                   };
    ...
    switch (parse_cmd->mode) {
...
#if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
            strncpy((char *)resonse.response_buf, "Not support GPIO AT command", strlen("Not support GPIO AT command"));
            resonse.response_len = strlen((char *)resonse.response_buf);
        ...
        case ATCI_CMD_MODE_EXECUTION:
            ...
            if (strncmp(parse_cmd->string_ptr, "AT+EGPIO=GPIO_GET:", strlen("AT+EGPIO=GPIO_GET:")) == 0) {
                ...
                for (i = pin_index[0]; i <= pin_index[1]; i++) {
                    ...
                    ret_len1 = snprintf((char *)&resonse.response_buf[resonse.response_len], ATCI_UART_TX_FIFO_BUFFER_SIZE - resonse.response_len, \
                                        "GPIO%-2d, md=%d, %s, di=%d, do=%d, %s, %s\r\n", \
                                        (int)i, (int)gpio_state.mode, (char *)direct[gpio_state.dir], (int)gpio_state.din, \
                                        (int)gpio_state.dout, (char *)current_state[(uint8_t)gpio_state.current_type], (char *)(pull_state[gpio_state.pull_type]));
            ...
            /*  set GPIO direction. */
                ...
                if (config_data[0] > 1) {
                    const char *str1 = "[error] inupt diretion is errror!\r\n 0:intput\r\n 1:output\r\n";
                    strncpy((char *)resonse.response_buf, str1, strlen(str1));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
            ...
            /*  set GPIO pull state. */
                ...
                if (config_data[0] > 2) {
                    const char *str2 = "[error] inupt pull state is errror!\r\n 0:pull up\r\n 1:pull down\r\n 2:disable pull\r\n";
                    strncpy((char *)resonse.response_buf, str2, strlen(str2));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
            ...
            /*  set GPIO pupd r0 r1 state. */
                ...
                if ((config_data[0] > 1) || (config_data[1] > 1) || (config_data[2] > 1)) {
                    const char *str3 = "[error] inupt pupd-r0-r1 is errror!\r\n 0-0-0/1-0-0: Hi-Z\r\n 0-0-1/0-1-0:pull-up 47k\r\n 0-1-1:pull-up 23.5k\r\n 1-0-1/1-1-0:pull-down 47k\r\n 1-1-1:pull-down 23.5K\r\n";
                    strncpy((char *)resonse.response_buf, str3, strlen(str3));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
            ...
            /*  set GPIO output data. */
                ...
                if (config_data[0] > 1) {
                    const char *str4 = "[error] inupt output data is errror!\r\n 0:low\r\n 1:high\r\n";
                    strncpy((char *)resonse.response_buf, str4, strlen(str4));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
            ...
            /*  set GPIO current driving. */
                ...
                if (config_data[0] > 3) {
                    ...
                    strncpy((char *)resonse.response_buf, str5, strlen(str5));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
            ...
            /* Set serveral configurations to one GPIO. */
                ...
                error_flag = 0;
                if (config_data[1] > 1) {
                    const char *str6 = "[error] inupt diretion is errror!\r\n 0:intput\r\n 1:output\r\n";
                    strncpy((char *)resonse.response_buf, str6, strlen(str6));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    error_flag++;
                ...
                if (config_data[2] > 1) {
                    const char *str7 = "[error] inupt output data is errror!\r\n 0:low\r\n 1:high\r\n";
                    strncpy((char *)resonse.response_buf, str7, strlen(str7));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    error_flag++;
                ...
                if (config_data[3] > 2) {
                    const char *str8 = "[error] inupt pull state is errror!\r\n 0:pull up\r\n 1:pull down\r\n 2:disable pull\r\n";
                    strncpy((char *)resonse.response_buf, str8, strlen(str8));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    error_flag++;
                ...
                if (config_data[4] > 3) {
                    const char *str9 = "[error] inupt driving type is errror!\r\n 0:4ma\r\n 1:8ma\r\n 2:12ma\r\n 3:16ma\r\n";
                    strncpy((char *)resonse.response_buf, str9, strlen(str9));
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    error_flag++;
                ...
                resonse.response_len = snprintf((char *)(resonse.response_buf), ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                                    "set GPIO%-2d, md=%d, %s, di=%d, do=%d, %s, %s done!\r\n", \
                                    (int)pin_index[0], (int)gpio_state.mode, (char *)direct[gpio_state.dir], (int)gpio_state.din, (int)gpio_state.dout, \
                                    (char *)current_state[(uint8_t)gpio_state.current_type], (char *)(pull_state[gpio_state.pull_type]));
                resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                atci_send_response(&resonse);
