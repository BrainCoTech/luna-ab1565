...
void USB2UART_Tx_DMA1_Callback(void)
{
    ...
    USB2UART_Thru_Tx_DMA_Callback(SERIAL_PORT_DEV_USB_COM1);
#else
...
#ifdef MTK_PORT_SERVICE_ENABLE
#ifdef MTK_USB2UART_DMA_THRU
        USB2UART_Thru_Tx_DMA_Callback(SERIAL_PORT_DEV_USB_COM2);
#else
...
void USB_Acm_BulkOut_Hdr(uint8_t port)
{
#if 0//(PRODUCT_VERSION == 1552)
    uint32_t nCount;
    uint8_t data[512];

    BUFFER_INFO *rx_buf_info = &(USB2UARTPort[port].Rx_Buffer);

    //LOG_I(common, "USB_Acm_BulkOut_Hdr\n");
    ...
    if (nCount != 0) {
        ...
        USB2UARTPort[port].rx_cb((uint32_t)port, nCount, (uint32_t)data);
    } else {
...
#if 0
USB_Send_Message(USB_ACM_MSG, data);

//dma
    //DMA echo test
    LOG_MSGID_E(common, "USB_Acm_BulkOut_Hdr: write %d, data, 0x%x\n",2, nCount, data[0]);
...
//fifo
//LOG_E(common, "USB_Acm_BulkOut_Hdr: write %d, data, 0x%x\n", nCount, data[0]);
//LOG_HEXDUMP_D(common, "USB2UART_BUFF dump\n", buffaddr, real_count);

hal_usb_write_endpoint_fifo(g_UsbACM[port].txpipe->byEP, nCount, data);

hal_usb_set_endpoint_tx_ready(g_UsbACM[port].txpipe->byEP);

while (!hal_usb_is_endpoint_tx_empty(g_UsbACM[port].txpipe->byEP));
