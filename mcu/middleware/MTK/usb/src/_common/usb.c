...
/**
 * AG3335's hal doesn't support this function.
 * And doesn't need this feature.
 */
#if defined(AG3335)
#define hal_usb_set_speed(x)
#endif
...
{
    uint8_t nvkey_setting_0 = 0;

/**
 * NOTE NVkey define guard reference from nvkey_id_list.h
 */
#if ((PRODUCT_VERSION == 1552) || (PRODUCT_VERSION == 2822) || defined(AM255X) || defined(AB156X) || defined(MT2822))
    uint32_t nvkey_length = 1;
    uint8_t status;
#endif
...
#if ((PRODUCT_VERSION == 1552) || (PRODUCT_VERSION == 2822) || defined(AM255X) || defined(AB156X) || defined(MT2822))
    /* Choose HS or FS by nvkey_usb_hsen*/
    status = nvkey_read_data(NVKEYID_USB_SETTING_0, &nvkey_setting_0, &nvkey_length);
    if (status != NVKEY_STATUS_OK) {
        LOG_MSGID_I(USB, "USB_Init, Can't read NVKEYID_USB_SETTING_0, set as 0", 0);
        nvkey_setting_0 = 0;
    }
    LOG_MSGID_I(USB, "USB_Init, nvkey_setting_0 bit 0 = %d, 1:HS, 0:FS", 1, (nvkey_setting_0 & USB_NEKEY_HS_ENABLE));
#endif
    hal_usb_set_speed((bool)(nvkey_setting_0 & USB_NEKEY_HS_ENABLE));
...
#ifdef MTK_USB11
extern void USB_Read_TX_Interrupt_Status(void);
#endif
