...
ATTR_ZIDATA_IN_NONCACHED_RAM_16BYTE_ALIGN static uint8_t g_spi_tx_buffer[SPI_TX_BUFFER];
ATTR_ZIDATA_IN_NONCACHED_RAM_16BYTE_ALIGN static uint8_t g_spi_rx_buffer[SPI_RX_BUFFER];
...
static void flash_dma_callback(hal_spi_master_callback_event_t event, void *user_data)
{
    ...
    //LOG_MSGID_I(common, "flash_dma_callback\r\n", 0);
}
...
static void serial_nand_set_feature(uint8_t reg, uint8_t value)
{
    ...
    if (ret != HAL_SPI_MASTER_STATUS_OK) {
        LOG_MSGID_E(common, "SET FEATURE fail\r\n", 0);
    }
}
...
static void serial_nand_get_feature(uint8_t reg, uint8_t *value)
{
    ...
    if (ret != HAL_SPI_MASTER_STATUS_OK) {
        LOG_MSGID_E(common, "GET FEATURE fail\r\n", 0);
    }
                                ...
                                uint32_t spare_offset, uint8_t *spare_data, uint32_t spare_length)
{
    ...
    //LOG_MSGID_I("[Read]:page=0x%x,column=%d,buffer=0x%x,length=%d,spare_offset=0x%x,spare_data=%08x,spare_length=%d\r\n", 7,
    //               page, column, (uint32_t)buffer, length, spare_offset, (uint32_t)spare_data, spare_length);

    /* 13H (PAGE READ to cache) */
    //LOG_MSGID_I("PAGE READ to cache\r\n", 0);
    ...
    if (buffer != NULL) {
        ...
        if (flash_internal_dma_read(g_spim_port, &spi_send_and_receive_config) == false) {
            LOG_MSGID_E(common, "Read from cache1\r\n", 0);
        }
        memcpy(buffer, &g_spi_rx_buffer[4], length);
    }
    if (spare_data != NULL) {
        //LOG_MSGID_I("Read from cache 2\r\n", 0);
        g_spi_tx_buffer[0] = SERIAL_NAND_CMD_NORM_READ;
                                 ...
                                 uint32_t spare_offset, uint8_t *spare_data, uint32_t spare_length)
{
    ...
    //LOG_MSGID_I("[Write]: page=0x%x,column=0x%x,buffer=%08x,length=%d,spare_offset=%d,spare_data=%08x,spare_length=%d\r\n", 7,
    //                page, column, (uint32_t)buffer, length, spare_offset, (uint32_t)spare_data, spare_length);
    ...
    if (buffer != NULL) {
        //LOG_MSGID_I("Program load random data for normal data\r\n", 0);
        ...
        if (flash_internal_dma_write(g_spim_port, g_spi_tx_buffer, 3 + length) == false) {
            LOG_MSGID_E(common, "Program load fail\r\n", 0);
        }
        //LOG_MSGID_I("Program load ok\r\n", 0);
    }
    if (spare_data != NULL) {
        //LOG_MSGID_I("Program load random data for spare data\r\n", 0);
        //memset(g_spi_tx_buffer, 0xFF, sizeof(g_spi_tx_buffer));
        g_spi_tx_buffer[0] = SERIAL_NAND_CMD_PLOAD_RANDOM;
    ...
    ret = hal_spi_master_send_polling(g_spim_port, g_spi_tx_buffer, 4);
    if (ret != HAL_SPI_MASTER_STATUS_OK) {
        LOG_MSGID_E(common, "PROGRAM EXECUTE fail 1\r\n", 0);
    }
    ...
    do {
        ...
        if (status & SERIAL_NAND_STATUS_PFAIL) {
            LOG_MSGID_E(common, "PROGRAM EXECUTE fail: %02x, page: %d \r\n",2, status, page);
            return;
        }
    } while (status & SERIAL_NAND_STATUS_BUSY);

    /* 06H (WRITE DISABLE) */
    //LOG_MSGID_I("WRITE DISABLE\r\n", 0);
    g_spi_tx_buffer[0] = SERIAL_NAND_CMD_WRDI;
    ret = hal_spi_master_send_polling(g_spim_port, g_spi_tx_buffer, 1);
    if (ret != HAL_SPI_MASTER_STATUS_OK) {
        LOG_MSGID_E(common, "WRITE DISABLE fail\r\n", 0);
    }
}
...
serial_nand_status_t serial_nand_init(hal_spi_master_port_t spim_port, uint32_t frequency)
{
    ...
    serial_nand_get_feature(SERIAL_NAND_STATUS_REG, &status);
    LOG_MSGID_E(common, "Flash status register: %02x\r\n",1, status);
    serial_nand_get_feature(SERIAL_NAND_FEATURE_REG, &status);
    LOG_MSGID_E(common, "Flash feature register: %02x\r\n",1, status);

    /* unlock the flash block */
    serial_nand_get_feature(SERIAL_NAND_PROTEC_REG, &status);
    LOG_MSGID_E(common, "Flash Protection register: %02x\r\n",1, status);
...
serial_nand_status_t serial_nand_erase(uint32_t address, uint32_t length)
{
    ...
    uint8_t status;
    hal_spi_master_status_t ret;
    //LOG_MSGID_I("[ERASE]:address=%x,length=0x%x \r\n", 2, address, length);
    ...
    ret = hal_spi_master_send_polling(g_spim_port, g_spi_tx_buffer, 1);
    if (ret != HAL_SPI_MASTER_STATUS_OK) {
        LOG_MSGID_E(common, "WRITE ENBALE command fail\r\n", 0);
        return SERIAL_NAND_STATUS_ERROR;
    }
    ...
    while (block_count) {
        ...
        ret = hal_spi_master_send_polling(g_spim_port, g_spi_tx_buffer, 4);
        if (ret != HAL_SPI_MASTER_STATUS_OK) {
            LOG_MSGID_E(common, "Block Erase fail 1\r\n", 0);
            return SERIAL_NAND_STATUS_ERROR;
        }
        ...
        do {
            ...
            if (status & SERIAL_NAND_STATUS_EFAIL) {
                LOG_MSGID_E(common, "Block Erase status fail: %02x  addr: 0x%x \r\n",2, status, address);
                return SERIAL_NAND_STATUS_ERROR;
            }
        } while (status & SERIAL_NAND_STATUS_BUSY);
    ...
    ret = hal_spi_master_send_polling(g_spim_port, g_spi_tx_buffer, 1);
    if (ret != HAL_SPI_MASTER_STATUS_OK) {
        LOG_MSGID_E(common, "Write disable command fail\r\n", 0);
        return SERIAL_NAND_STATUS_ERROR;
    }
