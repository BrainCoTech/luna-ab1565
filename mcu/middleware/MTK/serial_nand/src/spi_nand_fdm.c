    ...
    if (SysInitStatus_Query() == 1) {
        if (HAL_NVIC_QUERY_EXCEPTION_NUMBER == 0) {
            if (xSemaphoreTake(g_xSemaphore_ms, portMAX_DELAY) == pdFALSE) {
                LOG_MSGID_E(common, "ms_mutex_lock error\r\n", 0);
            }
        } else {
            //LOG_MSGID_E("take ms_mutex_lock in \r\n", 0);
        }
    }
}
...
void ms_mutex_unlock(void)
{
    ...
    if (SysInitStatus_Query() == 1) {
        ...
        //LOG_MSGID_E(common, "take ms_mutex_lock in \r\n", 0);
    }
}
...
int32_t spi_nand_fdm_LowLevelFormat(void)
{
    ...
    for (block_index = 0; block_index < 2048; block_index++) {
        ...
        if (status != 0) {
            ms_mutex_unlock();
            //LOG_MSGID_E(common, "spi_nand_fdm_LowLevelFormat block_index = %d, status = %d \r\n", 2, block_index, status );
            return (-1);
