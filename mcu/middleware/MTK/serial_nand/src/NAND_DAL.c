                ...
                while (1) {
                    ...
                    if ( (ZoneNo == ProgBadAddr[Count].Zone) &&
                        ...
                        if (PF_EF_AutoTest == TRUE) {
                            LOG_MSGID_I(common, "Program Before PFTestCase:0x%X Block:0x%X Page:0x%X\r\n",3, PFTestCase, ProgBadAddr[Count].Block, Add_Cnvt.pageIdx );

                            PFTestCase &= (~PFTrigerByTestCase);
                            LOG_MSGID_I(common, "Program After PFTestCase:0x%X Block:0x%X Page:0x%X\r\n",3, PFTestCase, ProgBadAddr[Count].Block, Add_Cnvt.pageIdx );
...
uint32_t DAL_ReadPage_internal(DAL_ACC_Type Acc_Type, uint8_t ZoneNo, uint16_t *Bank, uint16_t PageAddr, uint16_t PageLength, uint8_t *Data, uint8_t *Spare)
{
    ...
    while ( PageLength ) {
        ...
        for ( i = 0; i < readingLen; i++) {
            ...
            LOG_MSGID_I(common, "Read CE%d page %d to 0x%x\n\r",3, Mtd_para.ce, Mtd_para.pageIdx[i], Mtd_para.buff);
#endif
            if (Flash_Info.deviceInfo_CE[Add_Cnvt.ce].deviceSize * 1024 * 1024 <= Flash_Info.pageSize * Add_Cnvt.phyPage) {
                //ASSERT_RET(0, MTD_STATUS_ERROR);
                free_MTD_lock();
...
DAL_ReadPage_Retry_R:
        trycount++;
#if ( defined(__NAND_FLASH_HARDWARE_TEST__) && defined(__MAUI_BASIC__) && !defined(__UBL__))
        LOG_MSGID_I(common, "DAL_ReadPage Retry %d time\r\n", 1, trycount);
...
DAL_COPY_PAGE_RETRY:
                ...
                if (try_count < 3) {
#if ( defined(__NAND_FLASH_HARDWARE_TEST__) && defined(__MAUI_BASIC__) && !defined(__UBL__))
                    LOG_MSGID_I(common, "DAL_CopyPage Retry %d time\r\n", 1, try_count);
            ...
            for (Index = 0; Index < writingLen; Index++) {
                ...
                while (1) {
                    ...
                    if ( (bCurrZone == ProgBadAddr[Count].Zone) &&
                        ...
                        if (PF_EF_AutoTest == TRUE) {
                            LOG_MSGID_I(common, "Copy Before PFTestCase:0x%X Block:0x%X Page:0x%X\r\n", 3, PFTestCase, ProgBadAddr[Count].Block, Add_Cnvt.pageIdx );

                            PFTestCase &= (~PFTrigerByTestCase);
                            LOG_MSGID_I(common, "Copy After PFTestCase:0x%X Block:0x%X Page:0x%X\r\n", 3, PFTestCase, ProgBadAddr[Count].Block, Add_Cnvt.pageIdx );
            ...
            for (Index = 0; Index < erasingLen; Index++) {
                ...
                while (1) {
                    ...
                    if ((ZoneNo == EraseBadAddr[Count].Zone) &&
                        ...
                        if (PF_EF_AutoTest == TRUE) {
                            LOG_MSGID_I(common, "Before EFTestCase:0x%X \r\n", 1, EFTestCase);
                            EFTestCase &= (~EFTrigerByTestCase);
                            LOG_MSGID_I(common, "After EFTestCase:0x%X \r\n", 1, EFTestCase);
                        }
                    }
                    Count++;
                }
...
uint32_t NFB_ReadPhysicalPage(uint32_t PhyBlock, uint32_t PhyPage, void *Data)
{
    ...
    LOG_MSGID_I(common, "NFB download into address %xh\r\n", 1, Data);
...
#else // defined(__UBL__)
                LOG_MSGID_I(common, "NFB compare error in ECC test. Address %x. Wrong data=%x, Right data=%x\r\n",3, &(((uint32_t *)Data)[Offset]),
                      ((uint32_t *)Data)[Offset], D_Buff[(_DAL_MAX_PAGE_SIZE_ / 4) + Offset]);
...
#else // defined(__UBL__)
        LOG_MSGID_I(common, "NFB compare error in ECC test.\r\n", 0);
#endif // defined(__UBL__)
