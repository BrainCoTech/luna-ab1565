...
bt_status_t race_le_audio_notify_big_info(uint8_t status, bt_sink_srv_cap_event_base_biginfo_adv_report_t *msg)
{
    RACE_LOG_MSGID_I("[RACE_LEA] notify_big_info", 0);
...
void *race_le_audio_set_dongle_stream_mode_handler(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    ...
    RACE_LOG_MSGID_I("[RACE_LEA] set_dongle_stream_mode_handler, set mode:%d", 1, pCmd->mode);
...
void *race_le_audio_get_dongle_stream_mode_handler(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    ...
    RACE_LOG_MSGID_I("[RACE_LEA] get_dongle_stream_mode_handler", 0);
...
void *race_le_audio_dongle_broadcast_device_handler(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    uint8_t *p_Data = &pCmdMsg->payload[0];
    uint8_t opt = 0;
    int32_t ret = RACE_ERRCODE_FAIL;

    RACE_LOG_MSGID_I("[RACE_LEA] dongle_broadcast_device_handler", 0);
    ...
    if (pRaceHeaderCmd->hdr.type == RACE_TYPE_COMMAND ||
        ...
        switch (pRaceHeaderCmd->hdr.id) {
            ...
            case RACE_LEAUDIO_DONGLE_BROADCAST_DEVICE: {
                ptr = race_le_audio_dongle_broadcast_device_handler(pRaceHeaderCmd, channel_id);
                break;
            }
#endif /* AIR_BLE_AUDIO_DONGLE_ENABLE */
            default: {
