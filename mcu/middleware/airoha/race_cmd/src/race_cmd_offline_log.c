...
void *RACE_QUERY_OFFLINE_DUMP_REGION_INFO_HDR(PTR_RACE_COMMON_HDR_STRU pCmdMsg, uint16_t length, uint8_t channel_id)
{
    ...
    RACE_LOG_MSGID_I("OFFLINE DUMP: query_seq_range status:%d [0:fail 1:pass] type:%d min:%d max:%d", 4,
                        pEvt->status, pEvt->region_type, pEvt->res_min_seq, pEvt->res_max_seq);
...
void *RACE_QUERY_OFFLINE_DUMP_REGION_ADDRESS_HDR(PTR_RACE_COMMON_HDR_STRU pCmdMsg, uint16_t length, uint8_t channel_id)
{
    ...
    RACE_LOG_MSGID_D("OFFLINE DUMP: query_by_seq status:%d [0:fail 1:pass] type:%d length:%d address:0x%08x", 4,
                        pEvt->status, pEvt->region_type, pEvt->block_length, pEvt->dump_address);
