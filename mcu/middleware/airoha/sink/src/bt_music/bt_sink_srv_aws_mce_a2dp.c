...
void bt_sink_srv_aws_mce_a2dp_app_report_callback(bt_aws_mce_report_info_t *para)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]app_report-module: 0x%0x, data_len:0x%08x, type:0x%02x, len: 0x%02x, val: 0x%02x, a2dp_dev:0x%x", 6,
...
#endif
    bt_sink_srv_report_id("[mce_a2dp]a2dp_init-aws_aid: %d", 1, ctx->aws_aid);
}
...
#else
    bt_sink_srv_report_id("[mce_a2dp]dump_eir-state: %d, vol: %d, nclk: 0x%08x, nclk_intra: 0x%08x, ts: 0x%08x, ratio:0x%02x, samples:0x%04x, latency:0x%08x, special_device_flag:0x%02x, dev_flag:0x%02x, a2dp_mtu:0x%04x", 11,
                          a2dp_eir->state, a2dp_eir->vol_lev, a2dp_eir->nclk,
                          a2dp_eir->nclk_intra,
                          a2dp_eir->ts, a2dp_eir->ratio,
                          a2dp_eir->sample_count,
...
void bt_sink_srv_aws_mce_a2dp_ami_hdr(bt_sink_srv_am_id_t aud_id, bt_sink_srv_am_cb_msg_class_t msg_id, bt_sink_srv_am_cb_sub_msg_t sub_msg, void *param)
{
    ...
    if (ctx->aws_aid == aud_id) {
        if ((AUD_AWS_A2DP_PROC_IND == msg_id)
            && (AUD_STREAM_EVENT_DATA_REQ == sub_msg || AUD_CODEC_MEDIA_AWS_CHECK_CLOCK_SKEW == sub_msg)) {
            return;
        }

        bt_sink_srv_report_id("[mce_a2dp]ami_hdr--msg_id:%d,sub_msg:%d, run_dev:0x%x", 3, msg_id, sub_msg, run_dev);

        switch (msg_id) {
            case AUD_SINK_OPEN_CODEC: {
                if (run_dev && (run_dev->flag & BT_SINK_SRV_MUSIC_FLAG_WAIT_AMI_OPEN_CODEC)) {
                    bt_sink_srv_report_id("[mce_a2dp]open codec success--flag:0x%x, op:0x%x, substate:0x%x, sec_type:0x%x",
                    ...
                    if (run_dev->codec.sec_type) {
                        bt_sink_srv_report_id("[sink][mce_a2dp]set content protection", 0);
            ...
            case AUD_A2DP_PROC_IND: {
                switch (sub_msg) {
                    case AUD_A2DP_DL_REINIT_REQUEST: {
                        ...
                        bt_sink_srv_report_id("[sink][mce_a2dp]reinitial sync para_data:%d, mode:0x%x", 2, *para_data, mode);
                    ...
                    case AUD_A2DP_ACTIVE_LATENCY_REQUEST: {
                        if (BT_SINK_SRV_MUSIC_MODE_BROADCAST != bt_sink_srv_music_get_spk_mode()) {
                            ...
                            bt_sink_srv_report_id("[sink][a2dp] client dsp ALC request, ami_ret:0x%02x, vol:0x%02x", 2, ami_ret, ctx->vol_lev);
                        }
#endif
                        break;
                    }
...
static void bt_sink_srv_aws_mce_a2dp_handle_state_change_ind(bt_aws_mce_state_change_ind_t *state_ind, bool need_change_state)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]state_change(s)-aws_hd: 0x%x, aws_handle:0x%x, state:0x%0x, role:0x%x, need_change_state:0x%x", 5,
...
static int32_t bt_sink_srv_aws_mce_a2dp_handle_connect_ind(bt_aws_mce_connected_t *conn_ind)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]connect_cnf(s)-aws_hd: 0x%x, conn_ind->handle, addr--0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x",
    ...
    bt_sink_srv_report_id("[mce_a2dp]connect_cnf(e)-ret: %d, dev:0x%x", 2, ret, dev);
...
static int32_t bt_sink_srv_aws_mce_a2dp_handle_disconnect_ind(uint32_t handle, bool free_handle)
{
    ...
    if (dev) {
        bt_sink_srv_report_id("[mce_a2dp]disconnect_ind[s]-aws_hd: 0x%x, free_handle:0x%x, flag:0x%x, substate:0x%x, context_flag:0x%x",
    ...
    bt_sink_srv_report_id("[mce_a2dp]disconnect_ind[e]-dev: 0x%x", 1, dev);
    return 0;
}
...
static void bt_sink_srv_aws_mce_a2dp_handle_a2dp_start_ind(uint32_t handle, bt_sink_srv_aws_mce_a2dp_base_eir_t *a2dp_eir)
{
    ...
    if (dev) {
        bt_sink_srv_report_id("[mce_a2dp]strat_ind-hd: 0x%08x, flag: 0x%08x, op: 0x%08x, substate:0x%x",
    ...
    bt_sink_srv_report_id("[mce_a2dp][mce_a2dp]strat_ind-hd(e)-dev:0x%x", 1, dev);
}
...
static void bt_sink_srv_aws_mce_a2dp_handle_a2dp_suspend_ind(uint32_t handle, bt_sink_srv_aws_mce_a2dp_base_eir_t *a2dp_eir)
{
    ...
    if (dev) {
        bt_sink_srv_report_id("[mce_a2dp]suspend_ind-hd(s)-flag:0x%08x, run_dev:0x%x", 2, dev->flag, ctx->run_dev);
        if (dev->flag & BT_SINK_SRV_MUSIC_FLAG_WAIT_LIST_SINK_PLAY) {
        audio_src_srv_del_waiting_list(dev->handle);
        BT_SINK_SRV_REMOVE_FLAG(dev->flag, BT_SINK_SRV_MUSIC_FLAG_WAIT_LIST_SINK_PLAY);
        BT_SINK_SRV_REMOVE_FLAG(dev->op, BT_SINK_SRV_MUSIC_OP_PLAY_IND);
    ...
    bt_sink_srv_report_id("[mce_a2dp]suspend_ind-hd-dev:0x%08x", 1, dev);
}
...
static void bt_sink_srv_aws_mce_a2dp_handle_update_base(uint32_t handle, bt_sink_srv_aws_mce_a2dp_base_eir_t *a2dp_eir)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]update_base(e)--dev:0x%x, state: 0x%02x, us_asi:0x%08x, nclk: 0x%08x, intra: 0x%08x, drift:0x%04x", 6,
...
static void bt_sink_srv_aws_mce_a2dp_handle_sync_vol(uint32_t handle, bt_sink_srv_aws_mce_a2dp_base_eir_t *a2dp_eir)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]sync_vol-dev:0x%x, ami_ret: %d, vol: %d", 3, dev, ami_ret, ctx->vol_lev);
}
...
static void bt_sink_srv_aws_mce_a2dp_set_sta(uint32_t gap_handle)
{
    ...
    if (run_dev && (run_dev->op & BT_SINK_SRV_MUSIC_OP_PLAY_IND) && (run_dev->op & BT_SINK_SRV_MUSIC_OP_CODEC_OPEN)) {
        ...
        if (ctx->packet_count) {
            ...
            bt_sink_srv_report_id("[mce_a2dp]play_info, pta-nclk:0x%08x, pta-intra:0x%04x, asi:0x%08x", 3,
...
static bt_status_t bt_sink_srv_aws_mce_a2dp_send_action(uint8_t action_type, uint8_t action_value, void *param, uint32_t param_len)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]control_music--aws_a2dp_dev:0x%x, ret: %d, action_type:%d, action_value:%d, firmware_type:%d",
...
void bt_sink_srv_aws_mce_a2dp_play(audio_src_srv_handle_t *handle)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]play(s)--hd: 0x%x, type: %d, flag: 0x%x, op: 0x%x, role:0x%x, aws_aid:0x%x", 5,
...
void bt_sink_srv_aws_mce_a2dp_stop(audio_src_srv_handle_t *handle)
{
    ...
    bt_utils_assert(aws_a2dp_dev);
    bt_sink_srv_report_id("[mce_a2dp]stop(s)--hd: 0x%x, type: %d, flag: 0x%x, op: 0x%x, role:0x%x", 5,
...
void bt_sink_srv_aws_mce_a2dp_suspend(audio_src_srv_handle_t *handle, audio_src_srv_handle_t *int_hd)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]suspend(s)--hd:0x%x, type:%d, int:0x%x, type:%d, aws_a2dp_dev:0x%x, flag: 0x%x, op: 0x%x, substate:0x%x, role:0x%x",
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_data(bt_bd_addr_t *dev_addr, bt_sink_srv_aws_mce_a2dp_base_eir_t *a2dp_eir)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]send_if_packet(s), aws_handle:0x%08x,  aws_state:0x%02x", 2, aws_handle, aws_state);
    if (BT_SINK_SRV_MUSIC_MODE_BROADCAST != bt_sink_srv_music_get_spk_mode()
    ...
    bt_sink_srv_report_id("[mce_a2dp]send_eir_play_ind-1:0x%x,2:0x%x,3:0x%x,4:0x%x,5:0x%x,6:0x%x,7:0x%x,8:0x%x,9:0x%x,10:0x%x,11:0x%x,12:0x%x,13:0x%x",
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_disconn(bt_bd_addr_t *dev_addr)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]bt_sink_srv_aws_mce_a2dp_send_eir_disconn", 0);
}
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_start_streaming(uint32_t a2dp_hd)
{
    ...
    bt_sink_srv_report_id("[mce_a2dp]send_eir_start_streaming", 0);
}
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_suspend_streaming(uint32_t a2dp_hd)
{
    ...
    bt_sink_srv_memcpy(&(a2dp_base_eir.codec), &(a2dp_dev->codec), sizeof(bt_a2dp_codec_capability_t));
    bt_sink_srv_aws_mce_a2dp_send_eir_data(&(a2dp_dev->dev_addr), &a2dp_base_eir);
    bt_sink_srv_report_id("[mce_a2dp]send_eir_suspend_streaming", 0);
    /* clear fec cached media data && fec data when a2dp suspend */
#ifdef MTK_BT_SPEAKER_ENABLE
#ifdef MTK_BT_SPEAKER_FEC_ENABLE
    if (BT_SINK_SRV_MUSIC_MODE_BROADCAST == bt_sink_srv_music_get_spk_mode()) {
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_update_base(uint32_t a2dp_hd)
{
    ...
    /* Send A2DP EIR packet */
    bt_sink_srv_aws_mce_a2dp_send_eir_data(&(a2dp_dev->dev_addr), &a2dp_base_eir);
    bt_sink_srv_report_id("[mce_a2dp]send_eir_update_base-update_base_number:0x%x", 1, update_base_number);
}
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_vol_sync(uint32_t a2dp_hd, uint8_t change_type)
{
    ...
    /* Send A2DP EIR packet */
    bt_sink_srv_aws_mce_a2dp_send_eir_data(&(a2dp_dev->dev_addr), &a2dp_base_eir);
    bt_sink_srv_report_id("[mce_a2dp]send_eir_vol_sync", 0);
}
...
static void bt_sink_srv_aws_mce_a2dp_send_eir_play_ind(uint32_t a2dp_hd)
{
    ...
    /* Send A2DP EIR packet */
    bt_sink_srv_aws_mce_a2dp_send_eir_data(&(a2dp_dev->dev_addr), &a2dp_base_eir);
    bt_sink_srv_report_id("[mce_a2dp]send_eir_play_ind-play_info_number:0x%x", 1, play_info_number);
}
