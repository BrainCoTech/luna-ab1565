...
static int32_t bt_sink_srv_avrcp_handle_connect_cnf(bt_avrcp_connect_cnf_t *conn_cnf, bt_status_t status)
{
    ...
    if (dev) {
        bt_sink_srv_report_id("[sink][avrcp]connect_cnf-avrcp_hd: 0x%x, status:0x%x, dev:0x%x, flag:0x%x, op:0x%x, avrcp_role:0x%x, conn_bit:0x%x",
        ...
        if (BT_STATUS_SUCCESS == status) {
            ...
            bt_avrcp_register_notification(conn_cnf->handle, BT_AVRCP_EVENT_PLAYBACK_STATUS_CHANGED, 0);
...
static int32_t bt_sink_srv_avrcp_handle_event_notification_ind(bt_avrcp_event_notification_t *noti_ind, bt_status_t status)
{
    ...
    bt_sink_srv_report_id("[sink][avrcp]event_notification_ind--noti_ind:0x%x, status:0x%x", 2, noti_ind, status);
    if (noti_ind && BT_AVRCP_EVENT_PLAYBACK_STATUS_CHANGED == noti_ind->event_id) {
...
static int32_t bt_sink_srv_avrcp_handle_register_notification_ind(bt_avrcp_register_notification_commant_t *register_ind, bt_status_t status)
{
    ...
    if (BT_STATUS_SUCCESS == status) {
        if (BT_AVRCP_EVENT_VOLUME_CHANGED == register_ind->event_id) {
            ...
            bt_sink_srv_report_id("[sink][avrcp]register_notificatin_ind[s]-volume_value: %d, ret:0x%x", 2, rsp.volume, ret);
        }
    }
...
static void bt_sink_srv_avrcp_set_absolute_volume_timer(uint32_t timer_id, uint32_t data)
{
    ...
    bt_avrcp_set_absolute_volume_event_t set_vol_ind = {0};
...
bt_status_t bt_sink_srv_avrcp_action_handler(bt_sink_srv_action_t action, void *param)
{
    ...
    bt_sink_srv_report_id("[sink][avrcp]process_avrcp_action-action:0x%x, ret: %d", 2, action, ret);
    bt_sink_srv_mutex_unlock();
...
bt_status_t bt_sink_srv_avrcp_common_callback(bt_msg_type_t msg, bt_status_t status, void *buffer)
{
    ...
    bt_sink_srv_report_id("[sink][avrcp]common_hdr-msg: 0x%x, status: %d, buffer:0x%x, ret:0x%x", 2, msg, status, buffer, ret);
...
int32_t bt_sink_srv_avrcp_change_track(uint32_t handle, bool is_ptrack)
{
    ...
    if ((dev) && (dev->conn_bit & BT_SINK_SRV_MUSIC_AVRCP_CONN_BIT)) {
        ret = bt_avrcp_send_pass_through_command(handle, is_ptrack?BT_AVRCP_OPERATION_ID_BACKWARD : BT_AVRCP_OPERATION_ID_FORWARD, 0);
    }
    bt_sink_srv_report_id("[sink][avrcp]change_track-is_ptrack:%d, hd:0x%x, dev:%d, ret:0x%x", 4, is_ptrack, handle, dev, ret);
    return ret;
}
...
void bt_sink_srv_init_role(bt_avrcp_role_t role)
{
    ...
    bt_sink_srv_report_id("[sink][avrcp]init_role:0x%x", 1, role);
    init.role = role;
...
static void bt_sink_srv_avrcp_as_tg_callback(uint32_t timer_id, uint32_t data)
{
    ...
    bt_sink_srv_report_id("[sink][avrcp]avrcp_as_tg_callback, change role to connect again, ret = %x, hd:0x%x, dev:0x%x", 3, ret, hd, dev);
    if (ret == BT_STATUS_SUCCESS) {
        dev->avrcp_hd = hd;
...
static void bt_sink_srv_avrcp_handle_play_status_notification(uint32_t handle, uint8_t avrcp_status, bt_status_t status)
{
    ...
    if (dev->avrcp_status == BT_AVRCP_STATUS_PLAY_PLAYING
        ...
        if (ctx->rho_flag) {
            ...
            uint32_t check_ret = bt_sink_srv_a2dp_int_user_conf_check(dev);
            if ((check_ret == BT_SINK_SRV_A2DP_INT_USER_CONF_ALLOW) || (check_ret == BT_SINK_SRV_A2DP_INT_USER_CONF_BYPASS))
#endif
            {
...
static bt_status_t bt_sink_srv_avrcp_get_folder_item(bt_sink_srv_music_device_t *dev, bt_sink_srv_avrcp_get_folder_items_parameter_t *parameter)
{
    ...
    bt_sink_srv_report_id("[sink][avrcp]get_folder_item, ret:0x%08x, conn_bit:0x%08x, handle:0x%08x, avrcp_flag:0x%x",
