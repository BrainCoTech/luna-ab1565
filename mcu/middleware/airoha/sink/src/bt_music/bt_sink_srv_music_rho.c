...
void bt_sink_srv_music_role_handover_get_data(const bt_bd_addr_t *addr, uint8_t *data)
{
    ...
    bt_sink_srv_report_id("[music_rho]get_data-sp_dev:0x%x", 1, sp_dev);
...
#ifdef  AIR_MULTI_POINT_ENABLE
        bt_sink_srv_report_id("[music_rho]get_data-conn:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x", 10,
...
#else
        bt_sink_srv_report_id("[music_rho]get_data-conn:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x", 9,
            change_ctx->conn_bit, change_ctx->flag, change_ctx->a2dp_status, change_ctx->avrcp_status, change_ctx->last_play_pause_action,
...
static void bt_sink_srv_music_role_handover_update_sp_context(bt_sink_srv_music_device_t *sp_dev, bt_bd_addr_t *sp_addr, bt_sink_srv_music_change_context_t *change_ctx)
{
    ...
    bt_sink_srv_report_id("[music_rho]update_sp_context-run_dev:0x%x, sp_dev:0x%x, change_ctx:0x%x-0x%x-0x%x-0x%x-0x%x-0x%x-0x%x-0x%x", 10,
...
void bt_sink_srv_music_role_handover(const bt_bd_addr_t *addr, bt_aws_mce_role_handover_profile_t *profile_data, uint8_t *data, bool is_active)
{
    ...
    if (profile_data) {
        ...
        (void)addr_point;
        bt_sink_srv_report_id("[music_rho]addr:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x, flag:0x%x", 7,
        ...
        sp_dev = bt_sink_srv_music_get_device(BT_SINK_SRV_MUSIC_DEVICE_SP,
                                              NULL);
        while (sp_dev) {
            bt_sink_srv_report_id("[music_rho] to free mce dev, aws_a2dp_dev:0x%08x, aws_hd:0x%08x, flag:0x%x",
...
static bt_status_t bt_sink_srv_music_role_handover_service_allowed(const bt_bd_addr_t *addr)
{
    ...
    if (result) {
        ...
        if (avrcp_timer || bt_sink_srv_avrcp_is_push_release_command_timer_active()) {
