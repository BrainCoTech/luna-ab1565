    ...
    if (AUDIO_SRC_SRV_PSEUDO_DEVICE_A2DP == dev->handle->type) {
        ...
        if (BT_CODEC_MEDIA_STATUS_OK == ret) {
            ...
            if (AUDIO_SRC_SRV_PSEUDO_DEVICE_A2DP == dev->handle->type) {
                ...
                if (!(ctx->a2dp_eir.flag & BT_SINK_SRV_AWS_MCE_A2DP_REINIT_FLAG) || (dev->op & BT_SINK_SRV_MUSIC_NO_STREAMING_STATE_UPDATE_FLAG)
...
void bt_sink_srv_music_clear_codec(bt_sink_srv_music_device_t *dev, uint8_t aid)
{
    ...
    if (dev->op & BT_SINK_SRV_MUSIC_OP_CODEC_OPEN) {
        ...
        vTaskDelay(200 / portTICK_PERIOD_MS); //fix BTA-40938
        #endif
    ...
    bt_sink_srv_report_id("[sink][a2dp]clear_codec--ret: %d", 1, ret);
}
                                         ...
                                         bt_a2dp_codec_capability_t *a2dp_cap, bt_a2dp_role_t role, uint16_t a2dp_mtu)
{
    ...
    bt_sink_srv_report_id("fill_am_aud_param vol is %d", 1, aud_cap->audio_stream_out.audio_volume);
}
...
#ifdef AIR_BT_A2DP_VENDOR_2_ENABLE
    else if (src_codec->codec.vendor.codec_id == BT_A2DP_CODEC_VENDOR_2_CODEC_ID)
    {
        ...
        dst_codec->codec.vendor.resolution = (src_codec->codec.vendor.value[1] & BT_A2DP_VENDOR_2_CODEC_RESOLUTION_MASK);
    }
#endif
    ...
    *enable = 0;
    ...
    if(ctx != NULL &&  ctx->run_dev != NULL){
        ...
        if ((ctx->run_dev->codec.type == BT_A2DP_CODEC_VENDOR) 
            && (ctx->run_dev->codec.codec.vendor.codec_id == BT_A2DP_CODEC_VENDOR_2_CODEC_ID)){
...
uint64_t bt_sink_srv_music_convert_btaddr_to_devid(bt_bd_addr_t *bd_addr)
{
    ...
    bt_sink_srv_report_id("[music]convert_btaddr_to_devid--dev_id: 0x%x", 2, dev_id);
    return dev_id;
}
...
void bt_sink_srv_music_cancel_audio_play_en(bt_sink_srv_music_device_t *a2dp_dev)
{
    ...
    bt_sink_srv_report_id("[music] cancel_audio_play_en--gap_hd=0x%x, timer_ret:0x%x",2, gap_hd, timer_ret);
}
...
bt_status_t bt_sink_srv_music_get_nvdm_data(bt_bd_addr_t *remote_addr, bt_sink_srv_music_data_type_t data_type, void *data)
{
    ...
    bt_sink_srv_report_id("[sink][music]get_nvdm_data, data_type:%d, bt_addr:0x%08x, ret:%d, role:%d, data:%d",
                          5, data_type, (uint32_t)*remote_addr, ret, role, *((uint8_t *)data));
    return ret;
}
...
bt_status_t bt_sink_srv_music_set_nvdm_data(bt_bd_addr_t *remote_addr, bt_sink_srv_music_data_type_t data_type, void *data)
{
    ...
    bt_sink_srv_music_set_a2dp_nvdm_data(remote_addr, &dev_db, sizeof(dev_db));
    bt_sink_srv_report_id("[sink][music]set_nvdm_data, data_type:%d, bt_addr:0x%08x, ret:%d, get_data_ret:%d",
        5, data_type, (uint32_t)(*remote_addr), ret, get_data_ret, *((uint8_t *)data));
    return ret;
}
...
bt_sink_srv_music_device_list_t *bt_sink_srv_music_get_played_device_list(bool is_connected)
{
    ...
    if (played_dev_count) {
        ...
        for (i = 0; i < played_dev_count; i++) {
            bt_sink_srv_report_id("get_played_device_list--played_dev_count:%d, addr[%d]--0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x, dev_count:%d",
...
void bt_sink_srv_music_handle_long_time_drift(audio_dsp_a2dp_ltcs_report_param_t *report_param)
{
    ...
    bt_sink_srv_report_id("[sink][a2dp]handle_long_time_drift--drift_value:%d,asi:0x%08x,nclk:0x%08x,nclk_intra:0x%04x,asi_cur:0x%08x,asi_count:%d", 6,
...
bt_sink_srv_music_device_t *bt_sink_srv_music_get_other_streaming_devices(bt_sink_srv_music_device_t *current_streaming_dev)
{
    ...
    bt_sink_srv_report_id("[Sink][music] streaming device num=%d, streaming_dev:0x%08x",
        2, count, streaming_dev);
...
bool bt_sink_srv_music_is_must_play_dev_by_addr(bt_bd_addr_t *sp_addr)
{
    ...
    if (dev && (dev->avrcp_flag & BT_SINK_SRV_AVRCP_MUST_PLAY_RING_TONE_FLAG)) {
        return true;
    }
...
bt_status_t bt_sink_srv_music_stop_local_music(void)
{
    ...
    bt_sink_srv_report_id("[music]stop_local_music, run_dev:0x%08x", 1, run_dev);
    bt_sink_srv_mutex_unlock();
...
bt_status_t bt_sink_srv_music_resume_local_music(void)
{
    ...
    bt_sink_srv_report_id("[music]resume_local_music, run_dev:0x%08x, op:0x%08x", 2, resume_dev, op_flag);
    bt_sink_srv_mutex_unlock();
...
void bt_sink_srv_music_set_interrupt_flag(uint32_t flagMask, bool val)
{
    ...
    if (BT_SINK_SRV_CNTX_FLAG_A2DP_INTER_ULL_BY_AVRCP != flagMask) {
        bt_utils_assert(0 && "unexpect music interrupt flag");
    }
    if (val) {
    ...
    bt_sink_srv_report_id("[Sink][music]set_interrupt_flag: context_flag:0x%x, 0x%x, val:0x%08x",
