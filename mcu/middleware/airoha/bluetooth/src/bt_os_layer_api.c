...
static void bt_os_layer_aes_execute_internal(bt_os_layer_aes_buffer_t *out_data, bt_os_layer_aes_buffer_t *plain_text, bt_os_layer_aes_buffer_t *key, bool encrypt)
{
    ...
    memcpy(bt_hal_plain_text.buffer, plain_text->buffer, sizeof(bt_hal_plain));
    bt_hal_plain_text.length = plain_text->length;
    memcpy(bt_hal_key_text.buffer, key->buffer, sizeof(bt_hal_key));
    bt_hal_key_text.length = key->length;
    bt_hal_coded_text.length =  sizeof(bt_hal_coded);

    LOG_MSGID_I(BT_OS, "bt_os_layer_aes_execute before plain data 0x%x, key 0x%x, coded 0x%x, encrypt ? %d", 4,
        *(uint32_t *)bt_hal_plain_text.buffer, *(uint32_t *)bt_hal_key_text.buffer, *(uint32_t *)bt_hal_coded_text.buffer, encrypt);
    ...
    do {
        ...
        vTaskDelay(1);
    } while (HAL_AES_STATUS_OK != aes_status && retry_times--);
    LOG_MSGID_I(BT_OS, "bt_os_layer_aes_execute use hal version status %d expect len %d, real decrypt len %d", 3,
        aes_status, out_data->length, bt_hal_coded_text.length, encrypt);
    LOG_MSGID_I(BT_OS, "bt_os_layer_aes_execute after plain data 0x%x, key 0x%x, coded 0x%x", 3,
        *(uint32_t *)bt_hal_plain_text.buffer, *(uint32_t *)bt_hal_key_text.buffer, *(uint32_t *)bt_hal_coded_text.buffer);
    if (bt_hal_coded_text.length < out_data->length) {
        memcpy(out_data->buffer, bt_hal_coded_text.buffer, bt_hal_coded_text.length);
        out_data->length = bt_hal_coded_text.length;
    } else {
        memcpy(out_data->buffer, bt_hal_coded_text.buffer, out_data->length);
    }
#else
    ...
    mbedtls_aes_setkey_enc(&context, key->buffer, sizeof(bt_key_t) * 8);
        mbedtls_aes_crypt_ecb(&context, MBEDTLS_AES_ENCRYPT, (unsigned char *)plain_text->buffer, (unsigned char *)out_data->buffer);
    } else {
        mbedtls_aes_setkey_dec(&context, key->buffer, sizeof(bt_key_t) * 8);
        mbedtls_aes_crypt_ecb(&context, MBEDTLS_AES_DECRYPT, (unsigned char *)plain_text->buffer, (unsigned char *)out_data->buffer);
    }
    mbedtls_aes_free(&context);
    LOG_MSGID_I(BT_OS, "bt_os_layer_aes_execute use mbed version, encrypt ? %d", 1, encrypt);
#endif
}
...
{
    bt_os_layer_aes_execute_internal(encrypted_data, plain_text, key, true);
}
...
{
    bt_os_layer_aes_execute_internal(decrypted_data, plain_text, key, false);
}
...
void *bt_os_layer_mem_alloc(uint32_t size)
{
    void *pvReturn = NULL;
    uint32_t  free_size = xPortGetFreeHeapSize();
    if (free_size > size) {
...
void bt_os_layer_mem_free(void *buf)
{
    if (buf) {
        vPortFree(buf);
    }
