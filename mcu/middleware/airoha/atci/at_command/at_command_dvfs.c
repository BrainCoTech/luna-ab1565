            ...
            if (param != NULL) {
                ...
                if (op == 0) {
                    ...
                    for (idx = 0; idx < list_num; idx++) {
                        if (idx == list_num - 1) {
                            snprintf((char *)&response.response_buf[op], ATCI_UART_TX_FIFO_BUFFER_SIZE-op, "%d\r\n", cpufreq_list[idx]);
                        } else {
                            snprintf((char *)&response.response_buf[op], ATCI_UART_TX_FIFO_BUFFER_SIZE-op, "%d,", cpufreq_list[idx]);
                            op = strlen((char *)response.response_buf);
                        }
                    }
