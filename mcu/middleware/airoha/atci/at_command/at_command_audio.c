...
//                SltTaskSuspendReq();
            ...
            /* SMT item for airoha automotive product */
                ...
                extern void ble_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, hal_gpio_pin_t gpio_pin);
                sscanf((char *)parse_cmd->string_ptr,"%*[^,],%lu", &gpio_pin);
                ble_audio_dongle_rx_latency_debug_control(0, true, gpio_pin);
                presponse->response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                presponse->response_len = strlen((const char *)presponse->response_buf);
                atci_send_response(presponse);
                ...
                extern void ble_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, hal_gpio_pin_t gpio_pin);
                sscanf((char *)parse_cmd->string_ptr,"%*[^,],%lu", &gpio_pin);
                ble_audio_dongle_rx_latency_debug_control(0, false, gpio_pin);
                presponse->response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                presponse->response_len = strlen((const char *)presponse->response_buf);
                atci_send_response(presponse);
                ...
                extern void ble_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, hal_gpio_pin_t gpio_pin);
                sscanf((char *)parse_cmd->string_ptr,"%*[^,],%lu", &gpio_pin);
                ble_audio_dongle_rx_latency_debug_control(1, true, gpio_pin);
                presponse->response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                presponse->response_len = strlen((const char *)presponse->response_buf);
                atci_send_response(presponse);
                ...
                extern void ble_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, hal_gpio_pin_t gpio_pin);
                sscanf((char *)parse_cmd->string_ptr,"%*[^,],%lu", &gpio_pin);
                ble_audio_dongle_rx_latency_debug_control(1, false, gpio_pin);
                presponse->response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                presponse->response_len = strlen((const char *)presponse->response_buf);
                atci_send_response(presponse);
                ...
                extern void usb_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, uint32_t gpio_num);
                extern void usb_audio_dongle_tx_latency_debug_control(uint32_t port, bool enable, uint32_t gpio_num);
                unsigned int is_ul    = 0;
                unsigned int usb_port = 0;
                unsigned int control  = 0;
                unsigned int gpio     = 0;
                atci_response_flag_t response = ATCI_RESPONSE_FLAG_APPEND_OK;
                sscanf((char *)parse_cmd->string_ptr,"%*[^,],%u,%u,%u,%u",
                    &is_ul,
                    &usb_port,
                    &control,
                    &gpio
                    );
                LOGMSGIDI("AUDIO_DONGLE_LATENCY_DEBUG, is ul %d, usb port %d, on/off %d gpio %d", 4,
                    is_ul,
                    usb_port,
                    control,
                    gpio
                ...
                if (is_ul) {
                    if (usb_port == 0) {
                        usb_audio_dongle_tx_latency_debug_control(usb_port, control, gpio);
                    } else {
                        response = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    }
                } else {
                    if (usb_port <= 1) {
                        usb_audio_dongle_rx_latency_debug_control(usb_port, control, gpio);
                    } else {
                        response = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    }
                }
