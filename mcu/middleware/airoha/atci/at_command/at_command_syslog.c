...
static void syslog_show_usage(uint8_t *buf)
{
    ...
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
                    "(AT+SYSLOG?, query current setting)\r\n");
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
                    "(AT+SYSLOG=0,<cpu>,<switch>,<level>, config cpu's setting)\r\n");
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
                    "(AT+SYSLOG=1,<cpu>,<module>,<switch>,<level>, config module's setting)\r\n");
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
                    "(AT+SYSLOG=2, save total setting to NVDM)\r\n");
    ...
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
                    "(AT+SYSLOG=5,<switch>, batch configuration module switch)\r\n");
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
                    "  (<log_switch> = 0|1(on|off), <print_level>=0|1|2|3(D/I/W/E))\r\n");
}
...
#define SYSLOG_CFG_TX_FIFO_SIZE     (1000)
...
atci_status_t atci_cmd_hdlr_syslog(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION:
            /* AT+SYSLOG=0,<cpu>,<log_switch>,<print_level> */
            if (strstr(parse_cmd->string_ptr, "AT+SYSLOG=0,") != NULL) {
                var_size = atci_get_parameter_list(parse_cmd, &var_list[0], 4);
                if (var_size == 4) {
                    ret = log_set_cpu_filter_config((uint32_t)(var_list[1][0]-'0'),
                                                    (log_switch_t)(var_list[2][0]-'0'),
                                                    (print_level_t)(var_list[3][0]-'0'));
                }
                if (ret == false) {
                    response.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                } else {
                ...
                /* AT+SYSLOG=1,<cpu>,<module>,<log_switch>,<print_level> */
                var_size = atci_get_parameter_list(parse_cmd, &var_list[0], 5);
                if (var_size == 5) {
                    ret = log_set_module_filter_config((uint32_t)(var_list[1][0]-'0'),
                                                        (char *)var_list[2],
                                                        (log_switch_t)(var_list[3][0]-'0'),
                                                        (print_level_t)(var_list[4][0]-'0'));
                }
                if (ret == false) {
                    response.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                } else {
                ...
                response.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            } else if (strstr(parse_cmd->string_ptr, "AT+SYSLOG=5,") != NULL) {
                /* AT+SYSLOG=5,<switch> */
                var_size = atci_get_parameter_list(parse_cmd, &var_list[0], 2);
                if (var_size == 2) {
                    ret = log_set_all_module_filter_off((log_switch_t)(var_list[1][0]-'0'));
                }
                if (ret == false) {
                    response.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                } else {
