...
static atci_status_t atci_init_uart(mux_port_t port);
#if defined (AIR_1WIRE_ENABLE)
static atci_status_t atci_init_1wire(mux_port_t port);
#endif
static atci_status_t atci_init_usb(mux_port_t port);
...
static atci_status_t atci_init_uart(mux_port_t port)
{
    ...
    atci_config_port(&port_config, port, RACE_PORT_TYPE_NORMAL_UART);
    ...
    atci_config_user(&user_config, port, RACE_PORT_TYPE_NORMAL_UART);
...
static atci_status_t atci_init_1wire(mux_port_t port)
{
    ...
    atci_config_port(&port_config, port, RACE_PORT_TYPE_1WIRE_UART);
    ...
    memset(&user_config, 0, sizeof(race_user_config_t));
    atci_config_user(&user_config, port, RACE_PORT_TYPE_1WIRE_UART);
...
static atci_status_t atci_init_usb(mux_port_t port)
{
    ...
    memset(&port_config, 0, sizeof(race_port_init_t));
    port_config.port_settings = &atci_setting;
    atci_config_port(&port_config, port, RACE_PORT_TYPE_USB);
    ...
    memset(&user_config, 0, sizeof(race_user_config_t));
    atci_config_user(&user_config, port, RACE_PORT_TYPE_USB);
...
atci_status_t atci_init_port(mux_port_t port, race_port_type_t type)
{
    ...
    if (RACE_PORT_TYPE_NORMAL_UART == type) {
        res = atci_init_uart(port);
#if defined (AIR_1WIRE_ENABLE)
    } else if (RACE_PORT_TYPE_1WIRE_UART == type) {
        res = atci_init_1wire(port);
#endif
    } else if (RACE_PORT_TYPE_USB) {
        res = atci_init_usb(port);
    } else {
        assert("ERROR: atci_init_port, unsupported port !" && 0);
    }
