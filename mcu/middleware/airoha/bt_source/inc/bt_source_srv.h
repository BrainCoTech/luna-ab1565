...
#define BT_SOURCE_SRV_ACTION_SWITCH_AUDIO_PATH              (BT_SOURCE_MODULE_CALL_ACTION | 0x008U)         /**< This action request to witch audio path, with the payload is NULL 0xF8520008 */
...
#define BT_SOURCE_SRV_EVENT_MUSIC_STREAM_OPERAION_IND            (BT_SOURCE_MODULE_MUSIC_EVENT | 0x003U)        /**< The event is triggered after received start/suspend streaming req from remote device*/
...
typedef struct {
    ...
    bt_status_t              status;
    bt_srv_music_operation_t operation;
...
/**
* @brief The struct for #BT_SOURCE_SRV_AVRCP_CHNAGE_VOLUME_CNF.
*/
typedef struct {
    bt_addr_t    peer_address;
    bt_status_t  status;
}bt_source_srv_music_change_volume_cnf_t;
