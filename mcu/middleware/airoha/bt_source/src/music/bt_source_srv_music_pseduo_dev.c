...
static void bt_source_srv_music_psd_reset(bt_source_srv_music_pseduo_dev_t *device)
{
    ...
    device->audio_id = AUD_ID_INVALID;
...
void bt_source_srv_music_psd_set_speaker_volume(void *device, bt_source_srv_music_audio_volume_t volume)
{
    ...
    LOG_MSGID_I(source_srv, "[MUSIC][PSD] set device = %02x speaker volume = %02x", 2, device, volume);
    if (music_device->codec_type != BT_HFP_CODEC_TYPE_NONE) {
        bt_sink_srv_ami_audio_set_volume(music_device->audio_id, music_device->speaker_volume, STREAM_OUT);
    }
}
...
bt_status_t bt_source_srv_music_audio_port_update(bt_source_srv_port_t audio_port)
{
    bt_status_t status = BT_STATUS_FAIL;
    if (!bt_source_srv_common_audio_port_is_valid(audio_port)) {
        return status;
    }
    status = bt_source_srv_a2dp_audio_port_update(audio_port);
    return status;
}
...
bool bt_source_srv_music_psd_is_playing(void *device)
{
    ...
    if ((music_device->state == BT_SOURCE_SRV_MUSIC_PSD_STATE_PLAY) &&
            ((music_device->sub_state == BT_SOURCE_SRV_MUSIC_PSD_SUB_STATE_PLAY_IDLE) ||
             (music_device->sub_state == BT_SOURCE_SRV_MUSIC_PSD_SUB_STATE_PLAYING))) {
        return true;
    }
    return false;
}
