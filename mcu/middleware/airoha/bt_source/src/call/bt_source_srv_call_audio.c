...
#define BT_SOURCE_SRV_CALL_AUDIO_BT_FRAME_INTERVAL                 15000U   /* UL 7.5ms + DL 7.5ms */
...
bt_source_srv_call_audio_id_t bt_source_srv_call_audio_config_init(bt_source_srv_call_audio_config_t *config, bt_source_srv_call_audio_codec_type_t codec_type, void *user_data, void *user_callback)
{
    ...
    switch (config->type) {
        case BT_SOURCE_SRV_CALL_AUDIO_PLAY_TYPE_DL: {
            ...
            if (!bt_source_srv_common_audio_port_is_valid(port_context.port)) {
                return BT_SOURCE_SRV_CALL_AUDIO_INVALID_ID;
            }
        ...
        case BT_SOURCE_SRV_CALL_AUDIO_PLAY_TYPE_I2S_IN: {
            ...
            /* PC->audio:source config */
            dl_config->source.afe_in.channel_num = port_context.sample_channel;
            dl_config->source.afe_in.sample_rate = port_context.sample_rate;
            dl_config->source.afe_in.sample_format = g_audio_dsp_format_mapping[port_context.sample_size];
            dl_config->source.afe_in.channel_select = HAL_AUDIO_DIRECT;
            LOG_MSGID_I(source_srv, "[AG][AUDIO] audio line in/I2S in config pcm sample_rate = %02x channel_mode = %02x, format = %02x", 4,
                        config->type, port_context.sample_rate,  port_context.sample_channel, g_audio_dsp_format_mapping[port_context.sample_size]);
