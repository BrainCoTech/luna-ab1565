        ...
        bt_source_srv_call_audio_volume_t gain_level)
{
    ...
    if (bt_source_srv_call_read_nvdm(bd_address, &stored_data) != BT_STATUS_SUCCESS) {
        LOG_MSGID_E(source_srv, "[HFP][AG][CALL] device = %02x gain type = %02x update fail", 2, device, gain_type);
        return status;
    }
    ...
    LOG_MSGID_I(source_srv, "[HFP][AG][CALL] device = %02x gain type = %02x update level = %02x", 3, device, gain_type, gain_level);
...
bt_status_t bt_source_srv_call_audio_gain_init(bt_bd_addr_t *bd_address, void *device)
{
    ...
    bt_source_srv_call_stored_data_t stored_data = {0};
    if (bt_source_srv_call_read_nvdm(bd_address, &stored_data) == BT_STATUS_SUCCESS) {
        LOG_MSGID_I(source_srv, "[HFP][AG][CALL] read nvdm local speker volume = %02x, speaker volume = %02x, mic gain = %02x", 3,
                    stored_data.local_speaker_volume, stored_data.speaker_volume, stored_data.mic_gain);
    } else {
        LOG_MSGID_W(source_srv, "[HFP][AG][CALL] read nvdm speaker volume and mic gain fail", 0);
...
uint32_t bt_source_srv_call_get_audio_gain_level(bt_source_srv_call_gain_t gain_type, bt_bd_addr_t *bd_address)
{
    ...
    bt_source_srv_call_stored_data_t stored_data = {0};
    if (bt_source_srv_call_read_nvdm(bd_address, &stored_data) != BT_STATUS_SUCCESS) {
        LOG_MSGID_E(source_srv, "[HFP][AG][CALL] get gain type = %02x update fail", 1, gain_type);
        return gain_level;
    }
    ...
    LOG_MSGID_I(source_srv, "[HFP][AG][CALL] get gain type = %02x level = %02x", 2, gain_type, gain_level);
    return gain_level;
}

uint32_t bt_source_srv_call_convert_volume(bt_source_srv_call_vcf_t vcf_type, uint32_t volume_value, bool is_forward)
{
    if (is_forward) {
