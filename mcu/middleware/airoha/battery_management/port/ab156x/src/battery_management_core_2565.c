    ...
    switch (property)
    {
        ...
        case BATTERY_PROPERTY_TEMPERATURE:
            if (battery_sw_ntc_feature == BATTERY_OPERATE_ON) {
                ...
                property_value = 25;
                bm_dbg_print_I(battery_management, "SW NTC not enable\r\n", 0);
            }
            break;
...
void battery_charger_setting(TimerHandle_t pxTimer) {
    ...
    if(battery_sw_ntc_feature == BATTERY_OPERATE_ON){
            ...
            break;
        default:
            bm_dbg_print_I(battery_management, "AIR_BTA_PMIC_G2_LP charger step fail\n", 0);
            break;
    }
}
...
void battery_charger_task(void * pvParameters) {
    ...
    while (1) {
        ...
        LOG_W(MPLOG,"[BM_TASK]Bat temp = %d(Celsius), capacity = %d(%%), chg status = %d, bat volt = %d(mV), bat state = %d\n"
...
void battery_timer_init(void) {
    ...
    if(battery_sw_ntc_feature == BATTERY_OPERATE_ON){
        ...
        if (xTimerStart(xbm_jeita_timer, 0) != pdPASS) {
            bm_dbg_print_I(battery_management, "xbm_jeita_Timer xTimerStart fail\n", 0);
        }
        bm_ctrl_info.jeita_state = sw_ntc_charger_normal;
    }else{
        bm_ctrl_info.jeita_state = sw_ntc_charger_normal;
    bm_dbg_print_I(battery_management, "battery_sw_ntc_feature fail %d \n", 1,battery_sw_ntc_feature);
    }
#ifdef CHARGER_CALIBRATION
