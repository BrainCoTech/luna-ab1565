    ...
    if (handle != BT_HANDLE_INVALID) {
        ...
        if (rw == BT_GATTS_CALLBACK_WRITE) {
            ...
            if (size != len) {
                ...
                LOG_MSGID_I(common, "voice_write(%x)", 1, record);
                if (record == NULL) {
                    /* Flow have problem. */
                    return 0;
...
static uint32_t bt_aws_mce_le_association_custom_data_write_callback(const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset)
{
    ...
    if (handle != BT_HANDLE_INVALID) {
        ...
        if (rw == BT_GATTS_CALLBACK_WRITE) {
            ...
            ind.info.len = size;
            LOG_MSGID_I(common, "custom_data_write_callback: size(%x), data[%x, %x, %x, %x]", 5, size, ind.info.data[0], ind.info.data[1], ind.info.data[2], ind.info.data[3]);
