...
static int16_t g_wired_audio_usb_tx_threshlod_size;
...
void audio_transmitter_wired_audio_open_playback(audio_transmitter_config_t *config, mcu2dsp_open_param_t *open_param)
{
    ...
    if (0) {
        ...
        WIRED_AUDIO_LOG_MSGID_I("[USB_OUT] init: type %d, sub_id %d, is_with_ecnr %d, codec_type %d, sample_rate = %d, channel_mode %d, format %d, 0x%08x, 0x%08x, %d, %d, %d, %d, %d, %d, %d, %d", 17,
        ...
        open_param->stream_in_param.virtual.virtual_mem_size = (WIRED_AUDIO_DL_PROCESS_PERIOD * WIRED_AUDIO_UL_MAX_SAMPLE_RATE) / 1000 * 4;
    }
...
void audio_transmitter_wired_audio_main_stream_open_start_playback(void)
{
    ...
    open_param->stream_in_param.virtual.virtual_mem_size = (dl_process_period * WIRED_AUDIO_UL_MAX_SAMPLE_RATE) / 1000 * 4;
