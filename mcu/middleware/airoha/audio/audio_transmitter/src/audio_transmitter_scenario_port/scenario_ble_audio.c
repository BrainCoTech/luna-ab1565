...
static void usb_audio_rx_cb_latency_debug(uint32_t port, uint8_t *p_source_buf)
{
    ...
    if (usb_stream_rx_handle[port].latency_debug_enable) {
        ...
        if ((*(start_address + 2 * 0) > usb_stream_rx_handle[port].latency_debug_last_sample) &&
            (*(start_address + 2 * 0) - usb_stream_rx_handle[port].latency_debug_last_sample > 40000)) {
            current_level = 1;
        } else if ((*(start_address + 2 * 0) < usb_stream_rx_handle[port].latency_debug_last_sample) &&
                    (usb_stream_rx_handle[port].latency_debug_last_sample - * (start_address + 2 * 0) > 40000)) {
            current_level = 0;
        }
        for (i = 0; i < usb_stream_rx_handle[port].frame_size / 4 - 1; i++) {
        ...
        usb_stream_rx_handle[port].latency_debug_last_sample = *(start_address + 2 * (usb_stream_rx_handle[port].frame_size / 4 - 1));
...
static void usb_audio_rx_cb_ble_audio_dongle_0(void)
{
    ...
    if (usb_stream_rx_handle[0].usb_first_in_flag == 0) {
        ...
        if (available_data_size > usb_stream_rx_handle[0].frame_size) {
            TRANSMITTER_LOG_E("[BLE Audio Dongle][USB_RX_DEBUG 0]Too much data in USB buffer %d\r\n", 1, available_data_size);
        }
...
/* Public functions ----------------------------------------------------------*/
#if BLE_AUDIO_DONGLE_DEBUG_LANTENCY
ATTR_TEXT_IN_RAM_FOR_MASK_IRQ void ble_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, hal_gpio_pin_t gpio_pin)
{
    uint32_t saved_mask;
