        ...
        case AUDIO_DSP_CODEC_TYPE_SBC:
            #ifdef AIR_BT_AUDIO_DONGLE_ENABLE
            {
                ...
                if ((codec_param->sbc_enc.channel_mode == SBC_ENCODER_MONO) || (codec_param->sbc_enc.channel_mode == SBC_ENCODER_DUAL_CHANNEL)) {
                    frame_size += blocks * channel * codec_param->sbc_enc.bit_pool / 8;
                    if (blocks * channel * codec_param->sbc_enc.bit_pool % 8) {
                        frame_size += 1;
                    }
                } else if (codec_param->sbc_enc.channel_mode == SBC_ENCODER_JOINT_STEREO) {
                    frame_size += (sub_band + blocks * codec_param->sbc_enc.bit_pool) / 8;
                    if ((sub_band + blocks * codec_param->sbc_enc.bit_pool) % 8) {
                        frame_size += 1;
                    }
                } else if (codec_param->sbc_enc.channel_mode == SBC_ENCODER_STEREO) {
                    frame_size += (blocks * codec_param->sbc_enc.bit_pool) / 8;
                    if ((blocks * codec_param->sbc_enc.bit_pool) % 8) {
                        frame_size += 1;
                    }
                } else {
...
ATTR_TEXT_IN_RAM_FOR_MASK_IRQ void usb_audio_dongle_rx_latency_debug_control(uint32_t port, bool enable, uint32_t gpio_num)
{
    uint32_t saved_mask;
    TRANSMITTER_LOG_I("[Dongle Common][Rx_Cb] latency debug %d %d %d", 3, port, enable, gpio_num);
...
ATTR_TEXT_IN_RAM_FOR_MASK_IRQ void usb_audio_dongle_tx_latency_debug_control(uint32_t port, bool enable, uint32_t gpio_num)
{
    uint32_t saved_mask;
    TRANSMITTER_LOG_I("[Dongle Common][tx_Cb] latency debug %d %d %d", 3, port, enable, gpio_num);
    hal_nvic_save_and_set_interrupt_mask(&saved_mask);
...
static void audio_dongle_usb0_rx_cb_handle(uint8_t port)
{
    ...
    if (audio_dongle_usb_rx_handle[port].first_time == 0) {
        ...
        if (available_data_size > audio_dongle_usb_rx_handle[port].frame_size) {
            TRANSMITTER_LOG_E("[Dongle Common][ERROR][Rx_Cb]Too much data in USB %d buffer %d\r\n", 2, port, available_data_size);
        }

        /* copy usb data into share buffer block one by one */
        while (available_data_size >= audio_dongle_usb_rx_handle[port].frame_size) {
...
#if defined AIR_BLE_AUDIO_DONGLE_LINE_IN_ENABLE || defined AIR_GAMING_MODE_DONGLE_V2_LINE_IN_ENABLE || defined(AIR_BT_AUDIO_DONGLE_LINE_IN_ENABLE)
...
// parameter setting
static void bt_audio_dongle_config_i2s_parameter(uint32_t id, audio_transmitter_config_t *config, mcu2dsp_open_param_t *open_param)
{
    (void)(id);
    (void)(config);
    ...
    open_param->stream_in_param.afe.frame_size = sink_info->bt_out.frame_samples * audio_dongle_get_format_bytes(open_param->stream_in_param.afe.format);
...
static void bt_audio_dongle_config_analog_parameter(uint32_t id, audio_transmitter_config_t *config, mcu2dsp_open_param_t *open_param)
{
    (void)(id);
    (void)(config);
    bt_audio_dongle_dl_sink_info_t *sink_info = &(open_param->stream_out_param.bt_common.scenario_param.bt_audio_dongle_param.dl_info.sink);
...
static void audio_dongle_config_afe_in_parameter(uint32_t id, audio_transmitter_config_t *config, mcu2dsp_open_param_t *open_param)
{
    ...
    // device setting
    switch (open_param->stream_in_param.afe.audio_device) {
        case HAL_AUDIO_DEVICE_I2S_SLAVE:
        case HAL_AUDIO_DEVICE_I2S_MASTER:
            #if defined(AIR_BT_AUDIO_DONGLE_I2S_IN_ENABLE) || defined(AIR_BT_AUDIO_DONGLE_LINE_IN_ENABLE)
                if ((id >> 16) == AUDIO_TRANSMITTER_BT_AUDIO_DONGLE) {
                    bt_audio_dongle_config_i2s_parameter(id, config, open_param);
                }
            #endif
            break;
        ...
        case HAL_AUDIO_DEVICE_DIGITAL_MIC_DUAL:
            #if defined(AIR_BT_AUDIO_DONGLE_I2S_IN_ENABLE) || defined(AIR_BT_AUDIO_DONGLE_LINE_IN_ENABLE)
                if ((id >> 16) == AUDIO_TRANSMITTER_BT_AUDIO_DONGLE) {
                    bt_audio_dongle_config_analog_parameter(id, config, open_param);
                }
            #endif
            break;
        default:
