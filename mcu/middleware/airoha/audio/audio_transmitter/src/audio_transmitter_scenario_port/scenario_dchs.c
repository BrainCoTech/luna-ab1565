...
static dchs_dl_status_t dchs_dl_status = {false, AUDIO_SCENARIO_TYPE_COMMON,AUDIO_SCENARIO_TYPE_COMMON, 0, 0, false};
                    ...
                    if (cur_uart_cmd_status.cmd_status == WAITING_READ){
                        ...
                        if(uart_data_size >= cur_uart_cmd_status.param_size){
                            ...
                            if(cur_uart_cmd_status.ctrl_type == AUDIO_UART_COSYS_DL_OPEN){
                                ...
                                //mcu ccni dsp sync scenario status immediately, for BTA-40042 issue
                                dual_chip_dl_sync_msg_t dl_mcu2dsp_sync_msg;
                                dl_mcu2dsp_sync_msg.data_scenario_type = dl_close_ctrl_param.scenario_type;
                                dl_mcu2dsp_sync_msg.sample_rate = 0;
                                dl_mcu2dsp_sync_msg.format_bytes = 0;
                                dl_mcu2dsp_sync_msg.chip_role = OTHER_CHIP;
                                dl_mcu2dsp_sync_msg.other_chip_dl_exist = false;
                                dchs_cosys_sync.sync_type = DCHS_DL_OTHER_CHIP_DL_EXIST;
                                memcpy(&dchs_cosys_sync.sync_msg.dual_chip_dl_sync , &dl_mcu2dsp_sync_msg, sizeof(dual_chip_dl_sync_msg_t));
                                if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
                                    hal_audio_dsp_controller_send_message(MSG_MCU2DSP_DCHS_COSYS_SYNC, 0, (uint32_t)&dchs_cosys_sync, false);
                                }else if(dchs_get_device_mode() == DCHS_MODE_LEFT && !dchs_dl_check_scenario_exist_flag(LOCAL_CHIP)){
                                    bool need_send_close_msg = false;
                                    if(dl_close_ctrl_param.scenario_type == AUDIO_SCENARIO_TYPE_VP){
                                        if(!BIT_GET(dchs_dl_other_chip_exist, SCENARIO_1_POST)){
                                            need_send_close_msg = true;
                                        }
                                    }else{
                                        if(!BIT_GET(dchs_dl_other_chip_exist, SCENARIO_2_POST)){
                                            need_send_close_msg = true;
                                        }
                                    }
                                    if(need_send_close_msg){
                                        hal_audio_dsp_controller_send_message(MSG_MCU2DSP_DCHS_COSYS_SYNC, 0, (uint32_t)&dchs_cosys_sync, false);
                                    } 
                                }
                                //--for BTA-40042 issue
