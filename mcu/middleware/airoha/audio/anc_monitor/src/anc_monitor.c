...
uint32_t g_ANC_monitor_get_ANC_type, g_ANC_monitor_get_ANC_type_pre = 0; //0 for ANC, 1 for PT, 2 for Sidetone
...
static void audio_anc_monitor_update_nvkey_id_by_anc_type(uint32_t anc_type)
{
    if (anc_type == 0) { //ANC
        #ifdef AIR_ANC_WIND_DETECTION_ENABLE
...
static void audio_anc_monitor_get_anc_status(uint8_t *enable, uint32_t *anc_type, uint32_t *misc1, uint32_t *misc2)
{
    uint8_t                            anc_enable;
    ...
    uint32_t                           local_anc_type = 0; //0 for ANC, 1 for PT, 2 for Sidetone
    ...
    if ((anc_enable != 0) && (anc_current_type & AUDIO_ANC_CONTROL_TYPE_PT_ADAP_FF)) {
        local_anc_type = 1;
    }
    #endif
...
void audio_anc_monitor_anc_callback(audio_anc_control_event_t event_id, audio_anc_control_result_t result)
{
    ...
    if ((g_ANC_monitor_get_ANC_type == 2) && (event_id == AUDIO_ANC_CONTROL_EVENT_OFF)) { //pure sidetone special regnization
