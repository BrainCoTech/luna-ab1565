...
static void usb_device_cfg_nvkey_read(void)
{
    ...
    nvkey_status = nvkey_read_data(NVID_USB_STRING_15, (uint8_t *)&nvstr, &nvkey_size);
    if (nvkey_status == NVKEY_STATUS_OK)
    {
        uint32_t tmp = nvstr.str[30]; tmp <<= 8;
        tmp |= nvstr.str[29]; tmp <<= 8;
        tmp |= nvstr.str[28]; tmp <<= 8;
        tmp |= nvstr.str[27];
        g_USB_DBG_filter = tmp;
        LOG_MSGID_I(USB_MAIN, "g_USB_DBG_filter 0x%08x", 1, g_USB_DBG_filter);
    }
