...
void USB_Ms_If_Create(void *ifname)
{
    ...
    if (gUsbDevice.use_iad == true) {
        g_UsbMS.iad_desc = USB_Get_IAD_Number();
        Usb_IAD_Dscr *iad_desc = g_UsbMS.iad_desc;

        iad_desc->bLength           = USB_IAD_LENGTH;
        iad_desc->bDescriptorType   = USB_INTERFACE_ASSOCIATION;
        iad_desc->bFirstInterface   = if_id;
        iad_desc->bInterfaceCount   = 1;
        iad_desc->bFunctionClass    = USB_MS_INTERFACE_CLASS_CODE;
        iad_desc->bFunctionSubClass = USB_MS_INTERFACE_SUBCLASS_CODE;
        iad_desc->bFunctionProtocol = USB_MS_INTERFACE_PROTOCOL_CODE;
