...
{
    LOG_MSGID_I(BT_HID_SRV, "[HID][SRV] report map value callback, rw = %02x, handle = %02x, size = %02x", 3,
                rw, handle, size);
    bt_hogp_report_descriptor_t *report_descritor = bt_hogp_get_report_descriptor();
    if (rw == BT_GATTS_CALLBACK_READ) {
        if (size == 0) {
            return report_descritor->length;
        } else if (size == report_descritor->length) {
            memcpy(data, report_descritor->descriptor, size);
            return size;
        }
    }
    return 0;
...
static uint32_t bt_hid_get_report_id_handle(uint16_t handle, void *data, uint16_t size, bt_hogp_report_index_t index)
{
    ...
    if (size == 0) {
        ...
        LOG_MSGID_I(BT_HID_SRV, "[HID][SRV] get report id = %02x", 1, report_group->report_id);
