...
#define BT_GATTS_SRV_DATABASE_LENGTH             2048U
...
extern void bt_cmac_reverse_and_encrypt(const bt_key_t key, uint32_t msg_size, const uint8_t *msg, bt_key_t mac, uint8_t mac_size);
static bt_status_t bt_gatts_service_calculate_database_hash(uint32_t connection_handle, uint8_t *hash)
{
    ...
    /* padding a multiple of 128bit */
    uint32_t remainder = database_length & (uint32_t)(sizeof(bt_key_t) - 1);
    if (remainder != 0) {
        database_length = database_length + sizeof(bt_key_t) - remainder;
    }
    LOG_MSGID_I(BT_GATTS, "[GATTS][SRV] hash database length = %02x by padding", 1, database_length);

    if (database_length > BT_GATTS_SRV_DATABASE_LENGTH) {
        bt_utils_assert(0 && "database length is over alloc buffer");
    }
    /* calculate hash */
    bt_cmac_reverse_and_encrypt(key, database_length, database_value, hash_value, sizeof(bt_key_t));
    bt_utils_memcpy(hash, &hash_value, sizeof(bt_key_t));
    LOG_MSGID_I(BT_GATTS, "[GATTS][SRV] hash value:", 0);
    for (uint32_t i = 0; i < 16; i++) {
        LOG_MSGID_I(BT_GATTS, "%02X", 1, hash[i]);
    }
    bt_utils_memory_free(database_value);
    return BT_STATUS_SUCCESS;
}
