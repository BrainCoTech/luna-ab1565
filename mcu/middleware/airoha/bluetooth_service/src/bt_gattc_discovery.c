...
static bt_status_t  bt_gattc_discovery_save_connection_info(bt_handle_t conn_handle);
...
static void bt_gattc_discovery_connected_handle(bt_status_t status, void *buff)
{
    ...
    bt_gattc_discovery_save_connection_info(conn_ind->connection_handle);
    return;
}
        ...
        if (info->length == sizeof(rho_conn_handles) && info->data) {
            ...
            for (i = 0; i < BT_LE_CONNECTION_NUM; i++) {
                if (rho_conn_handles[i] != BT_HANDLE_INVALID) {
                    ...
                    bt_gattc_discovery_save_connection_info(mew_handle);
                }
            }
        }else {
