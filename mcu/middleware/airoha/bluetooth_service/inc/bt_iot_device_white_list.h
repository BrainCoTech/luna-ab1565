...
typedef enum {
    ...
    BT_IOT_CALL_ESCO_DISCONNECT_WITH_CALL_ACTIVE_RECONNECT,     /* IOT issue: esco is disconnected when the headset is reconnected and call is active */
    BT_IOT_CALL_HEADSET_BLOCK_ESCO_CONNECTION_WITH_RECONNECT,   /* IOT issue: when the HFP reconnect and the call is exist, the headset will block the esco connection initiated by the dongle. */
