...
ifeq ($(MTK_AVM_DIRECT), y)
    ifeq ($(AIR_BT_CODEC_ENABLED),y)
        ...
        CFLAGS  += -I$(SOURCE_DIR)/prebuilt/middleware/airoha/bluetooth/$(IC_CONFIG)/inc
        ifeq ($(MTK_BT_A2DP_SOURCE_ENABLE), y)
            CFLAGS  += -I$(SOURCE_DIR)/middleware/airoha/audio/sbc_codec/inc
        endif
    endif
...
ifeq ($(AIR_ADVANCED_PASSTHROUGH_ENABLE),y)
  ifeq ($(IC_CONFIG),ab156x)
    ...
    include $(SOURCE_DIR)/prebuilt/middleware/airoha/audio/$(IC_CONFIG)/psap/module.mk
    endif
  endif
endif
...
ifneq ($(AIR_HEARTHROUGH_MAIN_ENABLE)_$(AIR_CUSTOMIZED_LLF_ENABLE),n_n)
  ...
  ifeq ($(PSAP_SOURCE_CODE), $(wildcard $(PSAP_SOURCE_CODE)))
    ...
    ifneq ($(AIR_HEARTHROUGH_PSAP_ENABLE)_$(AIR_HEARING_AID_ENABLE)_$(AIR_HEARTHROUGH_HA_ENABLE),n_n_n)
      AIR_PSAP_LIB = $(SOURCE_DIR)/prebuilt/$(MIDDLEWARE_PROPRIETARY)/audio_psap/$(IC_CONFIG)/module.mk
      ifneq ($(wildcard $(AIR_PSAP_LIB)),)
        include $(SOURCE_DIR)/prebuilt/middleware/airoha/audio_psap/$(IC_CONFIG)/module.mk
        CFLAGS += -I$(SOURCE_DIR)/middleware/airoha/audio/llf_common/inc
      endif
    endif
    ifeq ($(AIR_HEARTHROUGH_VIVID_PT_ENABLE),y)
      AIR_VIVID_PT_LIB = $(SOURCE_DIR)/prebuilt/$(MIDDLEWARE_PROPRIETARY)/audio_vivid_pt/$(IC_CONFIG)/module.mk
      ifneq ($(wildcard $(AIR_VIVID_PT_LIB)),)
        include $(SOURCE_DIR)/prebuilt/middleware/airoha/audio_vivid_pt/$(IC_CONFIG)/module.mk
        CFLAGS += -I$(SOURCE_DIR)/middleware/airoha/audio/llf_common/inc
      endif
    endif
	ifeq ($(AIR_CUSTOMIZED_LLF_ENABLE),y)
	  AIR_LLF_LIB = $(SOURCE_DIR)/prebuilt/$(MIDDLEWARE_PROPRIETARY)/low_latency_framework/$(IC_CONFIG)/module.mk
	  ifneq ($(wildcard $(AIR_LLF_LIB)),)
	    include $(SOURCE_DIR)/prebuilt/middleware/airoha/low_latency_framework/$(IC_CONFIG)/module.mk
	  endif
	endif
  endif

ifeq ($(AIR_CUSTOMIZED_LLF_ENABLE),y)
  C_FILES += $(AUDIO_SRC)/llf_common/src/llf_example.c
  CFLAGS += -I$(SOURCE_DIR)/middleware/airoha/audio/llf_common/inc
endif
endif
