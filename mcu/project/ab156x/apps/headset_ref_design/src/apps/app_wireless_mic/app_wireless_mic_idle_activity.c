...
/*
void app_wireless_mic_usb_plug_in_cb(usb_evt_t event, void* usb_data, void* user_data) {
    APPS_LOG_MSGID_I(LOG_TAG" app_wireless_mic_usb_plug_in_cb: type[%d]", 1, s_app_wireless_mic_context.usb_mode);
    //usb_set_device_type(s_app_wireless_mic_context.usb_type);
    s_app_wireless_mic_context.usb_plug_in = true;
}

void app_wireless_mic_usb_plug_out_cb(usb_evt_t event, void* usb_data, void* user_data) {
    bool is_real_plug = is_vusb_ready();
    APPS_LOG_MSGID_I(LOG_TAG" app_wireless_mic_usb_plug_out_cb: type[%d], is_real_plug=%d", 2, s_app_wireless_mic_context.usb_mode, is_real_plug);
    if (is_real_plug) {
        return;
    }
...
/*
static bool app_wireless_mic_idle_proc_usb_audio_event(ui_shell_activity_t *self,
                                                       uint32_t event_id,
                                                       void *extra_data,
                                                       size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_USB_CONFIG_DONE: {
            APPS_LOG_MSGID_I(LOG_TAG" USB configure done", 0);
            ui_shell_send_event(false, EVENT_PRIORITY_HIGHEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
                                APPS_EVENTS_INTERACTION_USB_PLUG_STATE,
                                (void *)s_app_wireless_mic_context.usb_plug_in, 0, NULL, 0);
        }
