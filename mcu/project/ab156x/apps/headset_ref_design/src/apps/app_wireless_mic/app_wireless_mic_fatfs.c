...
FRESULT app_wireless_mic_fatfs_creat_wav_file(void)
{
    ...
    if (s_app_wm_fs_context.fatfs->free_clst >= s_app_wm_fs_context.reserved_clst) {
        fre_size = ((s_app_wm_fs_context.fatfs->free_clst - s_app_wm_fs_context.reserved_clst) << s_app_wm_fs_context.cluster_size_shift);
    }

    APPS_LOG_MSGID_I(LOG_TAG" creat_file: free_size=0x%x-byte", 1, fre_size);
...
extern uint32_t sd_read_time;
extern uint32_t sd_write_time;
FRESULT app_wireless_mic_fatfs_write_wav_data(uint8_t *data, uint32_t len)
{
    ...
    if (s_app_wm_fs_context.wav_cache_size + len < APP_WAV_DATA_CHACHE_MAX_LENGTH) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG" write_data: speed = %dKBs, count = %d, sd_r_time = %d, sd_w_time = %d", 4,
                         (4096 * 1000) / count, count, sd_read_time, sd_write_time);
#endif
        if (FR_OK != ret) {
