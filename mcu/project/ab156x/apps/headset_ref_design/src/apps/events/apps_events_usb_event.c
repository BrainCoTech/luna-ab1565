...
static uint8_t s_8ch_volumes[8];
uint8_t *apps_event_usb_8ch_volumes()
{
        ...
        if (chat_ep == ep_number) {
            if (channel == 0x00) {
                for (int32_t i = 0; i < 8; i++) {
                    s_8ch_volumes[i] = (volume > 100) ? 100 : (uint8_t)volume;
                }
            } else if (channel < 8) {
                s_8ch_volumes[channel - 1] = (volume > 100) ? 100 : (uint8_t)volume;
            }
        }
#endif
    }
