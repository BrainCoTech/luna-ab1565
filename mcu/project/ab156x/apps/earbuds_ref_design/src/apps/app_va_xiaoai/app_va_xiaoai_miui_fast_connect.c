...
#define MIUI_FC_ACCOUNT_KEY_HEADER_BYTE         0x04 // same as MIUI_FC_MESSAGE_TYPE_ACCOUNT_KEY

#define MIUI_FC_PASSKEY_REQUEST_TIMER           (10 * 100)
                                                      ...
                                                      uint16_t offset)
{
    if (rw == BT_GATTS_CALLBACK_READ && data != NULL) {
        ...
        APPS_LOG_MSGID_E(LOG_TAG" product_id_read_char_callback, error rw=%d data=0x%08X size=%d",
                         3, rw, data, size);
    }
    return 0;
}
...
static void miui_fc_update_ble_adv_data(uint8_t *adv_data)
{
    ...
    uint8_t byte2_ad_flags_value = 0x06;              // Byte 2, LE General Discoverable Mode + Simultaneous LE and BR/EDR to Same Device Capable
    // AD Type
    uint8_t byte3_ad_data_length = 0x12;              // Byte 3, fixed 0x12 - 18bytes
    uint8_t byte4_ad_type = 0x16;                     // Byte 4, fixed 0x16 - service data
    uint8_t byte5_ad_srv_uuid1 = ((MIUI_FC_BLE_SRV_UUID >> 8) & 0xFF);         // Byte 5, fixed
    uint8_t byte6_ad_srv_uuid2 = (MIUI_FC_BLE_SRV_UUID & 0xFF);                // Byte 6, fixed
    ...
    uint8_t byte12_flag2 = 0;                                       // Byte 12, bit1 - XiaoAI SPP/BLE Connection
...
static void miui_fc_interupt_pairing()
{
    ...
    if (miui_fc_context.conn_handle == 0) {
        APPS_LOG_MSGID_E(LOG_TAG" miui_fc_interupt_pairing, ignore", 0);
        return;
    }
    ...
    bt_status_t bt_status = bt_gap_le_disconnect(&param);
    APPS_LOG_MSGID_I(LOG_TAG" miui_fc_interupt_pairing, bt_status=0x%08X", 1, bt_status);
}
...
static void miui_fc_handle_account_key(uint8_t *key, bool is_sync)
{
    ...
    if (miui_fc_is_account_key_pairing()) {
        ...
        nvkey_status_t status = nvkey_write_data(NVKEYID_XIAOAI_KEY_MIUI_FAST_CONNECT,
...
void miui_fast_connect_init()
{
    ...
    nvkey_status_t status = nvkey_read_data(NVKEYID_XIAOAI_KEY_MIUI_FAST_CONNECT, account_key, &size);
    ...
    if (status == NVKEY_STATUS_ITEM_NOT_FOUND
        ...
        status = nvkey_write_data(NVKEYID_XIAOAI_KEY_MIUI_FAST_CONNECT, (const uint8_t *)account_key, size);
    ...
    miui_fc_context.random_account_key[2] = ((random >> 16) & 0xFF);
    miui_fc_context.random_account_key[3] = ((random >> 8) & 0xFF);
    miui_fc_context.random_account_key[4] = (random & 0xFF);
}
