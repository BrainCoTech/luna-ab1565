...
uint8_t app_va_xiaoai_get_anti_lost_state()
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" Anti-lost charger, is_left=%d own=%d peer=%d",
                     3, is_left, own, peer);
    if (is_left) {
...
#endif
#ifdef MTK_IN_EAR_FEATURE_ENABLE
    bool own_in_ear = app_in_ear_get_own_state();
    bool peer_in_ear = app_in_ear_get_peer_state();
    APPS_LOG_MSGID_I(LOG_TAG" Anti-lost in_ear, is_left=%d own=%d peer=%d",
                     3, is_left, own_in_ear, peer_in_ear);
    if (is_left) {
...
#endif
    APPS_LOG_MSGID_I(LOG_TAG" Anti-lost result=0x%02X", 1, result);
    return result;
}
