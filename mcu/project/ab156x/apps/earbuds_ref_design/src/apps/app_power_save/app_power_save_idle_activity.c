...
#ifdef AIR_LE_AUDIO_ENABLE
static void app_power_save_idle_refresh_targe_mode_by_le(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    if (NULL == self) {
        return;
    }
    if (BT_SINK_SRV_EVENT_LE_BIDIRECTION_LEA_UPDATE == event_id) {
        app_power_saving_idle_state_proc(self);
        APPS_LOG_MSGID_I(LOG_TAG", BT_SINK_SRV_EVENT_LE_BIDIRECTION_LEA_UPDATE.", 0);
    }
    if (LE_SINK_SRV_EVENT_REMOTE_INFO_UPDATE == event_id && extra_data && data_len > 0) {
        ...
        if (ind->pre_state == BT_BLE_LINK_CONNECTED && ind->state == BT_BLE_LINK_DISCONNECTED) {
            app_power_saving_idle_state_proc(self);
            APPS_LOG_MSGID_I(LOG_TAG", LE_SINK_SRV_EVENT_REMOTE_INFO_UPDATE.disconnected", 0);
        }
    }
}
#endif
...
#ifdef AIR_LE_AUDIO_ENABLE
    app_power_save_idle_refresh_targe_mode_by_le(self, event_id, extra_data, data_len);
#endif
