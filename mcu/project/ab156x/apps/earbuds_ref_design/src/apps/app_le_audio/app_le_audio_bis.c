...
bool app_le_audio_bis_start(bool start)
{
    ...
    if (start && app_bis_default_scan_params.sync_policy == BT_SINK_SRV_CAP_STREAM_SYNC_POLICY_NEXT) {
        ...
        return app_le_audio_bis_start(FALSE);
    }
    ...
    if (start) {
        ...
        } else if (is_bis_scanning) {
            success = (bt_sink_srv_cap_stream_stop_scanning_broadcast_source() == BT_STATUS_SUCCESS);
        } else {
...
static void app_le_audio_bis_action_by_peer(void)
{
    ...
    if (app_bis_sync_feature) {
        ...
        if ((app_bis_ctx.peer_state == APP_LEA_BIS_STATE_STREAMING) ||
            ...
            if (app_bis_ctx.state == APP_LEA_BIS_STATE_IDLE) {
                if (memcmp(peer_addr, empty_addr, BT_BD_ADDR_LEN) != 0) {
            ...
            } else if (app_bis_ctx.state == APP_LEA_BIS_STATE_SCANNING) {
                APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, already scanning", 0);
            } else if (app_bis_ctx.state == APP_LEA_BIS_STATE_STREAMING) {
                APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, already streaming", 0);
            } else {
                APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, not sync and start", 0);
            }
        }
    ...
    if (app_bis_ctx.peer_state == APP_LEA_BIS_STATE_IDLE) {
        if (app_bis_ctx.state != APP_LEA_BIS_STATE_IDLE) {
            ...
            if (app_bis_ctx.stopping_scan_then_retry) {
                // Trigger By Scan next, only support when MTK_AWS_MCE_ENABLE is enable
                // Wait both state to be idle state, then retry scan
                APPS_LOG_MSGID_I(LOG_TAG" SCAN NEXT, wait both in idle state, and retry scan", 0);
                app_bis_ctx.stopping_scan_then_retry = FALSE;
                app_le_audio_bis_start(TRUE);
            } else if (ble_bap_is_syncing_to_pa()) {
                bt_sink_srv_cap_stream_bmr_scan_info_ex_t *scan_info = bt_sink_srv_cap_stream_get_bmr_scan_info_ex();
                if (scan_info->sync_policy == BT_SINK_SRV_CAP_STREAM_SYNC_POLICY_NONE) {
                    APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, pa exist in idle state, stop pa", 0);
                    app_le_audio_bis_start(FALSE);
                } else if (scan_info->sync_policy == BT_SINK_SRV_CAP_STREAM_SYNC_POLICY_SELECT_BIS) {
#if defined (AIR_LE_AUDIO_BIS_ENABLE) && defined (MTK_RACE_CMD_ENABLE) && !defined(AIR_BLE_AUDIO_DONGLE_ENABLE)
