                            ...
                            audio_anc_control_misc_t *control_misc)
{
    ...
    if (anc_type == AUDIO_ANC_CONTROL_TYPE_PASSTHRU_FF) {
        APPS_LOG_MSGID_E(LOG_TAG"[HearThrough] Hearing Aid, cannot enable HW PT", 0);
        return FALSE;
    }
#endif /* AIR_HEARTHROUGH_MAIN_ENABLE */
...
{
    APPS_LOG_MSGID_I(LOG_TAG"[HearThrough] app_anc_service_set_hear_through_enabled, %d -> %d",
                        2,
...
{
    APPS_LOG_MSGID_I(LOG_TAG" [HearThrough] app_anc_service_reset_hear_through_anc, enable : %d, enabled : %d, filter_id : %d, type : %d, gain : %d",
                        5,
                        enable,
                        g_anc_srv_ht_context.enable,
                        g_anc_srv_ht_context.cur_filter_id,
    ...
    if (enable == false) {
        if (g_anc_srv_ht_context.enable == true) {
            app_anc_service_enable(g_anc_srv_ht_context.cur_filter_id,
                                    g_anc_srv_ht_context.cur_type,
                                    g_anc_srv_ht_context.cur_runtime_gain,
                                    NULL);
        } else {
