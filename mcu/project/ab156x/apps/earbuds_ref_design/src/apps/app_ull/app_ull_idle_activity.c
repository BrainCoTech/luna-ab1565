...
static atci_status_t app_le_ull_atci_hdl(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    if (0 == memcmp(pChar, "SIRK", 4)) {
        ...
        if (0 == memcmp(pChar, "96000", 5)) {
            ...
            codec_param->dl_samplerate = bt_ull_le_srv_get_client_preffered_dl_codec_samplerate();
            codec_param->ul_samplerate = bt_ull_le_srv_get_client_preffered_ul_codec_samplerate();
    ...
    switch (event) {
        case ULL_EVENTS_DONGLE_CONNECTED: {
            ...
            if (ULL_LINK_MODE_SINGLE == s_ull_context.link_mode) {
                ...
                bt_app_common_pre_set_ultra_low_latency_retry_count(BT_APP_COMMON_ULL_LATENCY_MODULE_RECONNECT, BT_ULL_LE_SRV_LATENCY_MULTI_LINK_LEA_INTERVAL_MORE_THAN_30MS_STANDBY_MODE);
            }
#endif
            else {
...
static bool app_ull_proc_ui_shell_group(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len) {
    ...
    switch (event_id) {
        case EVENT_ID_SHELL_SYSTEM_ON_CREATE: {
            ...
            APPS_LOG_MSGID_I(LOG_TAG", ull speaker mode en=%d", 1, s_speaker_mode);
...
#if defined(AIR_BLE_ULL_PARING_MODE_ENABLE) && defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
