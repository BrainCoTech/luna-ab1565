...
typedef struct {
    ...
    uint16_t            freq;
    uint8_t             dbfs;
} __attribute__((packed)) app_hearing_aid_pure_tone_generator_get_response_t;
#endif

typedef struct {
    uint8_t a2dp_mix_mode_switch  : 1;
    uint8_t reserved_1            : 7;
    int8_t reserved_2;
    int8_t reserved_3;

    uint8_t sco_mix_mode_switch   : 1;
    uint8_t reserved_4            : 7;
    int8_t reserved_5;
    int8_t reserved_6;

    uint8_t vp_mix_mode_switch   : 1;
    uint8_t reserved_7           : 7;
    int8_t reserved_8;
    int8_t reserved_9;
} __attribute__((packed)) app_ha_mix_mode_t;
...
extern audio_psap_status_t audio_psap_control_trial_run(ha_trial_run_event_t event, U32 data_len, void* data);
...
static void app_hearing_aid_utils_ha_callback_handler(ha_noti_event_t event, uint8_t *extra_data, uint16_t extra_data_len)
{
    ...
    if (event == HA_NOTI_EVENT_FB_DETECT) {
    ...
    } else if (event == HA_NOTI_EVENT_AEA_CHANGE_MODE) {
...
void app_hearing_aid_utils_init(notify handler)
{
    ...
    audio_psap_status_t init_status = audio_psap_control_init();
    audio_psap_status_t init_detect_fb_status = audio_psap_control_register_notification_callback(app_hearing_aid_utils_ha_callback_handler);
...
void app_hearing_aid_utils_deinit()
{
    ...
    // audio_psap_control_close_framework();
    memset(&app_ha_utils_context, 0, sizeof(app_hearing_aid_utils_context_t));
}
...
static bool app_hearing_aid_utils_get_combine_response_checker(uint8_t *local, uint16_t local_length, uint8_t *remote, uint16_t remote_len, uint8_t *response, uint16_t *response_len)
{
    if (local == NULL || local_length == 0
        || remote == NULL || remote_len == 0
...
void app_hearing_aid_utils_handle_get_combine_response(uint16_t type,
                                                       uint8_t *local_response, uint16_t local_response_len,
                                                       uint8_t *remote_response, uint16_t remote_response_len,
                                                       uint8_t *response, uint16_t *response_len)
{
    if (app_hearing_aid_utils_get_combine_response_checker(local_response, local_response_len, remote_response, remote_response_len, response, response_len) == false) {
    ...
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    APPS_LOG_MSGID_I(APP_HA_UTILS_TAG"[get_combine_response] role : %d", 1, role);

    if (app_ha_exe_handler_list[type].ha_cmd_get_combine_handler != NULL) {
        app_ha_exe_handler_list[type].ha_cmd_get_combine_handler(local_response, local_response_len,
...
bool app_hearing_aid_utils_get_level_index(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    uint8_t l_index = 0;
    uint8_t r_index = 0;
    audio_psap_status_t status = audio_psap_control_get_level_index(&l_index, &r_index);
...
bool app_hearing_aid_utils_get_mode_max_count(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_level_mode_max_count(&level_max_count, &mode_max_count, &vol_max_count);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
    *response_len = 0;
    if (ret_value == true) {
...
bool app_hearing_aid_utils_get_volume_index(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_volume_index(&l_vol_index, &r_vol_index);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
    *response_len = 0;
    if (ret_value == true) {
...
bool app_hearing_aid_utils_get_mode_index(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_mode_index(&mode_index);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
{
    ha_mode_table_t mode_table = {0};
    uint8_t mode_index = request->op_parameter[0];
    audio_psap_status_t status = audio_psap_control_get_specific_mode_table(mode_index, &mode_table);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        response[0] = mode_index;
        memcpy(response + 1, &mode_table, sizeof(ha_mode_table_t));
        *response_len = sizeof(ha_mode_table_t) + 1;
    }
    return ret_value;
}
...
{
    ha_aea_config_t config = {0};
    audio_psap_status_t status = audio_psap_control_get_aea_configuration(&config);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &config, sizeof(ha_aea_config_t));
        *response_len = sizeof(ha_aea_config_t);
    }
    return ret_value;
}
...
{
    bool enable = 0;
    audio_psap_status_t status = audio_psap_control_get_wnr_switch(&enable);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
{
    ha_bf_config_t config = {0};
    audio_psap_status_t status = audio_psap_control_get_beamforming_setting(&config);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &config, sizeof(ha_bf_config_t));
        *response_len = sizeof(ha_bf_config_t);
    }
    return ret_value;
}
...
{
    ha_afc_config_t config = {0};
    audio_psap_status_t status = audio_psap_control_get_afc_configuration(&config);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &config, sizeof(ha_afc_config_t));
        *response_len = sizeof(ha_afc_config_t);
    }
    return ret_value;
}
...
bool app_hearing_aid_utils_get_inr_config(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_inr_configuration(&config);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
{
    ha_user_eq_switch_t config = {0};
    audio_psap_status_t status = audio_psap_control_get_user_eq_switch(&config);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &config, sizeof(ha_user_eq_switch_t));
        *response_len = sizeof(ha_user_eq_switch_t);
    }
    return ret_value;
}
...
{
    ha_usr_eq_para_t eq_para = {0};
    audio_psap_status_t status = audio_psap_control_get_user_eq_gain(&eq_para);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &eq_para, sizeof(ha_usr_eq_para_t));
        *response_len = sizeof(ha_usr_eq_para_t);
    }
    return ret_value;
}
...
{
    ha_test_spk_ref_t spk_ref = {0};
    audio_psap_status_t status = audio_psap_control_get_speaker_reference(&spk_ref);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &spk_ref, sizeof(ha_test_spk_ref_t));
        *response_len = sizeof(ha_test_spk_ref_t);
    }
    return ret_value;
}
...
bool app_hearing_aid_utils_get_pure_tone_generator(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    status = audio_psap_control_get_puretone_generator(&l_enable, &l_freq, &l_dbfs, &r_enable, &r_freq, &r_dbfs);
...
{
    ha_scenario_mix_mode_t mix_mode = {0};
    audio_psap_status_t status = audio_psap_control_get_mix_mode(&mix_mode);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    app_ha_mix_mode_t app_mix_mode;
    memcpy(&app_mix_mode, &mix_mode, sizeof(app_ha_mix_mode_t));
    ...
    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &app_mix_mode, sizeof(ha_scenario_mix_mode_t));
        *response_len = sizeof(ha_scenario_mix_mode_t);
    }
    return ret_value;
}
...
bool app_hearing_aid_utils_get_hearing_tuning_mode_switch(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_tuning_mode(&mode_switch);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
{
    uint8_t mp_mode = 0;
    audio_psap_status_t status = audio_psap_control_get_mp_test_mode(&mp_mode);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
bool app_hearing_aid_utils_get_feedback_detection(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    *response_len = 0;
    ...
    if (enable == true) {
        get_fb_status = audio_psap_control_detect_feedback();
    }
    ...
    if (get_fb_status == AUDIO_PSAP_STATUS_SUCCESS) {
...
bool app_hearing_aid_utils_get_mute(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_mute(&l_mute, &r_mute);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
{
    ha_how_det_t det = {0};
    audio_psap_status_t status = audio_psap_control_get_howling_detection(&det);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;

    *response_len = 0;
    if (ret_value == true) {
        memcpy(response, &det, sizeof(ha_how_det_t));
        *response_len = sizeof(ha_how_det_t);
    }
    return ret_value;
}
...
bool app_hearing_aid_utils_get_mpo_adjustment(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_status_t status = audio_psap_control_get_mpo_adjustment(&adjust);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
bool app_hearing_aid_utils_get_passthrough_switch(app_hear_through_request_t *request, uint8_t *response, uint16_t *response_len)
{
    bool l_enable = 0;
    bool r_enable = 0;
    audio_psap_status_t status = audio_psap_control_get_passthrough_switch(&l_enable, &r_enable);
    bool ret_value = (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
{
    uint8_t channel = 0;
    audio_psap_status_t status = audio_psap_control_get_mic_channel(&channel);
...
bool app_hearing_aid_utils_set_level_index(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_level_index(parameter[0], parameter[1], role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_volume_index(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_volume_index(index->l_vol_index, index->r_vol_index, role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_mode_index(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_mode_index(parameter[0]);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_specific_mode_table(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_specific_mode_table(parameter[0], (ha_mode_table_t *)(parameter + 1), role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_aea_configuration(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_aea_configuration((ha_aea_config_t *)parameter);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
...
bool app_hearing_aid_utils_set_wnr_switch(uint8_t *parameter)
{
    ...
    bool enable = (bool)(parameter[0]);
    audio_psap_status_t status = audio_psap_control_set_wnr_switch(enable);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_beam_forming_setting(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_beamforming_setting((ha_bf_config_t *)parameter);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_afc_config(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_afc_configuration((ha_afc_config_t *)parameter, role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_inr_config(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_inr_configuration((ha_inr_config_t *)parameter, role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_user_eq_switch(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_user_eq_switch((ha_user_eq_switch_t *)parameter, role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_user_eq_gain(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_user_eq_gain((ha_usr_eq_para_t *)parameter, role);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
#if 0
    if (enable == true) {
        enable_status = audio_psap_control_enable();
    }
#endif
#if 0
    if (((enable == true)/* && (enable_status == AUDIO_PSAP_STATUS_SUCCESS)*/)
        || (enable == false)) {
        set_pure_status = audio_psap_control_set_puretone_generator(enable, freq, dbfs);
    }
#endif

    set_pure_status = audio_psap_control_set_puretone_generator(config->l_switch,
                                                                   config->l_freq,
                                                                   config->l_dbfs,
                                                                   config->r_switch,
                                                                   config->r_freq,
...
bool app_hearing_aid_utils_set_mix_mode_total_setting(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_mix_mode((ha_scenario_mix_mode_t *)parameter, role);

    app_ha_mix_mode_t *mix_mode = (app_ha_mix_mode_t *)parameter;
    app_hearing_aid_storage_set_a2dp_mix_switch(mix_mode->a2dp_mix_mode_switch);
    app_hearing_aid_storage_set_sco_mix_switch(mix_mode->sco_mix_mode_switch);
    app_hearing_aid_storage_set_vp_mix_switch(mix_mode->vp_mix_mode_switch);

    if (status == AUDIO_PSAP_STATUS_SUCCESS) {
        app_hearing_aid_activity_pre_proc_operate_ha(APP_HEARING_AID_CHANGE_CAUSE_MIX_TABLE, false, false);
    }
...
bool app_hearing_aid_utils_set_hearing_tuning_mode_switch(uint8_t *parameter)
{
    ...
    if ((role == AUDIO_PSAP_DEVICE_ROLE_LEFT) && (APP_HA_IS_ENABLE(mode_value, 0))) {
        status = audio_psap_control_set_tuning_mode(mode_value);
    }
...
bool app_hearing_aid_utils_set_mp_test_mode_switch(uint8_t *parameter)
{
    ...
    if (((enable == APP_HA_MP_TEST_MODE_SWITCH_ON) && (enable_status == AUDIO_PSAP_STATUS_SUCCESS))
        ...
        set_status = audio_psap_control_set_mp_test_mode(enable);
    }
...
bool app_hearing_aid_utils_set_restore_setting(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_restore_setting(parameter[0]);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}

bool app_hearing_aid_utils_set_mute(uint8_t *parameter)
{
    audio_psap_status_t status = audio_psap_control_set_mute(parameter[0], parameter[1]);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_howling_detection(uint8_t *parameter)
{
    ...
    audio_psap_status_t status = audio_psap_control_set_howling_detection((ha_how_det_t *)parameter);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}

bool app_hearing_aid_utils_set_mpo_adjustment(uint8_t *parameter)
{
    audio_psap_status_t status = audio_psap_control_set_mpo_adjustment((ha_mpo_adjust_t *)parameter);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
{
    audio_psap_status_t status = audio_psap_control_set_passthrough_switch(parameter[0], parameter[1]);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}

bool app_hearing_aid_utils_set_mic_channel(uint8_t *parameter)
{
    audio_psap_status_t status = audio_psap_control_set_mic_channel(parameter[0]);
    return (status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false;
}
...
bool app_hearing_aid_utils_set_trial_run(uint8_t *parameter)
{
    ...
    if (((role == AUDIO_PSAP_DEVICE_ROLE_LEFT) && (trial_run_payload->channel == APP_HA_CHANNEL_L))
        ...
        status = audio_psap_control_trial_run(trial_run_payload->trial_run_id,
                                                 trial_run_payload->trial_run_param_len,
...
{
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    ha_mode_table_t *local_mode = (ha_mode_table_t *)(local + 1);
    ha_mode_table_t *remote_mode = (ha_mode_table_t *)(remote + 1);
    ha_mode_table_t *rsp_mode = (ha_mode_table_t *)(response + 1);
    ...
    *response_len = 1 + sizeof(ha_mode_table_t);
    return true;
}
...
{
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    ha_afc_config_t *local_config = (ha_afc_config_t *)local;
    ha_afc_config_t *remote_config = (ha_afc_config_t *)remote;
    ha_afc_config_t *rsp_config = (ha_afc_config_t *)response;
...
{
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    ha_user_eq_switch_t *local_config = (ha_user_eq_switch_t *)local;
    ha_user_eq_switch_t *remote_config = (ha_user_eq_switch_t *)remote;
    ha_user_eq_switch_t *rsp_config = (ha_user_eq_switch_t *)response;
...
{
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    ha_usr_eq_para_t *local_config = (ha_usr_eq_para_t *)local;
    ha_usr_eq_para_t *remote_config = (ha_usr_eq_para_t *)remote;
    ha_usr_eq_para_t *rsp_config = (ha_usr_eq_para_t *)response;

    if (role == AUDIO_PSAP_DEVICE_ROLE_LEFT) {
        rsp_config->ha_user_eq_overall_l = local_config->ha_user_eq_overall_l;
        memcpy(rsp_config->ha_user_eq_band_l, local_config->ha_user_eq_band_l, HA_BAND_NUM);
        rsp_config->ha_user_eq_overall_r = remote_config->ha_user_eq_overall_r;
        memcpy(rsp_config->ha_user_eq_band_r, remote_config->ha_user_eq_band_r, HA_BAND_NUM);
    } else if (role == AUDIO_PSAP_DEVICE_ROLE_RIGHT) {
        rsp_config->ha_user_eq_overall_l = remote_config->ha_user_eq_overall_l;
        memcpy(rsp_config->ha_user_eq_band_l, remote_config->ha_user_eq_band_l, HA_BAND_NUM);
        rsp_config->ha_user_eq_overall_r = local_config->ha_user_eq_overall_r;
        memcpy(rsp_config->ha_user_eq_band_r, local_config->ha_user_eq_band_r, HA_BAND_NUM);
    } else {
        *response_len = 0;
        return false;
    }
...
bool app_hearing_aid_utils_get_speaker_reference_combine_response(uint8_t *local, uint16_t local_length, uint8_t *remote, uint16_t remote_len, uint8_t *response, uint16_t *response_len)
{
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    ha_test_spk_ref_t *local_config = (ha_test_spk_ref_t *)local;
    ha_test_spk_ref_t *remote_config = (ha_test_spk_ref_t *)remote;
    ha_test_spk_ref_t *rsp_config = (ha_test_spk_ref_t *)response;
    ...
    if (role == AUDIO_PSAP_DEVICE_ROLE_LEFT) {
        ...
        rsp_config->test_spk_ref_R_64       =  remote_config->test_spk_ref_R_64;
        rsp_config->test_spk_ref_R_125      =  remote_config->test_spk_ref_R_125;
        rsp_config->test_spk_ref_R_250      =  remote_config->test_spk_ref_R_250;
        rsp_config->test_spk_ref_R_500      =  remote_config->test_spk_ref_R_500;
        rsp_config->test_spk_ref_R_1000     =  remote_config->test_spk_ref_R_1000;
        rsp_config->test_spk_ref_R_2000     =  remote_config->test_spk_ref_R_2000;
        rsp_config->test_spk_ref_R_4000     =  remote_config->test_spk_ref_R_4000;
        rsp_config->test_spk_ref_R_6000     =  remote_config->test_spk_ref_R_6000;
        rsp_config->test_spk_ref_R_8000     =  remote_config->test_spk_ref_R_8000;
        rsp_config->test_spk_ref_R_12000    =  remote_config->test_spk_ref_R_12000;
#if 0
        ...
        rsp_config->test_spk_ref_R_64       =  local_config->test_spk_ref_R_64;
        rsp_config->test_spk_ref_R_125      =  local_config->test_spk_ref_R_125;
        rsp_config->test_spk_ref_R_250      =  local_config->test_spk_ref_R_250;
        rsp_config->test_spk_ref_R_500      =  local_config->test_spk_ref_R_500;
        rsp_config->test_spk_ref_R_1000     =  local_config->test_spk_ref_R_1000;
        rsp_config->test_spk_ref_R_2000     =  local_config->test_spk_ref_R_2000;
        rsp_config->test_spk_ref_R_4000     =  local_config->test_spk_ref_R_4000;
        rsp_config->test_spk_ref_R_6000     =  local_config->test_spk_ref_R_6000;
        rsp_config->test_spk_ref_R_8000     =  local_config->test_spk_ref_R_8000;
        rsp_config->test_spk_ref_R_12000    =  local_config->test_spk_ref_R_12000;
    } else {
        *response_len = 0;
        return false;
    }
...
bool app_hearing_aid_utils_get_mix_mode_total_setting_combine_response(uint8_t *local, uint16_t local_length, uint8_t *remote, uint16_t remote_len, uint8_t *response, uint16_t *response_len)
{
    ...
    audio_psap_device_role_t role = app_hearing_aid_utils_get_role();
    ha_scenario_mix_mode_t *local_config = (ha_scenario_mix_mode_t *)local;
    ha_scenario_mix_mode_t *remote_config = (ha_scenario_mix_mode_t *)remote;
    ha_scenario_mix_mode_t *rsp_config = (ha_scenario_mix_mode_t *)response;
...
bool app_hearing_aid_utils_volume_index_notify(uint8_t role, uint8_t *data, uint16_t data_len, uint8_t *notify_data, uint16_t *notify_data_len)
{
    ...
    uint8_t l_index = 0;
    uint8_t r_index = 0;
    audio_psap_status_t status = audio_psap_control_get_volume_index(&l_index, &r_index);
...
{
#if 0
    memcpy(notify_data, data, sizeof(ha_aea_config_t));
    *notify_data_len = sizeof(ha_aea_config_t);
#endif
    app_hearing_aid_utils_common_notify_handler(data, data_len, notify_data, notify_data_len);
    return true;
}
...
{
#if 0
    memcpy(notify_data, data, sizeof(ha_bf_config_t));
    *notify_data_len = sizeof(ha_bf_config_t);
#endif
    app_hearing_aid_utils_common_notify_handler(data, data_len, notify_data, notify_data_len);
    return true;
}
...
{
    ha_test_spk_ref_t *data_spk_ref = (ha_test_spk_ref_t *)data;
    ha_test_spk_ref_t local_spk_ref = {0};
    audio_psap_device_role_t local_role = app_hearing_aid_utils_get_role();

    if (role == local_role) {
        memcpy(notify_data, data_spk_ref, sizeof(ha_test_spk_ref_t));
        *notify_data_len = sizeof(ha_test_spk_ref_t);
    } else {
        audio_psap_status_t status = audio_psap_control_get_speaker_reference(&local_spk_ref);
        ...
        app_hearing_aid_utils_get_speaker_reference_combine_response((uint8_t *)&local_spk_ref,
                                                                     sizeof(ha_test_spk_ref_t),
                                                                     data,
                                                                     sizeof(ha_test_spk_ref_t),
                                                                     notify_data,
                                                                     notify_data_len);
...
bool app_hearing_aid_utils_hearing_tuning_mode_switch_notify(uint8_t role, uint8_t *data, uint16_t data_len, uint8_t *notify_data, uint16_t *notify_data_len)
{
    ...
    if (role == local_role) {
        ...
        *notify_data_len = sizeof(app_hearing_aid_hearing_tuning_mode_t);
        ...
        uint8_t mode = 0;
        audio_psap_status_t status = audio_psap_control_get_tuning_mode(&mode);
...
bool app_hearing_aid_utils_is_ha_running()
{
    ...
    status = audio_psap_control_get_status(&ha_enable_status);
...
{
    ha_bf_config_t bf_config = {0};
    audio_psap_status_t status = audio_psap_control_get_beamforming_setting(&bf_config);
...
{
    ha_aea_config_t current_config = {0};
    audio_psap_status_t get_status = audio_psap_control_get_aea_configuration(&current_config);
...
{
    uint8_t channel = 0;
    audio_psap_status_t get_status = audio_psap_control_get_mic_channel(&channel);
...
bool app_hearing_aid_utils_control_ha(bool enable)
{
    ...
    // if (enable != ha_enable_status) {
        if (enable == true) {
            status = audio_psap_control_enable();
        } else {
            status = audio_psap_control_disable();
        }
        return ((status == AUDIO_PSAP_STATUS_SUCCESS) ? true : false);
    // }
...
bool app_hearing_aid_utils_adjust_level(uint8_t l_index, uint8_t r_index)
{
    ...
    audio_psap_status_t set_result = audio_psap_control_set_level_index(l_index, r_index, device_role);
...
bool app_hearing_aid_utils_adjust_volume(uint8_t l_index, uint8_t r_index)
{
    ...
    audio_psap_status_t set_result = audio_psap_control_set_volume_index(l_index, r_index, device_role);
...
bool app_hearing_aid_utils_adjust_mode(uint8_t target_mode_index)
{
    ...
    audio_psap_status_t mode_set_status = audio_psap_control_set_mode_index(target_mode_index);
...
bool app_hearing_aid_utils_get_level_change_value(bool up, bool circular, app_hearing_aid_change_value_t *change_value)
{
    ...
    audio_psap_status_t get_level_index_status = audio_psap_control_get_level_index(&l_level_index, &r_level_index);
    audio_psap_status_t level_max_count_status = audio_psap_control_get_level_mode_max_count(&level_max_count, &mode_max_count, &vol_max_count);
    ...
    APPS_LOG_MSGID_I(APP_HA_UTILS_TAG"[app_hearing_aid_utils_get_level_change_value] up : %d, circular : %d, l_index : %d, r_index : %d, vol_max : %d, sync switch ; %d",
                     6,
                     up,
                     circular,
                     l_level_index,
...
bool app_hearing_aid_utils_get_volume_change_value(bool up, bool circular, app_hearing_aid_change_value_t *change_value)
{
    ...
    audio_psap_status_t get_vol_index_status = audio_psap_control_get_volume_index(&l_vol_index, &r_vol_index);
    audio_psap_status_t vol_max_count_status = audio_psap_control_get_level_mode_max_count(&level_max_count, &mode_max_count, &vol_max_count);
...
bool app_hearing_aid_utils_get_mode_change_value(bool up, bool circular, uint8_t *out_mode, bool *max, bool *min)
{
    ...
    audio_psap_status_t mode_index_status = audio_psap_control_get_mode_index(&mode_index);
    audio_psap_status_t mode_max_count_status = audio_psap_control_get_level_mode_max_count(&level_max_count, &mode_max_count, &vol_max_count);
...
bool app_hearing_aid_utils_hearing_tuning_mode_toggle(bool from_remote)
{
    ...
    if (status == AUDIO_PSAP_STATUS_SUCCESS) {
        ...
        status = audio_psap_control_set_tuning_mode(temp_tuning_mode_t.uint8_t_mode);
...
bool app_hearing_aid_utils_beam_forming_switch_toggle(bool enable)
{
    ...
    status = audio_psap_control_set_beamforming_setting(&bf_config);
...
bool app_hearing_aid_utils_aea_switch_toggle(bool enable)
{
    ...
    set_status = audio_psap_control_set_aea_configuration(&current_config);
...
bool app_hearing_aid_utils_master_mic_channel_switch_toggle(uint8_t target_channel)
{
    ...
    set_status = audio_psap_control_set_mic_channel(channel);
...
{
    audio_psap_control_save_setting();
    return true;
}

bool app_hearing_aid_utils_control_fwk(bool enable)
{
    audio_psap_status_t set_status = AUDIO_PSAP_STATUS_SUCCESS;
    if (enable == true) {
        set_status = audio_psap_control_open_framework();
    } else {
        set_status = audio_psap_control_close_framework();
    }
...
{
    ha_scenario_mix_mode_t mix_mode = {0};
    audio_psap_status_t psap_status = audio_psap_control_get_mix_mode(&mix_mode);
