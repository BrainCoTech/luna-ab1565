                                                  ...
                                                  size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case BT_SINK_SRV_EVENT_LE_BIDIRECTION_LEA_UPDATE: {
            ...
            if (hfp_context && event
                && BT_SINK_SRV_BIDIRECTION_LEA_STATE_ENABLE == event->state
                && false == app_le_audio_aird_client_is_support(event->le_handle)/*(true) = device is dongle?*/) {
                ...
                APPS_LOG_MSGID_I(APP_HFP_IDLE",[BIDIRECTION_LEA_STATE] le call start ", 0);
            }
