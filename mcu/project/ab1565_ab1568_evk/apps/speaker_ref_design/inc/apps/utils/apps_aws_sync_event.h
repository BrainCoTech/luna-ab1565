...
/**
* @brief      This function is used to send UI Shell event without extra_data between Agent and Partner through urgent channel.
* @param[in]  event_group, the current event group to be sent.
* @param[in]  event_id, the current event ID to be sent.
* @return
* #BT_STATUS_SUCCESS, if the operation is successful.
...
*/
void apps_aws_sync_event_decode_extra(bt_aws_mce_report_info_t *aws_data_ind,
...
* @param[out] event_id, received event ID.
*/
void apps_aws_sync_event_decode(bt_aws_mce_report_info_t *aws_data_ind,
                                uint32_t *event_group, uint32_t *event_id);
