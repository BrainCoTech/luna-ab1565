...
static uint32_t app_test_ble_adv_data_func(multi_ble_adv_info_t *adv_info)
{
    if (adv_info) {
...
void apps_events_test_event_ble_adv_start(bool enable)
{
    ...
    if (enable) {
        multi_ble_adv_manager_add_ble_adv(MULTI_ADV_INSTANCE_DEFAULT, app_test_ble_adv_data_func, 1);
    } else {
        multi_ble_adv_manager_remove_ble_adv(MULTI_ADV_INSTANCE_DEFAULT, app_test_ble_adv_data_func);
    }
