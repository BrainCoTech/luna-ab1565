        ...
        case KEY_DISCOVERABLE:
            ...
            if (bt_device_manager_aws_local_info_get_role() == BT_AWS_MCE_ROLE_PARTNER) {
                if (local_context->aws_connected) {
                    ...
                    APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY", Partner aws disconnected, ignore KEY_DISCOVERABLE", 0);
#else
...
static bool homescreen_app_aws_event_proc(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    ret = bt_conn_component_aws_event_proc(self, event_id, extra_data, data_len);
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_INTERACTION_BT_RECONNECT_TIMEOUT:
            if (!local_ctx->connection_state) {
                ...
                if (connecting_number > 0 && 0 == bt_cm_get_gap_handle(p_bd_addr[0])) {
                    bt_cm_connect_t connect_param = { {0},
                        ~(BT_CM_PROFILE_SERVICE_MASK(BT_CM_PROFILE_SERVICE_AWS))
                    };
                    memcpy(connect_param.address, p_bd_addr[1], sizeof(bt_bd_addr_t));
                    bt_cm_disconnect(&connect_param);
                } else {
                    APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY", stop reconnect, addr is NULL.", 0);
                }
            }
            break;
