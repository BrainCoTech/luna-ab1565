...
/* RHO communication structure with is_vp (HFP incoming VP) flag. */
typedef struct {
    uint8_t is_vp;
} app_rho_data_t;
...
uint8_t App_role_handover_service_get_data_len_callback(const bt_bd_addr_t *addr)
{
    uint8_t len = sizeof(app_rho_data_t);
    APPS_LOG_MSGID_I(RHO_IDLE_ACTIVITY" get_data_len_callback len=%d", 1, len);
    return len;
}

bt_status_t App_role_handover_get_data_callback(const bt_bd_addr_t *addr, void *data)
{
    app_rho_data_t *data_p = (app_rho_data_t *)data;
    uint8_t is_vp = app_hfp_get_incoming_call_vp_flag();
    APPS_LOG_MSGID_I(RHO_IDLE_ACTIVITY" get_data_cb is_vp=%d", 1, is_vp);
    data_p->is_vp = is_vp;
    return BT_STATUS_SUCCESS;
}

bt_status_t App_role_handover_update_data_callback(bt_role_handover_update_info_t *info)
{
    APPS_LOG_MSGID_I(RHO_IDLE_ACTIVITY" update_data_cb role=0x%02X", 1, info->role);
    if (info->role == BT_AWS_MCE_ROLE_PARTNER) {
        app_rho_data_t *data_p = (app_rho_data_t *)info->data;
        uint8_t is_vp = data_p->is_vp;
        APPS_LOG_MSGID_I(RHO_IDLE_ACTIVITY" update_data_cb is_vp=%d", 1, is_vp);
        app_hfp_set_incoming_call_vp_flag(is_vp);
    }
    return BT_STATUS_SUCCESS;
}
                                                           ...
                                                           bt_status_t status)
{
    ...
    if (BT_ROLE_HANDOVER_START_IND == event) {
        ...
        if (status == BT_STATUS_CONNECTION_IN_SNIFF) {
            APPS_LOG_MSGID_I(RHO_IDLE_ACTIVITY" fail reason in sniff, retry", 0);
            _do_rho(&s_app_rho_context);
            return;
        }
    ...
    switch (event_id) {
        case EVENT_ID_SHELL_SYSTEM_ON_CREATE: {
            ...
            bt_role_handover_callbacks_t role_callbacks = {
                NULL,
                App_role_handover_service_get_data_len_callback,
                App_role_handover_get_data_callback,
                App_role_handover_update_data_callback,
