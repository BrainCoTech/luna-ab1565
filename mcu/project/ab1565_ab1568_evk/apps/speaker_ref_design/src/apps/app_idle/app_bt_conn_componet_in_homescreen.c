...
bool bt_conn_component_aws_event_proc(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    bool ret = false;

    if (BT_AWS_MCE_SRV_EVENT_MODE_CHANGED_IND == event_id && extra_data != NULL) {
        home_screen_local_context_type_t *local_ctx = (home_screen_local_context_type_t *)self->local_context;
        APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY" mode changed, clear conn info", 0);
        local_ctx->connection_state = false;
