...
char *app_ama_match_locale(char *language)
{
    ...
    for (index = 0; index < LOCAL_MAPPING_TABLE_LEN; index ++) {
        if (strcmp(language, local_mapping[index].expected_locale) == 0) {
            return local_mapping[index].target_locale;
        }
    }
    return NULL;
}
...
static bool ama_wwd_trigger_handler(uint32_t stop_index)
{
    ...
    } else if (bt_device_manager_aws_local_info_get_role() == BT_AWS_MCE_ROLE_CLIENT) {
        APPS_LOG_MSGID_I(APP_AMA_ACTI", [WWD] ama_wwd_trigger_handler, Client ignore", 0);
        return true;
    } else
...
static void audio_recorder_released_callback_handler(void)
{
    ...
    if (app_adaptive_anc_get_state() != APP_ADAPTIVE_ANC_IDLE) {
        APPS_LOG_MSGID_I(APP_AMA_ACTI", audio_recorder_released_callback_handler, recorder is using by adaptive ANC, cannot start AMA WWE", 0);
        return;
    }
#endif /* MTK_USER_TRIGGER_ADAPTIVE_FF_V2 */
...
static bool app_ama_handle_key_event(apps_config_key_action_t key_event)
{
    ...
    if (bt_device_manager_aws_local_info_get_role() == BT_AWS_MCE_ROLE_PARTNER) {
        ...
        APPS_LOG_MSGID_I(APP_AMA_ACTI", _proc_key_event_group, Client ignore", 0);
        return true;
    }
#endif /* MTK_AWS_MCE_ENABLE */
...
void app_ama_forward_at_cmd(AMA_FORWARD_AT_COMMAND_IND_T *ind)
{
    ...
    if (memcmp(ind->command, "ATD", strlen("ATD")) == 0) {
        ...
        if ((strlen(ind->command) - strlen("ATD") - 1) < 64) {
            memcpy(x_number, ind->command + strlen("ATD"), (strlen(ind->command) - strlen("ATD") - 1));

            send_status = bt_sink_srv_send_action(BT_SINK_SRV_ACTION_DIAL_NUMBER, (void*)x_number);
        }
    }
    if (memcmp(ind->command, "AT+BLDN", strlen("AT+BLDN")) == 0) {
...
bool app_ama_multi_va_set_configuration(bool selected)
{
    if (selected == false) {
        if (app_ama_is_valid_address(ama_activity_context.context_addr.addr) == true) {
            ...
            AMA_Target_DisconnectRequest(&(ama_activity_context.context_addr.addr));
            return true;
        } else {
...
void app_ama_configure_wwe_parameter(uint8_t *language)
{
    ...
    char *target_locale = app_ama_match_locale(language);
    if (target_locale == NULL) {
    ...
    err = va_model_manager_load(VA_MODEL_MANAGER_VA_TYPE_AMA, target_locale, &load_info);
    err = va_model_manager_load_finish(VA_MODEL_MANAGER_VA_TYPE_AMA, target_locale);
...
uint32_t app_ama_get_mtu()
{
    ...
    return AMA_Target_GetMtu(ama_activity_context.context_addr.addr);
}
