...
static bool app_music_proc_key_event_group(
...
#ifdef AIR_ROTARY_ENCODER_ENABLE
static bool app_music_proc_rotary_event_group(
    ...
    switch (key_action) {
        ...
        case KEY_VOICE_DN: {
            ...
            if ((BT_STATUS_SUCCESS == bt_ull_get_streaming_info(streaming,&info)) && info.is_playing) {
                ...
                APPS_LOG_MSGID_I("app_music_proc_rotary_event_group, ull delat volume : %d, result: %d", 2, rotary_data, bt_status);
            }
            else
#endif
            {
                ...
                APPS_LOG_MSGID_I("app_music_proc_rotary_event_group, music volume : %d", 1, volume);
...
static bool app_music_proc_bt_event_when_playing(
    struct _ui_shell_activity *self,
    uint32_t event_id,
    void *extra_data,
    size_t data_len)
{
    ...
    if (event_id == BT_SINK_SRV_EVENT_STATE_CHANGE) {
        ...
        if ((param->previous == BT_SINK_SRV_STATE_STREAMING) && (param->current != BT_SINK_SRV_STATE_STREAMING)) {
            ...
            APPS_LOG_MSGID_I(APP_MUSIC_ACTI" app_music_proc_bt_event:ui_shell_finish_activity, current activity:0x%x, isAutoPaused=%d", 2,
        ...
        if (BT_AVRCP_STATUS_PLAY_PAUSED == avrcp_status || BT_AVRCP_STATUS_PLAY_STOPPED == avrcp_status) {
            if (0 == memcmp(local_context->activate_con_addr, event->avrcp_status_change.address, sizeof(bt_bd_addr_t))){
                ...
                local_context->isAutoPaused = false;
                ui_shell_finish_activity(self, self);
                APPS_LOG_MSGID_I(APP_MUSIC_ACTI" app_music_proc_bt_event:ui_shell_finish_activity, current activity:0x%x, avrcp_status=%d", 2,
                        (uint32_t)self, avrcp_status);
            }
        }
    }
#endif
...
static bool app_music_proc_apps_internal_events(
    struct _ui_shell_activity *self,
    uint32_t event_id,
    void *extra_data,
    size_t data_len)
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_INTERACTION_IN_EAR_UPDATE_STA: {
            ...
            if (BT_STATUS_SUCCESS == bt_ull_get_streaming_info(streaming,&info)) {
                ...
                //ret = app_music_in_ear_update_music_state(self, extra_data);
            }
            break;
        }
#endif
        ...
        case APPS_EVENTS_INTERACTION_PARTNER_SWITCH_TO_AGENT: {
            ...
            APPS_LOG_MSGID_I(APP_MUSIC_ACTI" [new Agent] RHO done - rho_result=%d, music_playing=%d",
