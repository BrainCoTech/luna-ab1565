...
static uint8_t _role_handover_service_get_data_len_callback(const bt_bd_addr_t *addr)
{
...
#endif
    {
        ...
        APPS_LOG_MSGID_I(APP_MUSIC_TAG" get_data_len_callback: %d", 1, data_len);
...
static bt_status_t _role_handover_get_data_callback(const bt_bd_addr_t *addr, void *data)
{
...
#endif
    {
        ...
        APPS_LOG_MSGID_I(APP_MUSIC_TAG" get_data_callback isAutoPaused: %d", 1, s_app_music_context.isAutoPaused);
    }
...
{
    APPS_LOG_MSGID_I(APP_MUSIC_TAG" update_data_callback, recv role=0x%02X", 1, info->role);
...
#endif
    {
        if (info->role == BT_AWS_MCE_ROLE_PARTNER) {
            ...
            APPS_LOG_MSGID_I(APP_MUSIC_TAG" update_data_callback, recv role=0x%02X, isAutoPaused=%d, isPlaying=%d", 3,
                             info->role, isAutoPaused, s_app_music_context.music_playing);
            if (s_app_music_context.isAutoPaused != isAutoPaused) {
                APPS_LOG_MSGID_I(APP_MUSIC_TAG" update_data_callback rho status, isAutoPaused: old=%d, new=%d", 2,
                                 s_app_music_context.isAutoPaused, isAutoPaused);
                s_app_music_context.isAutoPaused = isAutoPaused;
            }

            if (s_app_music_context.music_playing != music_playing) {
                APPS_LOG_MSGID_I(APP_MUSIC_TAG" update_data_callback rho status, isPlaying: old=%d, new=%d", 2,
