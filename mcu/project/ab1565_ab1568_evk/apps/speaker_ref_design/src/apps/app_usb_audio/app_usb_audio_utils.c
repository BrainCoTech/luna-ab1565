...
void usb_audio_tx_ready()
{
    ...
    if (ctx->started) {
        ...
        if (s_voice_data_offset >= s_voice_data_len) {
            //USB_AUDIO_LOG_I("data read done", 0);
...
static void app_usb_am_callback(vendor_se_event_t event, void *arg)
{
    ui_shell_send_event(true, EVENT_PRIORITY_HIGNEST, EVENT_GROUP_SWITCH_USB_AUDIO,
                        APPS_EVENTS_INTERACTION_USB_AM,
                        (void*)event, 0, NULL, 0);
}

void app_usb_init_am() {
    int32_t se_id = 0;
    bt_sink_srv_am_result_t ret;

    se_id = ami_get_vendor_se_id();
    ret = ami_register_vendor_se(se_id, app_usb_am_callback);
    USB_AUDIO_LOG_I("am init: %d", 1, ret);
}
#endif
...
#if defined(AIR_USB_AUDIO_MIX_ENABLE) || defined(AIR_USB_AUDIO_OUT_ENABLE)
        case APPS_EVENTS_INTERACTION_USB_AM: {
