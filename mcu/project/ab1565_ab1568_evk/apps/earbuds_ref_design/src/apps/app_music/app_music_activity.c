...
static bool app_music_rotary_set_le_audio_volume(apps_config_key_action_t key_action, uint32_t rotary_data)
{
    ...
    APPS_LOG_MSGID_I(APP_MUSIC_ACTI" [LEA][BIS] app_music_rotary_set_le_audio_volume, action=0x%04X, is_bis_streaming=%d, volume=%d",
                     3, key_action, is_bis_streaming, volume);
    if (is_bis_streaming) {
        if (KEY_VOICE_UP == key_action) {
            volume += 1;
        ...
        ret = true;
        APPS_LOG_MSGID_I(APP_MUSIC_ACTI" [LEA][BIS] app_music_rotary_set_le_audio_volume set, volume=%d success=%d",
                         2, volume, success);
    }
#endif
