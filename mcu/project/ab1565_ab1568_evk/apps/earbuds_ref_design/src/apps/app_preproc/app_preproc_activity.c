...
const static uint8_t s_captouch_key_ids_array[] = APPS_CAPTOUCH_KEY_IDS;
...
static bool pre_proc_key_event_proc(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    if (extra_data) {
        ...
        *p_key_action = apps_config_key_event_remapper_map_action(key_id, key_event);
        ...
        if (0 == temp_touch_key_status) {
            for (int i = 0; i < (sizeof(s_captouch_key_ids_array)/sizeof(uint8_t)); i++) {
                if (key_id == s_captouch_key_ids_array[i]) {
                    APPS_LOG_MSGID_I("App_pre_pro touch key is disable: touch_key_status=%x, key_id=%x, key_event=%x",
                            3, temp_touch_key_status, key_id, key_event);
                    if (AIRO_KEY_PRESS == key_event) {
