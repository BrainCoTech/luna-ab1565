...
{
#if defined(MTK_3RD_PARTY_NR) && defined(HAL_DVFS_MODULE_ENABLED)
...
/**                                 Internal Function                              */
/**================================================================================*/
static bool app_advance_passthrough_check_condition(bool ignore_anc_pt)
{
    bool result = FALSE;
...
#endif
#ifdef MTK_3RD_PARTY_NR
    APPS_LOG_MSGID_I(LOG_TAG" check_condition, [%02X] anc_state=%d ld_ongoing=%d ull_lea=%d charger_in=%d ignore_anc_pt=%d",
                     6, role, anc_state, app_advance_pt_ctx.ld_ongoing, app_advance_pt_ctx.ull_lea_ongoing,
                     app_advance_pt_ctx.charger_in, ignore_anc_pt);
#else
    APPS_LOG_MSGID_I(LOG_TAG" check_condition, [%02X] anc_state=%d ld_ongoing=%d charger_in=%d ignore_anc_pt=%d",
...
#ifdef MTK_ANC_ENABLE
        && (ignore_anc_pt || anc_state != APP_ANC_STATE_PT_ENABLE)
#endif
#ifdef MTK_3RD_PARTY_NR
...
static bool app_advance_passthrough_resume(bool is_sync)
{
    if (app_advance_pt_ctx.enable) {
        ...
        return FALSE;
    } else if (!app_advance_passthrough_check_condition(app_advance_pt_ctx.need_off_hw_pt)) {
...
bool app_advance_passthrough_switch(void)
{
    ...
    if (enable) {
#ifdef MTK_ANC_ENABLE
        uint8_t anc_state = app_anc_server_get_state();
        if (anc_state == APP_ANC_STATE_PT_ENABLE) {
            if (app_advance_passthrough_check_condition(TRUE)) {
                bool ret = app_anc_service_disable();
                if (ret) {
                    APPS_LOG_MSGID_I(LOG_TAG" switch, HW PT ON but disable success", 0);
                    check_result = TRUE;
                } else {
                    APPS_LOG_MSGID_E(LOG_TAG" switch, HW PT ON and disable fail", 0);
                    return FALSE;
                }
            } else {
                APPS_LOG_MSGID_E(LOG_TAG" switch, HW PT ON and check other condition fail", 0);
                return FALSE;
            }
        } else {
            check_result = app_advance_passthrough_check_condition(FALSE);
        }
#else
        check_result = app_advance_passthrough_check_condition(FALSE);
#endif
        ...
        app_advance_passthrough_reset_resume();
    }
...
void app_advance_passthrough_reset_resume(void)
{
    APPS_LOG_MSGID_I(LOG_TAG" switch, reset need_resume %d", 1, app_advance_pt_ctx.need_resume);
    if (app_advance_pt_ctx.need_resume) {
        app_advance_pt_ctx.need_resume = FALSE;
