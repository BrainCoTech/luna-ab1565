...
extern bool g_le_audio_aird_device_busy_flag;

/**************************************************************************************************
* Prototype
**************************************************************************************************/
extern bt_status_t app_dhss_sdp_init(void);

extern bt_status_t app_dhss_get_paired_edr_addr(uint8_t *dst);
...
static void app_le_audio_send_battery_level_notification(void)
{
    ...
    while (i) {
        i--;
        if (BT_HANDLE_INVALID != g_le_audio_ctrl[i].handle) {
            app_mps_send_battery_level_notification(g_le_audio_ctrl[i].handle);
        }
    }
}
...
#if 0
static uint32_t app_le_audio_get_dhss_adv_data(multi_ble_adv_info_t *adv_data)
{
    //LE_AUDIO_MSGLOG_I("[APP] get_dhss_adv_data", 0);
    ...
    if ((NULL != adv_data->adv_data) && (NULL != adv_data->adv_data->data)) {
        ...
        adv_data->adv_data->data[len] = 0x11;
        adv_data->adv_data->data[len + 1] = BT_GAP_LE_AD_TYPE_MANUFACTURER_SPECIFIC;
        adv_data->adv_data->data[len + 2] = 0x46;
        adv_data->adv_data->data[len + 3] = 0x00;
        len += 4;

        if (BT_STATUS_SUCCESS == app_dhss_get_paired_le_addr(&le_addr)) {
            /* EDR Addr: AddrType + Addr */
            adv_data->adv_data->data[len] = 0;
            app_dhss_get_paired_edr_addr((uint8_t *)&adv_data->adv_data->data[len + 1]);

            /* LE Addr: AddrType + Addr */
            memcpy((uint8_t *)&adv_data->adv_data->data[len + 7], &le_addr, sizeof(bt_addr_t));

        } else {
            /* Not paired*/
            memset(&adv_data->adv_data->data[len], 0, 14);
        }
    ...
    if (NULL != adv_data->adv_param) {
        ...
        adv_data->adv_param->advertising_tx_power = 0;
...
static bool app_le_audio_get_curr_esco_state(void)
{
    bool  ret = false;
    uint8_t i = 0;

    bool                       temp_stata         = false;
    bt_sink_srv_device_state_t temp_state_list[3] = {0};
    bt_sink_srv_get_device_state(NULL, temp_state_list, 3);
    for (i=0; i<3; i++) {
        if (temp_state_list[i].sco_state == BT_SINK_SRV_SCO_CONNECTION_STATE_CONNECTED) {
            ret = true;
            break;
        }
    }
    ...
    switch (event_id) {
        case BT_CM_EVENT_REMOTE_INFO_UPDATE: {
            ...
            if ((BT_CM_PROFILE_SERVICE_MASK(BT_CM_PROFILE_SERVICE_HFP) & remote_update->pre_connected_service)
                ...
                LE_AUDIO_MSGLOG_I(LOG_TAG"[LEA] [%02X] HFP service=0x%08X", 2, role, remote_update->connected_service);
            ...
            if (!(BT_CM_PROFILE_SERVICE_MASK(BT_CM_PROFILE_SERVICE_AWS) & remote_update->pre_connected_service) &&
                ...
                LE_AUDIO_MSGLOG_I(LOG_TAG"[LEA][%02X] AWS Connected %x", 2, role, remote_update->reason);
                ...
                if (BT_SINK_SRV_STATE_STREAMING > bt_sink_srv_get_state()
                    || !app_le_audio_get_curr_esco_state()) {
                    s_le_audio_adv_interval_min = APP_LE_AUDIO_ADV_INTERVAL_MIN_S;
                    s_le_audio_adv_interval_max = APP_LE_AUDIO_ADV_INTERVAL_MAX_S;
                ...
                bt_status_t bt_status = apps_aws_sync_event_send_extra(EVENT_GROUP_UI_SHELL_LE_AUDIO,
                                                                       ...
                                                                       sizeof(uint8_t));
                APPS_LOG_MSGID_I(LOG_TAG"[LEA] AWS Connected, sync_cis_connected=%d bt_status=0x%08X",
                ...
                && !(BT_CM_PROFILE_SERVICE_MASK(BT_CM_PROFILE_SERVICE_AWS) & remote_update->connected_service)) {
                LE_AUDIO_MSGLOG_I(LOG_TAG"[LEA][%02X] AWS Disconnected %x", 2, role, remote_update->reason);
                /* disconnection reason
                    BT_HCI_STATUS_ROLE_SWITCH_PENDING: SP disconnect agent
                    BT_CM_AWS_LINK_DISCONNECT_REASON_MASK | BT_CM_AWS_LINK_DISCONNECT_BY_SWITCH_LINK  (0xE1): switch another aws link
                    BT_CM_AWS_LINK_DISCONNECT_REASON_MASK | BT_CM_AWS_LINK_DISCONNECT_NORMAL  (0xE2): agent diconnect partner
                */
                ...
                if (s_le_audio_adv_interval_min != APP_LE_AUDIO_ADV_INTERVAL_MIN_L
                    || s_le_audio_adv_interval_max != APP_LE_AUDIO_ADV_INTERVAL_MAX_L) {
                    s_le_audio_adv_interval_min = APP_LE_AUDIO_ADV_INTERVAL_MIN_L;
                    s_le_audio_adv_interval_max = APP_LE_AUDIO_ADV_INTERVAL_MAX_L;
                    app_le_audio_do_update_advertising_param();
            ...
            size_t data_len)
{
    ...
    switch (event_id) {
        case BT_POWER_ON_CNF: {
            ...
            g_le_audio_aird_device_busy_flag = false;
...
{
    /*if (g_le_audio_ctrl.handle != BT_HANDLE_INVALID) {
        app_le_audio_disconnect_by_handle(g_le_audio_ctrl.handle, reason);
    }*/
    for (uint8_t idx = 0; idx < APP_LE_AUDIO_MAX_LINK_NUM; idx++) {
        if (BT_HANDLE_INVALID != g_le_audio_ctrl[idx].handle) {
