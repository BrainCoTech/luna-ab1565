...
typedef struct {
    ...
    bool voice_stopping;                     /**<  Flag to indicate current playing voice should be stopped soon, so that other VP can be set. */
#if 0
...
volatile uint32_t g_gpt_count_stop = 0;                 /* Target gpt count to stop the current VP. */
...
/*
    ...
    if (tone_size != 0 && tone_buf != NULL) {
        ...
        if (g_gpt_count_play == 0 && cur_node->data.delay_time && cur_node->data.repeat) {
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &gpt_count);
            gpt_count += cur_node->data.delay_time * 1000;
            app_voice_prompt_msgid_report("play local loop with delay %d", 1, cur_node->data.delay_time);
            prompt_control_play_sync_tone(VPC_MP3, tone_buf, tone_size, gpt_count, app_voice_prompt_callback);
        } else {
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &gpt_count);
            if (gpt_count >= g_gpt_count_play) {
                app_voice_prompt_msgid_report("local play gpt(0x%08x) <= current(0x%08x)", 2, g_gpt_count_play, gpt_count);
                g_gpt_count_play = 0;
            }
        ...
        if (cur_queue == NULL || cur_node == NULL) {
...
static bool app_voice_prompt_local_stop(uint32_t id, uint32_t sync_time, app_vp_type type, bool preempted)
{
    ...
    if (type == VP_TYPE_VP) {
        if (item.id == id) {
            ...
            if (ret == VOICE_PROMPT_LIST_SUCCESS && item.played) {
...
static bool app_voice_prompt_sync_fail(uint32_t tone_idx, app_vp_play_callback_t callback)
{
    if (!callback) {
        return false;
    }
    ...
    app_voice_prompt_msgid_report("notify sync fail", 0);
    app_voice_prompt_noti(callback, tone_idx, VP_ERR_CODE_SYNC_FAIL);
    app_voice_prompt_play_next();
    return true;
}

static bool app_voice_prompt_sync_stop_fail()
{
    app_voice_prompt_list_data_t item = {0};
    app_voice_prompt_copy_item(app_voice_prompt_get_current_queue(), &item);
    ...
    if (!(item.callback)) {
        ...
        app_voice_prompt_msgid_report("notify sync stop fail", 0);
        app_voice_prompt_noti(item.callback, item.index, VP_ERR_CODE_SYNC_STOP_FAIL);
        return false;
    }
}
...
void app_voice_prompt_aws_play_impl(uint32_t tone_idx, uint32_t delay_time, app_vp_type type, app_vp_prio_t level, bool isCleanup, bool repeat, app_vp_play_callback_t callback)
{
    ...
    if (BT_AWS_MCE_SRV_LINK_NONE != bt_aws_mce_srv_get_link_type()) {
        ...
        if (bt_aws_mce_report_send_urgent_event(&info) != BT_STATUS_SUCCESS) {
            ...
            if (app_voice_prompt_sync_fail(tone_idx, callback)) {
                if (data) {
                    vPortFree(data);
                }
                vp_ctx.sync_immediately = false;
...
static bool app_voice_prompt_aws_stop(uint16_t id, uint32_t delay_time, app_vp_type type, bool preempted)
{
    ...
    if (BT_AWS_MCE_SRV_LINK_NONE != bt_aws_mce_srv_get_link_type()) {
        ...
        app_voice_prompt_msgid_report("voice promt stop id: 0x%x, type: 0x%x delay_time: %d, bt_clk 0x%08x 0x%08x", 5,
                                      id, type, delay_time, stop_clock.nclk, stop_clock.nclk_intra);

        if (bt_aws_mce_report_send_urgent_event(&info) != BT_STATUS_SUCCESS) {
            app_voice_prompt_msgid_report("sync stop fail", 0);
            if (app_voice_prompt_sync_stop_fail()) {
                ret = app_voice_prompt_local_stop(id, delay_time, type, preempted);
            }
        } else {
        /* Stop in local. */
            if (bt_sink_srv_convert_bt_clock_2_gpt_count((const bt_clock_t *)&stop_clock, (uint32_t *)&g_gpt_count_stop) == BT_STATUS_SUCCESS) {
                app_voice_prompt_msgid_report("voice prompt stop tar_gpt 0x%08x", 1, g_gpt_count_stop);
            } else {
                app_voice_prompt_msgid_report("voice prompt stop tar_gpt convert fail", 0);
                g_gpt_count_stop = 0;
            }
            ret = app_voice_prompt_local_stop(id, delay_time, type, preempted);
        }
...
void app_voice_prompt_synced_play(app_voice_prompt_synced_play_t *play_data)
{
    ...
    if (app_voice_prompt_get_current_index() == VP_INDEX_POWER_ON && play_data->play_index == VP_INDEX_INCOMING_CALL) {
        app_voice_prompt_msgid_report("app_voice_prompt_synced_play, skip a incoming call RT when playing POWER_ON", 0);
        return;
    }
...
void app_voice_prompt_sync_callback(bt_aws_mce_report_info_t *info)
{
    ...
    if (param[0] == APP_VOICE_PROMPT_SYNC_PLAY) {
        ...
        app_voice_prompt_msgid_report("[app_voice_prompt]  stop, id: 0x%x, type: 0x%x, bt_clk (0x%08x 0x%08x)", 4,
                sync_index.id, sync_index.type, sync_index.stop_time.nclk, sync_index.stop_time.nclk_intra);

        if (bt_sink_srv_convert_bt_clock_2_gpt_count((const bt_clock_t *)&(sync_index.stop_time), (uint32_t *)&g_gpt_count_stop) == BT_STATUS_SUCCESS) {
            app_voice_prompt_msgid_report("[app_voice_prompt] sync callback, target gpt 0x%08x", 1, g_gpt_count_stop);
        } else {
            app_voice_prompt_msgid_report("[app_voice_prompt] sync callback, target gpt convert fail", 0);
            g_gpt_count_stop = 0;
        }
...
static void app_voice_prompt_stop_and_play_next()
{
    ...
    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &gpt_count);
    app_voice_prompt_msgid_report("local stop gpt(0x%08x), current(0x%08x)", 2, g_gpt_count_stop, gpt_count);
    if (gpt_count >= g_gpt_count_stop) {
        g_gpt_count_play = 0;
    }

    if (g_gpt_count_play != 0) {
        cap.sync_scenario_type = MCU2DSP_SYNC_REQUEST_VP;
        cap.sync_action_type = MCU2DSP_SYNC_REQUEST_STOP;
        cap.target_gpt_cnt = g_gpt_count_stop;
...
extern uint8_t g_app_config_set_vp;
...
app_vp_type app_voice_prompt_get_current_type_unsafe()
{
    app_voice_prompt_list_data_t item = {0};
    app_voice_prompt_copy_item(app_voice_prompt_get_current_queue(), &item);
...
void app_voice_prompt_set_voice_stopping(bool stop)
{
    app_voice_prompt_msgid_report("app_voice_prompt_set_voice_stopping %d", 1, stop);
    vp_ctx.voice_stopping = stop;
}


bool app_voice_prompt_get_voice_stopping()
{
    return vp_ctx.voice_stopping;
}
...
void app_voice_prompt_remote_disconnect_impl()
{
    ...
    if (vp_ctx.bt_state == VP_BT_STATE_SNIFF_EXITING) {
        ...
        vp_queue *queue = app_voice_prompt_get_current_queue();
        app_voice_prompt_copy_item(queue, (DataType *)&item);
        app_voice_prompt_msgid_report("app_voice_prompt_remote_disconnect_impl: curr item idx %d, callback %x",
                2, item.index, (item.callback == NULL) ? 0 : item.callback);
        vp_ctx.bt_state = VP_BT_STATE_SNIFF;
        if (!app_voice_prompt_sync_fail(item.index, item.callback)) {
            app_voice_prompt_play_next();
        }
    }
    app_vp_mutex_unlock();
