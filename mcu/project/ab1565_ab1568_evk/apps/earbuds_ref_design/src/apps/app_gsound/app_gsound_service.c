...
bool gsound_port_is_in_charging()
{
    #ifdef MTK_SMART_CHARGER_ENABLE
    bool in_charger = (app_smcharger_is_charging() == APP_SMCHARGER_IN);
    GSOUND_LOG_I(TAG"gsound_port_is_in_charging ret=%d", 1, in_charger);
    return in_charger;
    #else
    int32_t charger_exist = battery_management_get_battery_property(BATTERY_PROPERTY_CHARGER_EXIST);
    GSOUND_LOG_I(TAG"gsound_port_is_in_charging charger_exist=%d", 1, charger_exist);
    return charger_exist == 1;
    #endif
}
