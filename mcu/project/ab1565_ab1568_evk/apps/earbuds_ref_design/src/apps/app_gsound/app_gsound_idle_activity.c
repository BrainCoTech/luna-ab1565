...
bool app_gsound_idle_activity_proc_music_event(bt_sink_srv_action_t sink_action)
{
    ...
    if(!gsound_connected && app_le_audio_is_connected()) {
        GSOUND_LOG_I(TAG"LE-Audio case and gsound not connected, return false", 0);
        return ret;
    }
    #endif
    ...
    size_t data_len)
{
    ...
    switch (event_group) {
        ...
        case EVENT_GROUP_UI_SHELL_KEY: {
            #ifdef AIR_LE_AUDIO_ENABLE
            if(!gsound_connected && app_le_audio_is_connected()) {
                GSOUND_LOG_I(TAG"LE-Audio case and gsound not connected", 0);
                return false;
            }
            #endif
