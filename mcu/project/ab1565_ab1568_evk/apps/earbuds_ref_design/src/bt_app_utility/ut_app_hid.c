...
bt_status_t bt_hid_notify_data(uint8_t index, uint8_t count)
{
    ...
    for (j = 0; j < count; j++) {
        ...
        printf("bt_hid_send_data: handle :%x", hid_cntx[0].handle);
        status = bt_hid_send_data(hid_cntx[0].handle, &value);
    ...
    void *buff)
{
    ...
    switch (msg) {
        case BT_HID_CONNECT_IND: {
            ...
            APPS_LOG_I("CONNECT_CNF: status = %d", 1, status);
            printf("connect_ind: handle :%x", ind->handle);
            break;
        }
        case BT_HID_CONNECT_CNF: {
            ...
            printf("conn_cnf: handle : %x", hid_cntx[0].handle);
            is_init = 0;
            break;
        }
...
void bt_hid_app_role_handover_status_callback(const bt_bd_addr_t *addr, bt_aws_mce_role_t role, bt_role_handover_event_t event, bt_status_t status)
{
    printf("hid_app_role_handover_status_callback: event :%x, role : %x", event, role);
    if (event == BT_ROLE_HANDOVER_COMPLETE_IND) {
        if (role == BT_AWS_MCE_ROLE_PARTNER) {
            printf("addr0:%x, addr1:%x, addr5:%x", *addr[0], *addr[1], *addr[5]);
            hid_cntx[0].handle = bt_hid_get_handle_by_addr(addr);
            printf( "bt_hid_app_role_handover_status_callback : handle = %x", hid_cntx[0].handle);
        }
    }
}


void bt_hid_app_init_rho()
{
    bt_role_handover_callbacks_t callbacks = {0};
    callbacks.status_cb = bt_hid_app_role_handover_status_callback;
    bt_role_handover_register_callbacks(BT_ROLE_HANDOVER_MODULE_HID_APP, &callbacks);
}
#endif
