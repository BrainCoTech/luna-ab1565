...
static atci_cmd_hdlr_item_t bt_app_comm_at_cmd[] = {
    ...
    ,
    {
        .command_head = "AT+HID",     /**< AT command string. */
        .hash_value1 = 0,
        .hash_value2 = 0,
        .command_hdlr = bt_hid_io_callback,
    }
#endif
#ifdef AIR_LE_AUDIO_ENABLE
    ,
...
static atci_status_t bt_app_comm_at_cmd_rho_hdl(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_ACTIVE: {
            ...
            extern bool g_xiaoai_ota_ongoing;
            if (g_xiaoai_ota_ongoing) {
                LOG_MSGID_I(BT_APP, "[XIAOAI] RHO fail due to ota_ongoing", 0);
                break;
            }
#endif
...
extern void ble_pacs_send_notify_after_connected(bt_handle_t handle);
...
static atci_status_t bt_app_comm_at_cmd_le_audio_hdl(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    if (parse_cmd->mode == ATCI_CMD_MODE_EXECUTION) {
		...
		if(0 == memcmp(pChar, "STARTADV", 8))
		{
            /* AT+LEAUDIO=STARTADV */
			
			app_le_audio_start_advertising(20000);
			ret_at = ATCI_RESPONSE_FLAG_APPEND_OK;
		}
            ...
            /* PACS operation*/
            /* AT+LEAUDIO=PACS,<ACTION1> */
            pChar = strchr(pChar, ',');
            pChar++;

            if (0 == memcmp(pChar, "NOTIFY", 6))
            {
                bt_handle_t handle = bt_sink_srv_cap_get_link_handle(0);

                ble_pacs_set_available_audio_contexts(AUDIO_CONTENT_TYPE_VA, AUDIO_CONTENT_TYPE_VA);
                ble_pacs_send_notify_after_connected(handle);
            }
            else
            {
                response->response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
...
#ifdef MTK_BT_HID_ENABLE
    extern void bt_hid_app_init_rho();
    bt_hid_app_init_rho();
#endif
