...
/**
 *  @brief This enum defines the extra data of APPS_EVENTS_INTERACTION_PLAY_VP.
 */
typedef struct{
    app_vp_type type;     /**<  The VP type. */
    uint32_t index;       /**<  The VP index. */
    bool need_sync;       /**<  Whether the VP need sync. */
    bool need_repeat;     /**<  Whether the VP need loop. */
    uint32_t delay_time;  /**<  Sync delay time, only matter when need_sync is true. */
    app_vp_prio_t level;  /**<  The VP priority. */
} apps_config_vp_t;
...
/**
* @brief      This function is for partner to set a VP on agent, especially for partner to set a sync VP.
* @param[in]  vp_index, the VP index which requires playing, refer to apps_config_vp_index_list.h.
* @param[in]  need_sync, true means that the VP needs to be synchronized to partner.
* @param[in]  sync_delay_time, after the time, agent and partner play the VP at the same time.
* @param[in]  level, the priority of the VP.
* @param[in]  is_ringtone, true if set ringtone, false if set VP.
* @param[in]  repeat, only use when set ringtone.
* @return     If the operation is completed successfully, the return value is true.
*/
...
/**
* @brief      This function is to set a VP with a target GPT count to start play.
* @param[in]  vp_index, the VP index which requires playing, refer to apps_config_vp_index_list.h.
* @param[in]  gpt_count, the target GPT count to start play. If the gpt_count is lower then current GPT, the VP will be played immediately.
* @param[in]  level, the priority of the VP.
...
/**
* @brief      This function is to set a VP witch can preempt the same level previous VP.
* @param[in]  vp_index, the VP index which requires playing, refer to apps_config_vp_index_list.h.
* @param[in]  need_sync, true means that the VP needs to be synchronized to partner.
* @param[in]  sync_delay_time, after the time, agent and partner play the VP at the same time.
* @param[in]  level, the priority of the VP.
...
* @param[in]  vp_index is ringtone or voice broadcast index which requires playing, refer to apps_config_vp_index_list.h.
...
* @param[in]  sync_delay_time, after the time, agent and partner play the ringtone or voice broadcast at the same time.
