...
volatile uint32_t g_gpt_local_td = 0;
volatile uint32_t g_bt_local_td = 0;
volatile uint32_t g_last_local_timestamp = 0;
volatile uint32_t g_last_bt_timestamp = 0;
...
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ uint32_t mux_tx_vfifo(mux_ringbuffer_t *tx_vfifo, mux_ringbuffer_t *tx_ufifo, mux_ll_user_config_t *user_config)
{
    ...
    RB_LOG_I("[mux_tx_vfifo] MUX_ID=0x%x, scid=%d dcid=%d uid=%d", 4, MUX_ID(user_config->scid, user_config->dcid, uattr->uid), user_config->scid, user_config->dcid, uattr->uid);
#endif
    ...
    if (real_pkt_len == 0) {
        // RB_LOG_W("[mux_tx_vfifo] uid=%d, data or vfifo not enough!!! ud_len=%d vd_len=%d", 3, uattr->uid, user_buffer_data_len, mux_ringbuffer_data_length(tx_vfifo));
        return 0;
    }

    pheader->head = MUX_LL_UART_PKT_HEAD;
    pheader->id = MUX_ID(user_config->scid, user_config->dcid, uattr->uid);
...
#ifdef MUX_LL_UART_TIMESTAMP_ENABLE
    g_last_local_timestamp = user_context->tx_local_timestamp;
    g_last_bt_timestamp = tx_timestamp;
#endif
...
#endif
    RB_LOG_I("[mux_tx_vfifo] uid[%x], seq=%3u, hw_wptr=%u, MCR=0x%x, rs_len=%3u, pkt_len=%4u, ud_len=%3u, tx_vd_len=%u:%u, rx_vd_len=%u, uwm=%u, vwm=%u, bt_ts=%u, gpt_ts=%u, tx_td=%4u gpt_lc_td=%4u, bt_lc_td=%4u", 17,\
        ...
        mux_ringbuffer_data_length(&g_port_config.rxbuf), uwm, vwm, tx_timestamp, user_context->tx_local_timestamp, mux_get_tick_elapse(user_context->tx_local_timestamp), g_gpt_local_td, g_bt_local_td);
...
#ifdef MUX_LL_UART_TIMESTAMP_ENABLE
            user_context = &g_mux_ll_user_context[uattr->uid];
            user_context->tx_timestamp = DCHS_bt_get_native_clock_us();
            user_context->tx_local_timestamp = mux_get_tick_count();
            g_gpt_local_td = mux_get_tick_elapse_with_end(g_last_local_timestamp, user_context->tx_local_timestamp);
            g_bt_local_td = DCHS_bt_get_native_clock_us_duration(g_last_bt_timestamp, user_context->tx_timestamp);
            if ((g_last_local_timestamp != 0) && (g_gpt_local_td != g_bt_local_td)) {
                user_context->tx_timestamp = g_last_local_timestamp + g_gpt_local_td;
            }
#endif
...
ATTR_TEXT_IN_FAST_MEM static bool mux_ll_uart_parse_data_and_notify_mcu_user_ready_to_read(mux_ll_user_config_t * user_config, uint32_t write_done_len)
{
    ...
    if (user_config->uattr.flags & MUX_LL_UART_ATTR_USER_TRX_PROTOCOL) {
        ...
        if (user_config->callback != NULL) {
            user_config->callback(user_id_to_handle(rx_ufifo->uid, port), MUX_EVENT_READY_TO_READ, RB_TOTAL_DATA_SIZE(rx_ufifo), user_config->user_data);
        } else {
            RB_LOG_E("[mux_rx_deliver] uid=%u new_pkt_len:%u callback not ready!", 2, rx_ufifo->uid, new_pkt_len);
        }
    }
...
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ void mux_rx_deliver(void)
{
    ...
    mux_ll_user_context_t *user_context;
    static uint32_t last_rx_end_gpt_ts = 0;
    static uint32_t last_rx_end_local_bt_ts = 0;
    static uint32_t pkt_rx_duration_max = 0;
    ...
    uint32_t rx_start_local_gpt_ts = 0;
    uint32_t rx_end_local_gpt_ts = 0;
    uint32_t rx_end_local_bt_ts = 0;
    uint32_t gpt_local_td = 0;
    uint32_t bt_local_td = 0;
    ...
    while(1) {
        rx_start_local_gpt_ts = mux_get_tick_count();
        ...
        new_pkt_len = peek_buffer_data_and_parse_first_package(rx_vfifo, &consume_len, total_rx_left_data_size, &duid, &dcid);
        ...
        /* FIXME: If user do not deal with all total_rx_left_data_size data at the following procedure,
           the data leng in rx_vfifo must be wrong, data length will larger than real data length.
        */
...
#ifdef MUX_LL_UART_CRC_CHECK_ENABLE
        memcpy(&vfifo_rxrb_tmp, rx_vfifo, sizeof(mux_ringbuffer_t));
#endif
        ...
        if(new_pkt_len > mux_ringbuffer_free_space(rx_ufifo)){
            ...
            // mux_ringbuffer_read_move_ht_st(rx_vfifo, new_pkt_len);
            // mux_ringbuffer_read_move_hw_tail_ht_st(rx_vfifo, new_pkt_len);
        } else {
            // receive new_pkt_len bytes data in vfifo, move SW wptr
            // mux_ringbuffer_write_move_ht_st(rx_vfifo, new_pkt_len);
...
#ifdef MUX_LL_UART_TIMESTAMP_ENABLE
        rx_end_local_bt_ts = DCHS_bt_get_native_clock_us();
        rx_end_local_gpt_ts = mux_get_tick_count();

        bt_local_td = DCHS_bt_get_native_clock_us_duration(last_rx_end_local_bt_ts, rx_end_local_bt_ts);
        gpt_local_td = mux_get_tick_elapse_with_end(last_rx_end_gpt_ts, rx_end_local_gpt_ts);
        if ((last_rx_end_local_bt_ts != 0) && (gpt_local_td != bt_local_td)) {
            rx_end_local_bt_ts = last_rx_end_local_bt_ts + gpt_local_td;
        }
        tx_start_bt_ts = user_context->rx_timestamp;
        rx_end_bt_ts = (uint32_t)((int)rx_end_local_bt_ts - (int)g_latch_phase_clock_diff);
        pkt_rx_duration = DCHS_bt_get_native_clock_us_duration(tx_start_bt_ts, rx_end_bt_ts);
        if (((int)pkt_rx_duration > 0) && (g_latch_phase_clock_diff != 0)) {
            pkt_rx_duration_max = RB_MAX(pkt_rx_duration, pkt_rx_duration_max);
#ifdef AIR_DCHS_MODE_SLAVE_ENABLE
        ...
        last_rx_end_gpt_ts = rx_end_local_gpt_ts;
        last_rx_end_local_bt_ts = rx_end_local_bt_ts;
#endif
#ifdef MUX_LL_UART_SEQ_ENABLE
...
#endif
        RB_LOG_I("[mux_rx_deliver] uid[%02x] %4uB rx done, rdy_to_rd=%u, ud_len=%u, peek=%u, seq=%3u, uwm=%5u, vwm=%4u, tx_vd_len=%4u, rx_vd_len=%4u, gpt_ts=%u, start_bt_ts=%u, end_bt_ts=%u:%u, bt_td=%4d:%4u, rx_td=%4u, co=%d, gpt_lc_td=%4u, bt_lc_td=%4u", 20, \
        duid, write_done_len, g_mux_uart_ready_to_read_count, mux_ringbuffer_data_length(rx_ufifo), g_mux_uart_peek_count, rx_pkt_current_seq, uwm, vwm, mux_ringbuffer_data_length(&g_port_config.txbuf), mux_ringbuffer_data_length(rx_vfifo), rx_end_local_gpt_ts, \
        tx_start_bt_ts, rx_end_bt_ts, rx_end_local_bt_ts, (int)pkt_rx_duration , pkt_rx_duration_max, mux_get_tick_elapse(rx_start_local_gpt_ts), (int)g_latch_phase_clock_diff, gpt_local_td, bt_local_td);
...
ATTR_TEXT_IN_FAST_MEM void mux_ll_uart_format_header(mux_ll_header_t *header, uint8_t uid, uint32_t pkt_len)
{
    ...
    header->id = MUX_ID(0, 0, uid);
    header->length = pkt_len;

#ifdef MUX_LL_UART_TIMESTAMP_ENABLE
    header->timestamp = DCHS_bt_get_native_clock_us();
    user_context->tx_timestamp = header->timestamp;
    user_context->tx_local_timestamp = mux_get_tick_count();
#endif
...
#ifdef MUX_LL_UART_RDWR_PTR_CHECK_ENABLE
    if ((sw_wptr & 0x0FFFFFFF) != hw_wptr) {
        RB_LOG_E("[send vfifo] sw_wptr=0x%x hw_wptr=0x%x", 2, sw_wptr, hw_wptr);
...
ATTR_TEXT_IN_FAST_MEM mux_status_t mux_rx_ll(mux_handle_t handle, mux_buffer_t *buffer, uint32_t *receive_done_data_len)
{
    ...
    LOG_MSGID_I(common, "[mux_rx_ll] [%02x] expect size=%d real size=%d, ud_len=%d vd_len=%d acc_cnt=%u damount=%u", 7, rxbuf->uid, buffer->buf_size, *receive_done_data_len,\
        mux_ringbuffer_data_length(rxbuf), mux_ringbuffer_data_length(&g_port_config.rxbuf), rxbuf->access_count, rxbuf->data_amount);
    return MUX_STATUS_OK;
}
...
ATTR_TEXT_IN_FAST_MEM mux_status_t mux_tx_ll(mux_handle_t handle, const mux_buffer_t buffers[], uint32_t buffers_counter, uint32_t *send_done_data_len)
{
    ...
    *send_done_data_len = send_len;
    LOG_MSGID_I(common, "[mux_tx_ll] uid=%02x send_len=%d ud_len=%d vd_len=%d MCR=0x%x acc_cnt=%u damount=%u", 7, txbuf->uid, send_len, mux_ringbuffer_data_length(txbuf), mux_ringbuffer_data_length(&g_port_config.txbuf), g_uart_regbase[g_port_config.txbuf.port_idx]->MCR_UNION.MCR, txbuf->access_count, txbuf->data_amount);
