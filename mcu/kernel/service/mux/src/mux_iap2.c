...
mux_status_t mux_iap2_normal_init(uint8_t port_index, mux_port_config_t *p_setting, mux_irq_handler_t irq_handler)
{
    ...
    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2]normal_init, port_index:0x%x p_setting:0x%x ", 2, port_index, p_setting);
...
static void mux_iap2_reset_context(mux_iap2_context_t *context)
{
    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2]reset context:0x%x", 1, context);
    ...
    while ((context->tx_head_packet != NULL) &&
        ...
        if (context->read_write_point.tx_buff_len != 0) {
            ...
            if (context->read_write_point.tx_buff_read_point + send_packet->data_length <= context->read_write_point.tx_buff_end) {
                ...
                if (full_data == NULL) {
                    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2]alloc full data FAILED!", 0);
                    break;
                }
...
static void mux_iap2_context_callback(mux_iap2_context_t *context, mux_event_t event, void *parameter)
{
    ...
    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2]context_callback, context:0x%x event:0x%x", 2, context, event);
    ...
    if ((port >= MUX_IAP2_BEGIN) && (port <= MUX_IAP2_END) && (context->handler != NULL)) {
        ...
        LOG_MSGID_E(MUX_IAP2, "[MUX][iAP2]context_callback, invalid context or port not opend!", 0);
    }
}
...
static void mux_iap2_callback(iap2_event_t event, void *parameter)
{
    ...
    switch (event) {
...
#endif
                    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2]reach max session number, session_id:%x", 1, ea_session->session_id);
                }
            }
            break;
        }
        ...
        case IAP2_RECIEVED_DATA_IND: {
            ...
            LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2]IAP2_RECIEVED_DATA_IND, session_type:0x%x", 1, received_data->session_type);
...
{
    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2][RHO]update, role:0x%x", 1, role);
    switch (role) {
...
bt_status_t mux_iap2_role_handover_update(bt_role_handover_update_info_t *info)
{
    ...
    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2][RHO]update, role:0x%x", 1, info->role);
...
void mux_iap2_role_handover_status_callback(const bt_bd_addr_t *address, bt_aws_mce_role_t role, bt_role_handover_event_t event,
                                            bt_status_t status)
{
    LOG_MSGID_I(MUX_IAP2, "[MUX][iAP2][RHO]status_callback, role:0x%x event:0x%x", 2, role, event);
    switch (event) {
