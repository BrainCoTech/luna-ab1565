...
/* TCM is mainly for Optimize log printing time */
ATTR_TEXT_IN_TCM uint32_t syslog_port_query_exception_number(void)
{
    return HAL_NVIC_QUERY_EXCEPTION_NUMBER;
}

/* TCM is mainly for Optimize log printing time */
ATTR_TEXT_IN_TCM uint32_t syslog_port_query_os_task_number(void)
{
#ifdef FREERTOS_ENABLE
...
uint32_t syslog_port_init(uint32_t port)
{
    ...
    if ((syslog_port == MUX_USB_COM_1) || (syslog_port == MUX_USB_COM_2)) {
        ...
        debug_printf("usb debug port_0 init");
        ...
        debug_printf("1 wire debug port init done");
    }
