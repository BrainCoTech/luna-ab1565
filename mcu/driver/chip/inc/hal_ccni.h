 ...
 *      (A) Check the event usage status in \\mcu\\driver\\chip\\*\\inc\\hal_ccni_config.h. The event in default status can be used.\n
 ...
 *
 *  - Step 3: Replace the event name in \\mcu\\driver\\chip\\*\\inc\\hal_ccni_config.h.\n
 ...
 *        if( pMsg != NULL) {
 *            uint32_t address;
 *            //Suppose MCU received an EVENT from DSP0 with sharing memory and the memory address is in pMsg[0]. \n
 ...
 *    @endcode
 *
 * - Step 5: Register the isr handler to ccni_*_to_*_function_table[] in \\mcu\\driver\\chip\\*\\inc\\hal_ccni_config.h. \n
 *
 ...
 * - Sample code:
 *    @code
 *       //MCU send an event IRQGEN_CM4_TO_DSP0_EVENT9 to DSP0.
 *       uint32_t status;
 ...
 *    @code
 *       //MCU send an event CCNI_CM4_TO_DSP0_EVENT1 to DSP0 with const value in message.
 *       uint32_t status;
         //MCU send event CCNI_CM4_TO_DSP0_EVENT1 to DSP0
 ...
 *       }
 *
 *       //MCU send an event CCNI_CM4_TO_DSP0_EVENT2 to DSP0 and notify send 128 Bytes data with sharing memory.
 *       uint32_t status;
 *       uint32_t sharing_memory_address;
 *       //MCU send event CCNI_CM4_TO_DSP0_EVENT2 to DSP0
 ...
 *       //Please mind the receiver should call hal_memview_dsp0_to_infrasys(msg_array[0]) to remap the address, otherwise the receiver can not get the right data.
 *       msg_array[1] = 0x80;
 *       //the second parameter should not be NULL.
 *       ret = hal_ccni_set_event(event, msg_array);
 *       if (HAL_CCNI_STATUS_OK != ret) {
