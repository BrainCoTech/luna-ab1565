...
ATTR_TEXT_IN_TCM int32_t SF_DAL_CheckReadyAndResume(void *MTDData, uint32_t addr, uint8_t data)
{
    ...
    // Read device status
    savedMask = SaveAndSetIRQMask();
    if(sfc_interrupted_by_ch2_or_ch3 == true) {
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &dsp_wait_us_end);
        hal_gpt_get_duration_count(dsp_wait_us_start, dsp_wait_us_end, &dsp_wait_us_duration);
        if (dsp_wait_us_duration >= g_flash_pingpang_flag) {
            /*
                       suppose DSP XIP has high priority. CM33 will unmask DSP and wait g_flash_pingpang_flag when recieved DSP XIP interrupt;
                       duration from unmask channel to now is over g_flash_pingpang_flag.
                    */
...
ATTR_TEXT_IN_TCM int32_t SF_DAL_CheckDeviceReady(void *MTDData, uint32_t BlockIndex)
{
    ...
    if(sfc_interrupted_by_ch2_or_ch3 == true) {
        ...
        if (dsp_wait_us_duration >= g_flash_pingpang_flag) {
            /*
             suppose DSP XIP has high priority. MCU will unmask DSP and wait g_flash_pingpang_flag when recieved DSP request;
             duration from unmask channel to now is over g_flash_pingpang_flag.
            */
            sfc_interrupted_by_ch2_or_ch3 = false;
            dsp_wait_us_start = 0;
