...
/*
 below option is for resolve isink timing limitation:
 read ISINK RG need delay 2T@32K after write
*/

#define ISINK_ACCESS_TIMING
...
#ifdef ISINK_ACCESS_TIMING
static  uint32_t s_tick_wr = 0;
#endif
 bool    led_write_register(uint16_t address, uint16_t value)
{
    pmu_operate_status_t status;

    status = pmu_set_register_value_lp(address, 0xFFFF, 0, value);
#ifdef ISINK_ACCESS_TIMING
    if(address == PMU_LED_EN_CTRL) {
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_32K, &s_tick_wr);
    }
#endif
    if(status != PMU_OK){
  ...
  uint32_t    led_read_register(uint16_t address)
{
    uint32_t  temp = 0;
#ifdef ISINK_ACCESS_TIMING
    uint32_t tick_dur = 0;
    ...
    if (address == PMU_LED_EN_CTRL) {
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_32K, &temp);
        hal_gpt_get_duration_count(s_tick_wr, temp, &tick_dur);
        if (tick_dur < 3) {
            hal_gpt_delay_us(30 * (3-tick_dur));
        }
    }
#endif
...
hal_isink_status_t  hal_isink_set_step_current(hal_isink_channel_t channel, hal_isink_current_t current)
{
    ...
    if(channel >= HAL_ISINK_MAX_CHANNEL){
        //og_hal_msgid_info("[hal][isink] config fail, invalid param", 0);
