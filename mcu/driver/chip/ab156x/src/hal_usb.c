...
static void hal_dump_regs(void)
{
    ...
    log_hal_msgid_info("phy->u2phyac0              (0x%08X) : 0x%08x", 2,   &phy->u2phyac0,     phy->u2phyac0);
    log_hal_msgid_info("phy->u2phyac1              (0x%08X) : 0x%08x", 2,   &phy->u2phyac1,     phy->u2phyac1);
    log_hal_msgid_info("phy->u2phyac2              (0x%08X) : 0x%08x", 2,   &phy->u2phyac2,     phy->u2phyac2);
    log_hal_msgid_info("phy->u2phyacr0             (0x%08X) : 0x%08x", 2,   &phy->u2phyacr0,    phy->u2phyacr0);
    log_hal_msgid_info("phy->u2phyacr1             (0x%08X) : 0x%08x", 2,   &phy->u2phyacr1,    phy->u2phyacr1);
    log_hal_msgid_info("phy->u2phyacr2             (0x%08X) : 0x%08x", 2,   &phy->u2phyacr2,    phy->u2phyacr2);
...
hal_usb_status_t hal_usb_init_tx_endpoint(uint32_t ep_num, uint16_t data_size, hal_usb_endpoint_transfer_type_t type, bool double_fifo)
{
    ...
    if(data_size>=1024 ) log_hal_msgid_error("hal_usb_init_tx_endpoint data_size:%d", 1, data_size);
...
hal_usb_status_t hal_usb_init_rx_endpoint(uint32_t ep_num, uint16_t data_size, hal_usb_endpoint_transfer_type_t type, bool double_fifo)
{
    ...
    if(data_size>=1024 ) log_hal_msgid_error("hal_usb_init_rx_endpoint data_size:%d", 1, data_size);
