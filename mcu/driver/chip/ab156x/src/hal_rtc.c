...
static  bool                        g_rtc_initialed = false;
/***********************************************************************************************
...
static void     rtc_eint_irq_callback(void * user_data)
{
    ...
    log_rtc_info("rtc irq triggered,int status %x\r\n", 1, value);
...
hal_rtc_status_t hal_rtc_init(void)
{
    ...
    if (g_rtc_initialed == true) {
        log_rtc_info("[hal][rtc] had been initialized!!!", 0);
    ...
    if (config->op_state & RTC_STATE_POWER_LOST_FLG) {
        ...
        is_init_by_bl = (config->op_state & RTC_STATE_INIT_BY_BL_FLG)? true : false;
        rtc_internal_dump(NULL);
        log_rtc_warn("rtc 1st power on, init by bl? %d (0:flase,1:true)\r\n", 1, is_init_by_bl);
        if (is_init_by_bl == false) {
            rtc_internal_init_register();
        }
    } else {
        if (rtc_internal_is_powered_by_rtc() == true) {
            log_rtc_warn("system is back from rtc mode:int stat:%x", 1, config->irq_status);
            config->op_state |= RTC_STATE_RTC_WAKE_FLG;
            rtc_internal_print_power_reason(config->irq_status);
        } else {
            log_rtc_warn("rtc back from power exception:int stat:%x", 1, config->irq_status);
        }
    }
    rtc_internal_set_spar_rg(temp);
    /*switch clock source*/
