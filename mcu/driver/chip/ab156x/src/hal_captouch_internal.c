...
bool captouch_sw_auto_tune(hal_captouch_channel_t channel, hal_captouch_tune_data_t *tune_data)
{
    ...
    HAL_CAPTOUCH_LOG_PRINT("captouch_sw_auto_tune ch=%d,fine_cap=%d, crs=%d, vadc_dbg=%d",4 ,channel,find_cap[1],coare_index,vadc_dbg);
...
bool captouch_rtc_clk_control(bool is_en)
{
    ...
    if (rtc_ret != HAL_RTC_STATUS_OK) {
        //HAL_CAPTOUCH_LOG_PRINT("[Captouch] RTC init/deinit:%d fail!",1 ,is_en);
