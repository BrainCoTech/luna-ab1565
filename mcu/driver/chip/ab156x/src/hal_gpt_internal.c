...
hal_gpt_status_t sw_gpt_start_timer_common_port(uint32_t handle,sw_gpt_note_t *start_timer_info,uint32_t current_count,hal_gpt_port_t gpt_port);
...
{
    uint32_t current_count=0,expire_count,timeout_tick=0;
    hal_gpt_port_t gpt_port = HAL_GPT_MAX_PORT;
    sw_gpt_note_t  temp_para;
    ...
    temp_para.expire_count = expire_count;
    temp_para.callback = callback;
    temp_para.status = SW_GPT_NOTE_STATUS_OF_RUNING;
    temp_para.user_data =user_data;
    temp_para.timer_port_type  = sw_type;

    return sw_gpt_start_timer_common_port(handle,&temp_para,current_count,gpt_port);
}
...
hal_gpt_status_t sw_gpt_start_timer_for_absolute_tick(uint32_t handle , hal_sw_gpt_absolute_parameter_t *absolute_parameter,sw_gpt_type_t sw_type)
{
    ...
    temp_para.expire_count = p_temp->absolute_time_count;
    temp_para.callback = p_temp->callback;
    temp_para.status = SW_GPT_NOTE_STATUS_OF_RUNING;
    temp_para.user_data =p_temp->user_data;
    temp_para.timer_port_type  = sw_type;
...
ATTR_TEXT_IN_TCM hal_gpt_status_t sw_gpt_start_timer_common_port(uint32_t handle,sw_gpt_note_t *start_timer_info,uint32_t current_count,hal_gpt_port_t gpt_port)
{
    uint32_t mask;
    uint32_t index = SW_GPT_HANDLE_TO_INDEX(handle);
    uint32_t next_index,next_count;
    sw_gpt_note_t *ptemp=start_timer_info;
    ...
    g_sw_gpt_notes.sw_gpt_timer[index].status= ptemp->status;
    g_sw_gpt_notes.sw_gpt_timer[index].expire_count = ptemp->expire_count;
    g_sw_gpt_notes.sw_gpt_timer[index].callback = ptemp->callback;
    g_sw_gpt_notes.sw_gpt_timer[index].user_data = ptemp->user_data;
    g_sw_gpt_notes.sw_gpt_timer[index].timer_port_type = ptemp->timer_port_type;
    /* step3: find the next expire timer  and re-start the gpt*/
    sw_gpt_find_the_next_expire_time(current_count,&next_index,&next_count,ptemp->timer_port_type);
    ...
    if(next_index != g_sw_gpt_running_note[ptemp->timer_port_type]){
        g_sw_gpt_running_note[ptemp->timer_port_type] = next_index;
