...
{
    log_rtc_warn("rtc power by %d (1:1ST,2:ALM,3:TICK,4~6:EINT,7:CAP)\r\n", 1, rtc_internal_power_reason(irq_status));
}
...
uint32_t    rtc_internal_get_eosc32_calibration()
{
    ...
    return eosc_cali;
error:
    log_rtc_error("rtc frequency cali fail: low_freq %d(cali %d), high_freq %d(cali %d)", 4, (unsigned int) low_frequency, (unsigned int)low_xosccali, (unsigned int) high_frequency, (unsigned int)high_xosccali);
    return eosc_cali;
}
...
void    rtc_internal_dump(char *comment)
{
    ...
    log_rtc_info("BBPU(%x),LPD(%x),WRTGR(%x),RESET(%x),CON0(%x),CON1(%x),CON2(%x)", 7,
    ...
    log_rtc_info("GPIO0(%x),GPIO1(%x),GPIO2(%x),GPIO_CON(%x),EINT0(%x),EINT1(%x),CALI(%x)", 7,
    ...
    log_rtc_info("AL_MASK(%x),CII_EN(%x),AL0(%x),AL1(%x),AL2(%x),AL3(%x),CAP_CON(%x)", 7,
    ...
    log_rtc_info("SPR_RG(%x),SPR0(%x),SPR1(%x),SPR2(%x),SPR3(%x),SPR4(%x),SPR5(%x)", 7,
