...
void    wdt_dump_register(uint32_t channel)
{
    ...
    base_reg = (channel == 0) ? (uint32_t)(&(WDT_REGISTER->WDT_EN)) : (uint32_t)(&(WDT_REGISTER->WDT1_EN));
    base_reg = base_reg;
    log_hal_msgid_info("wdt channel %d dump:", 1, channel);
    log_hal_msgid_info("WDT_EN(%x),WDT_LEN(%x),WDT_INTV(%x),WDT_AUTO_RESTR(%x),WDT_PMU(%x)", 5,
                        *((volatile uint32_t *) base_reg),
                        *((volatile uint32_t *) base_reg + 0x4),
                        *((volatile uint32_t *) base_reg + 0x8),
                        *((volatile uint32_t *) base_reg + 0x14),
                        WDT_REGISTER->WDT_MASK1_UNION.WDT_MASK1
                    );
    log_hal_msgid_info("WDT_STA(%x),WDT_IE(%x),WDT_INT(%x),WDT_WK_EN(%x),WDT_WKUP(%x)", 5,
                        *((volatile uint32_t *) base_reg + 0x18),
                        *((volatile uint32_t *) base_reg + 0x1C),
                        *((volatile uint32_t *) base_reg + 0x20),
                        *((volatile uint32_t *) base_reg + 0x24),
                        *((volatile uint32_t *) base_reg + 0x28)
                    );
}
