...
ATTR_ZIDATA_IN_TCM uint32_t mask_irq_func_specific = 0;
#endif
...
ATTR_TEXT_IN_TCM hal_nvic_status_t hal_nvic_save_and_set_interrupt_mask(uint32_t *mask)
{
    ...
    *mask = __get_BASEPRI();
    ...
    /*1.*mask = 0,before call this api,irq is eanble
     *2.*mask = bt irq,someone disable irq but unmask bt
     */
    if (((*mask == 0) && (hal_core_status_read(HAL_CORE_MCU) == HAL_CORE_ACTIVE))
        || (*mask == (((BT_IRQ_PRIORITY + 1) << (8 - __NVIC_PRIO_BITS)) & 0xFF))) {
...
ATTR_TEXT_IN_TCM hal_nvic_status_t hal_nvic_save_and_set_interrupt_mask_special(uint32_t *mask)
{
    ...
    *mask = __get_BASEPRI();
	...
	if (hal_core_status_read(HAL_CORE_MCU) != HAL_CORE_EXCEPTION ){
		/* someone has set basepri as bt irq,you cannot set basepri lower than bt */
		if ((((BT_IRQ_PRIORITY + 1) << (8 - __NVIC_PRIO_BITS)) & 0xFF) == *mask){
			assert("BASEPRI IS BT IRQ" && 0);
		}
	}
...
ATTR_TEXT_IN_TCM hal_nvic_status_t hal_nvic_save_and_set_specific_interrupt_mask(uint32_t *mask)
{

    if (hal_core_status_read(HAL_CORE_MCU) == HAL_CORE_ACTIVE)
    {
    	/* 1.bt irq cannot call this api
         * 2.someone has set basepri 0x10 by using hal_nvic_save_and_set_interrupt_mask or hal_nvic_save_and_set_interrupt_mask_special
         */
    	if ((BT_IRQn == (HAL_NVIC_QUERY_EXCEPTION_NUMBER - 0x10) || (__get_BASEPRI() == ((1 << (8 - __NVIC_PRIO_BITS)) & 0xFF)))){
    		assert("BT IRQ || BASEPRI is 0x10" && 0);
    	}
    }
    ...
    uint32_t xLinkRegAddr = 0;
#endif
    uint32_t priority = BT_IRQ_PRIORITY + 1;

    *mask = __get_BASEPRI();

    //set base priority as BT_IRQ_PRIORITY + 1,bt irq can preempt it.
    __set_BASEPRI(((priority << (8 - __NVIC_PRIO_BITS)) & 0xFF));
    __DMB();
    __ISB();
    ...
    if ((*mask == 0) && (hal_core_status_read(HAL_CORE_MCU) == HAL_CORE_ACTIVE)) {
        mask_irq_func_specific = xLinkRegAddr;
        hal_time_check_disable_irq_start();
    }
#endif
    return HAL_NVIC_STATUS_OK;
...
ATTR_TEXT_IN_TCM hal_nvic_status_t hal_nvic_restore_specific_interrupt_mask(uint32_t mask)
{
	/* bt irq cannot call this api */
	if (BT_IRQn == (HAL_NVIC_QUERY_EXCEPTION_NUMBER - 0x10)){
		assert("current irq is bt" && 0);
	}
