...
hal_sd_status_t sd_read_write_blocks_dma_init(hal_sd_port_t sd_port, uint32_t read_write_buffer, uint32_t block_number, uint32_t *block_multiplier)
{
    ...
    msdc_register_base->DMA_SA = (uint32_t)read_write_buffer;

    return HAL_SD_STATUS_OK;
}

void sd_error_handle(hal_sd_port_t sd_port, sd_internal_status_t status, uint32_t block_number)
{
    MSDC_REGISTER_T *msdc_register_base;
    msdc_register_base = MSDC0_REG;
