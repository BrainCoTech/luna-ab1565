...
/**
 * @brief Request to create multi audio afe in source
 * @param hardware The audio hardware which would be reserved as a source
 * @param instance The audio hardware instance (meaning depends on \e hardware)
 * @param channel The audio channel (meaning depends on \e hardware)
 *
 * @return The Source ID associated with the audio hardware.
 */
SOURCE StreamAudioAfeMultiSource(audio_hardware hardware, audio_instance instance, audio_channel channel)
{
    UNUSED(instance);
    ...
    for (source_type = SOURCE_TYPE_MULTI_AUDIO_MIN ; source_type <= SOURCE_TYPE_MULTI_AUDIO_MAX ; source_type++) {
    ...
    if (source == NULL) {
        AUDIO_ASSERT(0 && "there is no enough audio source type");
        return NULL;
    }
...
SOURCE StreamN9BleSource(void *param)
{
    ...
    N9BLE_setting.N9Ble_source.Input_sample_rate = open_param->stream_in_param.ble.sampling_frequency;
