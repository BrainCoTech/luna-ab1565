...
ATTR_TEXT_IN_IRAM int32_t audio_dongle_dl_usb_clock_skew_check(void *dongle_handle, uint32_t input_samples, uint32_t *buffer0_output_size, audio_dongle_type_t dongle_type)
{
            ...
            if (remain_samples < 0) {
                ...
                if (remain_samples >= clk_skew_watermark_samples) {
                    ...
                    if (frac_rpt == (clk_skew_compensation_mode - 1)) {
                        buffer_output_size = buffer_default_output_size - bytes_per_sample;
                    }
                }
            } else if (clk_skew_count == -DL_CLOCK_SKEW_CHECK_COUNT) {
                if (remain_samples <= clk_skew_watermark_samples) {
    ...
    COMMON_DONGLE_LOG_I("[Dongle Common][DL] %d %d %d %d %d %d", 6,
        remain_samples, clk_skew_count, compensatory_samples, buffer_output_size, clk_skew_watermark_samples, frac_rpt);
    /* update parameters to dongle */
    switch (dongle_type) {
#ifdef AIR_BT_AUDIO_DONGLE_ENABLE
        case AUDIO_DONGLE_TYPE_BT:
...
void audio_dongle_dl_afe_start(void *dongle_handle, audio_dongle_type_t dongle_type)
{
    ...
    if ((source->param.audio.mem_handle.pure_agent_with_src != true) && // i2s slv interconn tracking mode
        ...
        switch (agent) {
            ...
            case HAL_AUDIO_AGENT_MEMORY_VUL3:
                ...
                break;
            default:
                //COMMON_DONGLE_LOG_E("[Dongle Common][DL][AFE IN] ERROR: unknow agent = 0x%x", 1, agent);
                AUDIO_ASSERT(0);
        }
    } else {
        /* Tracking mode */
        if ((source->scenario_type == AUDIO_SCENARIO_TYPE_ULL_AUDIO_V2_DONGLE_DL_I2S_SLV_IN_0) ||
            ...
            if (source->param.audio.AfeBlkControl.u4asrcflag) { // vdma mode use hwsrc1
                if (source->param.audio.AfeBlkControl.u4asrcid == AFE_MEM_ASRC_1) {
                    afe_in->afe_vul_cur_addr  = ASM_CH01_OBUF_WRPNT;
                    afe_in->afe_vul_base_addr = ASM_OBUF_SADR;
                } else if (source->param.audio.AfeBlkControl.u4asrcid == AFE_MEM_ASRC_2) {
                    afe_in->afe_vul_cur_addr  = ASM2_CH01_OBUF_WRPNT;
                    afe_in->afe_vul_base_addr = ASM2_OBUF_SADR;
                } else {
                    AUDIO_ASSERT(0 && "ASSERT: hwsrc is not enable");
                }
            ...
            AUDIO_ASSERT(0 && "ASSERT: tracking mode, unknow agent");
        }
    }
    /* clear stream info */
    ...
    COMMON_DONGLE_LOG_I("[Dongle Common][DL][AFE IN] scenario type %d-%d, start stream gpt count = 0x%x, cur_addr = 0x%x, base_addr = 0x%x, cur_ro = 0x%x, cur_base = 0x%x", 7,
                source->scenario_type,
                sink->scenario_type,
                gpt_count,
...
void audio_dongle_afe_in_ccni_handler(void *dongle_handle, audio_dongle_type_t dongle_type)
{
    ...
    if (dongle_type == AUDIO_DONGLE_TYPE_BLE) {
            ...
            if (((bt_audio_dongle_handle_t *)dongle_handle)->link_type == BT_AUDIO_TYPE_A2DP) {
                // prefill 8ms data to avoid buffer underrun
                prefill_size = ((((bt_audio_dongle_handle_t *)dongle_handle)->stream_info.dl_info.sink.bt_out).frame_interval + 8000) * source->param.audio.src_rate / 1000 / 1000 * (source->param.audio.format_bytes);
            } else {
                // prefill 1ms data for clk skew
                prefill_size = ((((bt_audio_dongle_handle_t *)dongle_handle)->stream_info.dl_info.sink.bt_out).frame_interval + 1000) * source->param.audio.src_rate / 1000 / 1000 * (source->param.audio.format_bytes);
            }
#endif
    }
    UNUSED(dongle_handle);
    ...
    if ((stream_status == AUDIO_DONGLE_STREAM_START) || (stream_status == AUDIO_DONGLE_STREAM_RUNNING)) {
        ...
        if (runtime->irq_exist == false) {
            ...
            if ((source->scenario_type == AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_I2S_IN) ||
                ...
                if ((buffer_info->WriteOffset & (~15)) == (buffer_info->ReadOffset & (~15))) {
                    COMMON_DONGLE_LOG_E("[Dongle Common][DL][AFE IN] hwsrc start fail owo[%d] oro[%d]", buffer_info->WriteOffset, buffer_info->ReadOffset);
                }
            }
        } else {
        ...
        if ((source->param.audio.AfeBlkControl.u4asrcflag) && (!afe_in->i2s_slv_flag)) {
            ...
            if (i2s_vdma_status != VDMA_OK) {
                COMMON_DONGLE_LOG_E("[Dongle Common][DL][AFE IN] DSP UL start i2s slave set vdma_start fail %d", 1, i2s_vdma_status);
                AUDIO_ASSERT(0);
            }
            COMMON_DONGLE_LOG_I("[Dongle Common][DL][AFE IN] VDMA Start", 0);
        }
