...
#define UL_FIRST_PACKET_SAFE_DELAY_FRAME_1000US_LINE_OUT            (-2)
#define UL_PLAYEN_DELAY_FRAME_1000US_I2S_SLV_OUT                    (0) /* procsess time 2ms */
#define UL_FIRST_PACKET_SAFE_DELAY_FRAME_1000US_I2S_SLV_OUT         (-2)
...
static int16_t wireless_mic_rx_stream_process_count;
...
ATTR_TEXT_IN_IRAM static void wireless_mic_rx_ul_retrigger_software_timer_handler(void *user_data)
{
    ...
    //DSP_MW_LOG_I("[Wireless MIC RX][UL] retrigger stream\r\n", 0);
}
            ...
            switch (c_handle->sub_id)
            {
                case AUDIO_TRANSMITTER_WIRELESS_MIC_RX_UL_USB_OUT_0:
                    ...
                    if (c_handle->source_info.bt_in.frame_interval == 1000) {
                        ...
                        if (c_handle->first_packet_status == WIRELESS_MIC_RX_UL_FIRST_PACKET_NOT_READY) {
                            if (wireless_mic_rx_packet_anchor_in_mixer != UL_PACKET_ANCHOR_IN_MIXER_INVAILD_VALUE) {
                                ...
                                /* As the 1st packet is not running, so need to write seq number and payload_len into the share buffer */
                                audio_transmitter_share_information_fetch(NULL, source->transform->sink);
                                ShareBufferInfo = &(source->transform->sink->streamBuffer.ShareBufferInfo);
                                ((audio_transmitter_frame_header_t *)(ShareBufferInfo->start_addr + ShareBufferInfo->write_offset))->seq_num      = c_handle->sink_info.usb_out.seq_num;
                                ((audio_transmitter_frame_header_t *)(ShareBufferInfo->start_addr + ShareBufferInfo->write_offset))->payload_len  = c_handle->sink_info.usb_out.frame_size;
                                c_handle->first_packet_status = WIRELESS_MIC_RX_UL_FIRST_PACKET_READY;
                    ...
                    else if (c_handle->first_packet_status == WIRELESS_MIC_RX_UL_FIRST_PACKET_READY)
                    {
                        ...
                        MCE_GetBtClk((BTCLK*)&bt_clk, (BTPHASE*)&bt_phase, BT_CLK_Offset);
                        if ((c_handle->source_info.bt_in.frame_interval == 1000) ||
                            (wireless_mic_rx_ul_play_time_is_arrived(c_handle, bt_clk, bt_phase))) {
                        ...
                        if (c_handle->source_info.bt_in.frame_interval == 1000) {
                            ...
                            wireless_mic_rx_ul_adjust_bt_time(&bt_time);
                            if (MCE_Compare_Val_FromAB(&fetch_time, &bt_time) > 0) {
                                MCE_Add_us_FromA(c_handle->source_info.bt_in.frame_interval, &fetch_time, &fetch_time);
                                c_handle->source_info.bt_in.fetch_anchor = fetch_time.period;
                                c_handle->source_info.bt_in.fetch_anchor_phase = fetch_time.phase;
                            }
                    ...
                    else if (c_handle->first_packet_status == WIRELESS_MIC_RX_UL_FIRST_PACKET_PLAYED)
                    {
                        ...
                        if (c_handle->source_info.bt_in.frame_interval == 1000) {
                            /* Wake up the stream later to keep align with the BT fetch time */
                            if (c_handle->sink_info.usb_out.re_trigger_ongoing == 0) {
                                bt_time.period = bt_clk;
                                bt_time.phase = bt_phase;
                                wireless_mic_rx_ul_adjust_bt_time(&bt_time);
                                wireless_mic_rx_ul_retrigger_ccni_handler(c_handle, bt_time.period, bt_time.phase);
...
static void wireless_mic_rx_ul_usb_out_init(wireless_mic_rx_ul_handle_t *rx_handle, audio_transmitter_open_param_p audio_transmitter_open_param, bt_common_open_param_p bt_common_open_param)
{
    ...
    else if (rx_handle->source_info.bt_in.frame_interval == 1000)
    {
        ...
        rx_handle->source_info.bt_in.bt_retry_window_phase            = wireless_mic_rx_bt_init_play_info.dl_timestamp_phase;
...
static void wireless_mic_rx_ul_line_out_start(wireless_mic_rx_ul_handle_t *rx_handle)
{
    ...
    if (rx_handle->source_info.bt_in.codec_type == AUDIO_DSP_CODEC_TYPE_ULD) {
        rx_handle->sink->streamBuffer.BufferInfo.WriteOffset = (WIRELESS_MIC_RX_ULD_RESERVED_PROCESS_TIME * sample_size * rx_handle->sink_info.line_out.channel_num * rx_handle->sink_info.line_out.sample_rate) / (1000 * 1000) + sample_size * rx_handle->sink_info.line_out.channel_num * 5;
    } else if (rx_handle->source_info.bt_in.codec_type == AUDIO_DSP_CODEC_TYPE_LC3PLUS) {
...
ATTR_TEXT_IN_IRAM int32_t wireless_mic_rx_ul_get_irq_cp_samples(wireless_mic_rx_ul_handle_t *rx_handle, uint32_t bt_clk, uint16_t bt_phase)
{
    int32_t delta;
    uint32_t delta_val;
    BTTIME_STRU fetch_anchor_curr, fetch_anchor_timeout;

    if (rx_handle->source_info.bt_in.frame_interval == 1000) {
        fetch_anchor_curr.period = bt_clk & 0xFFFFFFFC;
        fetch_anchor_curr.phase = bt_phase + (bt_clk & 0x00000003) * 625;
        fetch_anchor_timeout.period = rx_handle->source_info.bt_in.fetch_anchor;
        fetch_anchor_timeout.phase = rx_handle->source_info.bt_in.fetch_anchor_phase;
        delta = MCE_Compare_Val_FromAB(&fetch_anchor_timeout, &fetch_anchor_curr);

        if (delta >= 0) {
            delta_val = MCE_Get_Offset_FromAB(&fetch_anchor_timeout, &fetch_anchor_curr);
        } else {
            delta_val = MCE_Get_Offset_FromAB(&fetch_anchor_curr, &fetch_anchor_timeout);
        }

        if ((delta < 0) || (delta_val < WIRELESS_MIC_RX_UL_IRQ_CP_LOW_THRE)) {
            if (rx_handle->irq_compensation_dir < 0) {
                rx_handle->irq_compensation_cnt--;
            } else {
                rx_handle->irq_compensation_cnt = 0;
            }
            rx_handle->irq_compensation_dir = -1;
        } else if (delta_val > WIRELESS_MIC_RX_UL_IRQ_CP_HIGH_THRE) {
            if (rx_handle->irq_compensation_dir > 0) {
                rx_handle->irq_compensation_cnt++;
            } else {
                rx_handle->irq_compensation_cnt = 0;
            ...
            rx_handle->irq_compensation_dir = 0;
            rx_handle->irq_compensation_cnt = 0;
        }

        if (rx_handle->irq_compensation_cnt >= WIRELESS_MIC_RX_UL_IRQ_CP_CNT) {
            rx_handle->irq_compensation_cnt = 0;
            DSP_MW_LOG_I("[Wireless MIC RX][UL][DEBUG] IRQ period - 1", 0);
            return -1;
        } else if (rx_handle->irq_compensation_cnt <= -WIRELESS_MIC_RX_UL_IRQ_CP_CNT) {
            rx_handle->irq_compensation_cnt = 0;
            DSP_MW_LOG_I("[Wireless MIC RX][UL][DEBUG] IRQ period + 1", 0);
            return 1;
        }
            ...
            if ((rx_handle->sub_id == AUDIO_TRANSMITTER_WIRELESS_MIC_RX_UL_USB_OUT_0) &&
                ...
                for (i = 0; i < wireless_mic_rx_stream_process_channel; i++) {
                    if (rx_handle->source_info.bt_in.bt_info[i] == NULL) {
                        ...
                        blk_num      = p_share_info->sub_info.block_info.block_num;
                        /* get new blk index */
                        blk_index = ((rx_handle->source_info.bt_in.bt_info[i])->blk_index + 1) % blk_num;
                        /* update read index */
                        StreamDSP_HWSemaphoreTake();
                        p_share_info->read_offset = blk_index * blk_size;
                        StreamDSP_HWSemaphoreGive();
...
ATTR_TEXT_IN_IRAM void wireless_mic_rx_ul_source_drop_postprocess(SOURCE source, uint32_t amount)
{
    ...
    if (wireless_mic_rx_stream_process_count >= wireless_mic_rx_first_ul_handle->total_number) {
        for (i = 0; i < WIRELESS_MIC_RX_DATA_CHANNEL_NUMBER; i++)
        {
            if (rx_handle->source_info.bt_in.bt_info[i] == NULL)
            {
