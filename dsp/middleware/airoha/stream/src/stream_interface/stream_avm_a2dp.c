    ...
    do {
        ...
        if (a2dp_hdr_info.pcb_state == PCB_STATE_LOST) {
            ...
            if ((afe_get_dl1_query_data_amount() > (((source->param.n9_a2dp.latency_monitor == true) ? BUFFER_WAIT_PLR_ISR_NUM_GAMING_MODE : \
                                                     (source->param.n9_a2dp.alc_monitor) ? BUFFER_WAIT_PLR_ISR_NUM_ALC : BUFFER_WAIT_PLR_ISR_NUM) * (source->transform->sink->param.audio.rate * source->transform->sink->param.audio.period / 1000))) || (afe_get_bt_sync_monitor(AUDIO_DIGITAL_BLOCK_MEM_DL1) == 0)) {
                S16 i;
...
ATTR_TEXT_IN_IRAM_LEVEL VOID SourceDrop_N9_a2dp(SOURCE source, U32 amount)
{
    ...
    if ((n9_a2dp_param->buffer_empty_cnt > (reinit_thd * (!source->param.n9_a2dp.alc_monitor)))
        ...
        if (read_reg != 0) {
            ...
            STRM_A2DP_LOG_I("Buffer status trigger re-sync empty_cnt:%d, ALC:%d", 2, n9_a2dp_param->buffer_empty_cnt, (source->param.n9_a2dp.alc_monitor)&ADATIVE_LATENCY);
