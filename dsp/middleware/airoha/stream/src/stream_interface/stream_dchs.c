...
bool g_dchs_dl_waiting_copy_uart_data = false;
...
void dchs_mcu2dsp_msg_sync_callback(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    if(sync_msg->sync_type == DCHS_DL_OTHER_CHIP_DL_EXIST){
        ...
        if(data_scenario_type == AUDIO_SCENARIO_TYPE_VP){
            ...
            if(sample_rate != source->param.audio.rate){
        ...
        if(data_scenario == AUDIO_SCENARIO_TYPE_VP || data_scenario == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_IN
            ...
            if(data_scenario == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0) {
                dchs_dl_set_play_en_ms(DCHS_DL_PLAY_EN_USB_IN_DELAY_MS, data_scenario);
            } else {
...
static void hal_audio_gpt_trigger_mem_vul(void)
{
    ...
    if (gpt_count_sub_end > curr_cnt) { // gpt register does not overflow
        ...
        DSP_MW_LOG_E("[DCHS UL][trigger_mem_vul]Warning: already expire\r\n", 0);
        // AUDIO_ASSERT(0);
    }
