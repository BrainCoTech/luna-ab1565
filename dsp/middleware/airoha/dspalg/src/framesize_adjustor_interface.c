...
framesize_adjustor_status_t stream_function_framesize_adjustor_init(framesize_adjustor_port_t *port, framesize_adjustor_config_t *config, uint32_t MemSize, CusFunc_initialize fun_initialize, CusFunc_process fun_process)
{
    ...
    if (config->type == FRAMESIZE_ADJUSTOR_FUNCTION_INTERLEAVED && config->total_channels != 2) {
        DSP_MW_LOG_E("[FRAMESIZE_ADJUSTOR] FUCNTION_TYPE is not match with tota_channels %d!", 1, config->total_channels);
        AUDIO_ASSERT(0);
    }
    ...
    watermark_max_size = (prefill_samples + config->inout_sample_num)<<(config->process_sample_resolution==RESOLUTION_16BIT?1:2);
...
bool stream_function_framesize_adjustor_initialize(void *para)
{
    ...
    if (port->status == FRAMESIZE_ADJUSTOR_PORT_STATUS_INIT) {
        ...
        /*Initialize*/
        framesize_adjustor_CusFunc_initialize(para);
...
ATTR_TEXT_IN_IRAM bool stream_function_framesize_adjustor_process(void *para)
{
    ...
    if (in_frame_size != port->inout_size) {
        DSP_MW_LOG_E("[FRAMESIZE_ADJUSTOR] frame size is not match %d %d!", 2, in_frame_size, port->inout_size);
        AUDIO_ASSERT(FALSE);
        return true;
    }
            ...
            if (port->type == FRAMESIZE_ADJUSTOR_FUNCTION_DEINTERLEAVED) {
                ...
                for (i = 0; (uint32_t)i < channel_number; i++) {
                    ...
                    if (in_frame_size > (channel_buf->buf_size - channel_buf->data_size)) {
                        DSP_MW_LOG_E("[FRAMESIZE_ADJUSTOR][in] port 0x%x not enough space, %d, %d", 3, port, in_frame_size, (channel_buf->buf_size - channel_buf->data_size));
                        AUDIO_ASSERT(FALSE);
                    }
                    ...
                    while (channel->in.data_size >= process_frame_size) {
                        ...
                        if (i == 1) {
                            //LOG_AUDIO_DUMP(channel->in.temp, port->process_size, AUDIO_DCHS_DL_SOURCE_IN_CH_1);
                        } else if(i == 2) {
                            //LOG_AUDIO_DUMP(channel->in.temp, port->process_size, AUDIO_DCHS_DL_SOURCE_IN_CH_2);
                        }
#endif
                        framesize_adjustor_Fun(channel->in.temp, channel->out.temp, port->process_size, working_buf);
#ifdef AIR_AUDIO_DUMP_ENABLE
                        if (i == 1) {
                            //LOG_AUDIO_DUMP(channel->out.temp, port->process_size, AUDIO_DCHS_DL_SOURCE_IN_CH_3);
                        } else if(i == 2) {
                            //LOG_AUDIO_DUMP(channel->out.temp, port->process_size, AUDIO_DCHS_DL_SOURCE_IN_CH_4);
                        }
#endif
                        ...
                        if (process_frame_size > (channel_buf->buf_size - channel_buf->data_size)) {
                            DSP_MW_LOG_E("[FRAMESIZE_ADJUSTOR][out][1] port 0x%x not enough space, %d, %d", 3, port, process_frame_size, (channel_buf->buf_size - channel_buf->data_size));
                            AUDIO_ASSERT(FALSE);
                        }
                    ...
                    /* Step3: read data from internal output circular buffer to inout buffer. (Should always have enought data in internal output circular buffer) */
                    channel_buf = &(channel->out);
                    if (channel_buf->data_size < in_frame_size) {
                        DSP_MW_LOG_E("[FRAMESIZE_ADJUSTOR][out][2] port 0x%x not enough data, %d, %d", 3, port, process_frame_size, (channel_buf->buf_size - channel_buf->data_size));
                        AUDIO_ASSERT(FALSE);
                    }
                    framesize_adjustor_read_data_from_channel_buf(channel_buf, buf, in_frame_size);
                }
            } else if (port->type == FRAMESIZE_ADJUSTOR_FUNCTION_INTERLEAVED) {
                /* Summary:
...
bool framesize_adjustor_sample_Fun1_initialize(void* para) {
    UNUSED(para);
    DSP_MW_LOG_I("[FRAMESIZE_ADJUSTOR] Fun1_initialize",0);
    return FALSE;
}

bool framesize_adjustor_sample_Fun2_initialize(void* para) {
    UNUSED(para);
