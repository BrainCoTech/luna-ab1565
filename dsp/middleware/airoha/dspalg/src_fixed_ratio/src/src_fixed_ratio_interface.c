...
bool stream_function_src_fixed_ratio_initialize(void *para)
{
    ...
    } else if ((port->status == SRC_FIXED_RATIO_PORT_STAT_RUNNING) || (port->status == SRC_FIXED_RATIO_PORT_STAT_BYPASS)) {
        hal_nvic_restore_interrupt_mask(saved_mask);
        return FALSE;
    } else {
    ...
    if (port->cvt_num > 1) { // initialize other ports of same owner
        ...
        for (j = i - 1; j >= 0; j--) {
            ...
            if (src_fixed_ratio_port[j].owner == src_fixed_ratio_port[i].owner) {
                ...
                } else if ((other_port->status == SRC_FIXED_RATIO_PORT_STAT_RUNNING) || (other_port->status == SRC_FIXED_RATIO_PORT_STAT_BYPASS)) {
