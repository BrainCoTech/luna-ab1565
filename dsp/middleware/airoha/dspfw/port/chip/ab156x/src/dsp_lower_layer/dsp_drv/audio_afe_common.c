...
ATTR_TEXT_IN_IRAM void i2s_slave_ul_port_interrupt_handler(vdma_event_t event, void  *user_data)
{
    ...
    UNUSED(event);
    UNUSED(user_data);
    hal_audio_interface_t audio_interface = (hal_audio_interface_t)(uint32_t)user_data;
    uint32_t mask, port, hw_current_write_idx, hw_current_read_idx;
    SOURCE source = NULL;
    afe_block_t *afe_block = NULL;
    U32 addr_offset = 0;
    if(audio_interface == HAL_AUDIO_INTERFACE_1 || audio_interface == HAL_AUDIO_INTERFACE_2){
        ...
        afe_block = &Source_blks[SOURCE_TYPE_AUDIO2]->param.audio.AfeBlkControl;
        addr_offset = 0x100;
    }
    //hal_audio_interface_t audio_interface = source->param.audio.audio_interface;
    vdma_channel_t rx_dma_channel;
    ...
    if (afe_block->u4asrcflag) {
        ...
        afe_block->u4ReadIdx = AFE_GET_REG(ASM_CH01_IBUF_RDPNT + addr_offset) - AFE_GET_REG(ASM_IBUF_SADR + addr_offset);
        ...
        if ((source->scenario_type == AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_I2S_IN) ||
            ...
            buffer_info->WriteOffset = AFE_GET_REG(ASM_CH01_OBUF_WRPNT + addr_offset) - AFE_GET_REG(ASM_OBUF_SADR + addr_offset);
        }
    } else {
        if ((source->scenario_type != AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_I2S_IN) &&
            (source->scenario_type != AUDIO_SCENARIO_TYPE_ULL_AUDIO_V2_DONGLE_DL_I2S_SLV_IN_0) &&
...
{
    uint32_t mask, first_dl_irq_time, hw_current_read_idx;
    SINK sink = Sink_blks[SINK_TYPE_AUDIO];
    SOURCE source;
    ...
    if (sink->scenario_type == AUDIO_SCENARIO_TYPE_WIRELESS_MIC_RX_UL_I2S_SLV_OUT_0) {
        int32_t wireless_mic_cp_samples;
        hal_audio_memory_irq_period_parameter_t irq_period;
        wireless_mic_cp_samples = wireless_mic_rx_ul_get_irq_cp_samples(rx_handle, bt_clk, bt_phase);
        irq_period.memory_select = HAL_AUDIO_MEMORY_DL_DL1;
        irq_period.rate = sink->param.audio.rate;
        irq_period.irq_counter = (uint32_t)((int32_t)sink->param.audio.count + wireless_mic_cp_samples);
...
#endif
    {
        ...
        if (source->type == SOURCE_TYPE_N9BLE) {
            ...
            if (callback_ptr->Status != CALLBACK_SUSPEND) {
                ...
                drop_amount = (source->param.n9ble.dual_cis_status != DUAL_CIS_DISABLED) ? source->param.n9ble.frame_length * source->param.n9ble.process_number * 2 : source->param.n9ble.frame_length * source->param.n9ble.process_number;
                ...
                DSP_MW_LOG_I("Callback Busy : %d, Source drop and move Sink WO, dl_irq_cnt:%d, flush_amount %d", 3, callback_ptr->Status, dl_irq_cnt, flush_amount);
            }
    ...
    if (1) { //modify for ab1568
        ...
        if ((afe_offset_overflow_check(pre_offset, buffer_info, true))
            ...
            if (afe_block->u4asrcflag) {
                ...
                uint32_t cur_src_ro = src_out_read - base;
                remain_data = (src_out_write > src_out_read) ? src_out_write - src_out_read : src_out_write + src_out_size - src_out_read;
                remain_data = (sink->param.audio.channel_num >= 2) ? remain_data >> 1 : remain_data;
                if (remain_data > (afe_block->u4asrcrate * (U32)sink->param.audio.period * sink->param.audio.format_bytes) / 1000) {
                    empty = false;
                }
#if defined BASE_STEREO_HIGH_G3_TYPE_77 || defined BASE_STEREO_HIGH_G3_TYPE_72
                UNUSED(cur_src_ro);
#else
                /* clear dirty data in src output sram(hw access), avoid hw access dirty data in abnormal */
                if (pre_src_ro == 0) {
                    pre_src_ro = cur_src_ro;
                } else {
                    if (cur_src_ro >= pre_src_ro) {
                        memset((void *)(base + pre_src_ro), 0, cur_src_ro - pre_src_ro);
                    } else {
                        memset((void *)(base + pre_src_ro), 0, src_out_size - pre_src_ro);
                        memset((void *)(base), 0, cur_src_ro);
                    }
                    pre_src_ro = cur_src_ro;
                }
#endif
    ...
    if (1) {//afe_get_memory_path_enable(AUDIO_DIGITAL_BLOCK_MEM_DL3)
        ...
        if ((sink->scenario_type >= AUDIO_SCENARIO_TYPE_WIRELESS_MIC_RX_UL_LINE_OUT) && (sink->scenario_type <= AUDIO_SCENARIO_TYPE_WIRELESS_MIC_RX_UL_I2S_SLV_OUT_0))
        {
            ...
            if (sink->scenario_type == AUDIO_SCENARIO_TYPE_WIRELESS_MIC_RX_UL_I2S_SLV_OUT_0) {
                ...
                hal_audio_memory_irq_period_parameter_t irq_period;
                wireless_mic_cp_samples = wireless_mic_rx_ul_get_irq_cp_samples(rx_handle, bt_clk, bt_phase);
                irq_period.memory_select = HAL_AUDIO_MEMORY_DL_DL3;
                irq_period.rate = runtime->rate;
                irq_period.irq_counter = (uint32_t)((int32_t)runtime->count + wireless_mic_cp_samples);
                hal_audio_control_set_value((hal_audio_set_value_parameter_t *)&irq_period, HAL_AUDIO_SET_MEMORY_IRQ_PERIOD);
            }
        }
#endif /* AIR_WIRELESS_MIC_RX_ENABLE */
...
void afe_set_asrc_ul_configuration_parameters(SOURCE source, afe_src_configuration_p asrc_config)
{
    ...
    DSP_MW_LOG_I("DSP UL asrc in rate:%d, out rate:%d, tracking %d, input offset %d, output offset %d",5,
...
{
    uint32_t *sync_info = (uint32_t *)user_data;
    //cm4_dsp_audio_sync_request_param_t *sync_info = (cm4_dsp_audio_sync_request_param_t *)user_data;
