...
#if defined(AIR_HEARING_AID_ENABLE) || defined(AIR_HEARTHROUGH_PSAP_ENABLE)
    {psap_features_get_list, psap_features_init, psap_features_deinit, psap_suspend, psap_resume, psap_runtime_config, stream_function_hearing_aid_set_dl_state,},//AUDIO_LLF_TYPE_HEARING_AID
#else
...
#endif
    source = StreamVirtualSource(NULL, type, open_param->stream_in_param.virtual.virtual_mem_size, channel_num, data_size, samplingrate);
    if (source != NULL) {
#if defined(AIR_WIRED_AUDIO_ENABLE)
        if (open_param->audio_scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_OUT_IEM){
...
void CB_N9_BLE_UL_OPEN(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    g_lc3_user_cnt += 1;
    DSP_MW_LOG_I("[BLE] g_lc3_user_cnt %d",1,g_lc3_user_cnt);
#if defined (AIR_DCHS_MODE_ENABLE)
    if(dchs_get_device_mode() != DCHS_MODE_SINGLE){
        dsp_uart_ul_open();
        dsp_uart_ul_clear_rx_buffer();
...
/*******************************************
* profiling on ab1565 @2022/12/22
* DSP clock: 208MHz
* BAND mode: WB
...
* - AFE write:           280us
*
* UL process time: 10886us (8409us + 584us + 1893us)
********************************************/
...
* - AFE write:   276us
*
* UL process time: 10730us (8708us + 2022us)
********************************************/
#define BLE_UL_TIME_STREAM_PROCESS      11000

#if defined(AIR_BTA_IC_STEREO_HIGH_G3) // && defined(AIR_AI_NR_PREMIUM_270K_ENABLE)
...
#endif
#ifdef AIR_ANC_WIND_DETECTION_ENABLE
    } else if ((nvkey_id == NVID_DSP_ALG_ANC_WIND_DET) || (nvkey_id == NVID_DSP_ALG_ANC_WIND_DET_PT) || (nvkey_id == NVID_DSP_ALG_ANC_WIND_DET_SIDETONE)) {
...
#endif
#ifdef AIR_ANC_ENVIRONMENT_DETECTION_ENABLE
    } else if ((nvkey_id == NVID_DSP_ALG_ANC_ENVIRONMENT_DETECTION) || (nvkey_id == NVID_DSP_ALG_ANC_ENVIRONMENT_DETECTION_PT) || (nvkey_id == NVID_DSP_ALG_ANC_ENVIRONMENT_DETECTION_SIDETONE)) {
...
ATTR_TEXT_IN_RAM_FOR_MASK_IRQ void CB_CM4_VP_PLAYBACK_START(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    if (playback_vp_if.sink->param.audio.aws_sync_request) {
        ...
        if (target_cnt < curr_cnt) {
            ...
            //platform_assert("vp sync (target-current) < DSP_VP_POLLING_TIME",__FILE__,__LINE__);
            time_out_flag = true;
        } else {
...
static void dsp_audio_msg_queue_handle(uint32_t msg_id, cm4_dsp_audio_sync_request_param_t *param)
{
    ...
    if (cb != NULL) {
        if (cb->cb_func != NULL) {
            callback = cb->cb_func;
            callback(action_id, &(param->nvkey_addr));
        } else {
            // error
