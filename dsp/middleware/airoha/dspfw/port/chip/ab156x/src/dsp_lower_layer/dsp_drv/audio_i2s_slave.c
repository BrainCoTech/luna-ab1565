...
#define ReadREG(_addr)          (*(volatile uint32_t *)(_addr))
    ...
    if (runtime->audio_interface == HAL_AUDIO_INTERFACE_1) {
        ...
        #endif
        vdma_register_callback(rx_dma_channel, i2s_slave_ul_port_interrupt_handler, (void*)HAL_AUDIO_INTERFACE_1);
        ...
        #endif
        vdma_register_callback(rx_dma_channel, i2s_slave_ul_port_interrupt_handler, (void*)HAL_AUDIO_INTERFACE_2);
        ...
        #endif
        vdma_register_callback(rx_dma_channel, i2s_slave_ul_port_interrupt_handler, (void*)HAL_AUDIO_INTERFACE_3);
    }
...
static int32_t i2s_slave_ul_stop(SOURCE source)
{
    ...
    if (runtime->memory == HAL_AUDIO_MEM2) {
        if (runtime->AfeBlkControl.u4asrcflag) {
            afe_set_asrc_enable(false, AFE_MEM_ASRC_2, (void *)&asrc_config);
            // afe_mem_asrc_enable(AFE_MEM_ASRC_2, false);
            hal_src_set_start(AFE_MEM_ASRC_2, false);
        }
    } else {
        if (runtime->AfeBlkControl.u4asrcflag) {
            afe_set_asrc_enable(false, AFE_MEM_ASRC_1, (void *)&asrc_config);
