...
#endif
            DSP_MW_LOG_I("[DSP_RESOURCE][DSP_Callback_PreloaderConfig] Feature ID load: 0x%X", 1, (*(feature_list_ptr)&0xFF));
...
#if defined(AIR_WIRELESS_MIC_RX_ENABLE)
    if (source != NULL) {
        if ((source->scenario_type >= AUDIO_SCENARIO_TYPE_WIRELESS_MIC_RX_UL_LINE_OUT) &&
