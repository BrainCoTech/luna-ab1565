...
ATTR_TEXT_IN_IRAM_LEVEL_1 VOID AudioCheckTransformHandle(TRANSFORM transform)
{
    ...
    if (transform != NULL) {
        ...
        if (transform->source->taskId == DHP_TASK_ID) {
            handler = pDHP_TaskHandler;
        } else if (transform->source->taskId == DAV_TASK_ID) {
            handler = pDAV_TaskHandler;
        }
...
VOID Sink_Audio_BufferInfo_Rst(SINK sink, U32 offset)
{
    ...
    if (sink->buftype == BUFFER_TYPE_CIRCULAR_BUFFER) {
        ...
        if (sink->param.audio.mem_handle.scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_MAINSTREAM) {
            ...
            afe_prefill = (sink->param.audio.format_bytes * sink->param.audio.channel_num * sink->param.audio.count * 15) / 10 + 32;
#else
            /* Don't use HWSRC, and prefill with 1.5 * frame time(3ms, 1ms) + AFE Pbuf prefill (208 byte) */
            afe_prefill = (sink->param.audio.format_bytes * sink->param.audio.channel_num * sink->param.audio.count * 15) / 10 + 208;
#endif
            ...
            afe_prefill = sink->param.audio.format_bytes * sink->param.audio.channel_num * (sink->param.audio.count * 2) + 32;
#else
            /* Don't use HWSRC, and prefill with 2 * frame time(2.67ms, 1.33ms, 0.67ms) + AFE Pbuf prefill (208 byte) */
            afe_prefill = sink->param.audio.format_bytes * sink->param.audio.channel_num * (sink->param.audio.count * 2) + 208;
#endif
        }
        else if (sink->param.audio.mem_handle.scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_OUT) {
...
ATTR_TEXT_IN_IRAM BOOL Sink_Audio_WirelessMic_WriteBuffer (SINK sink, U8 *src_addr, U32 length)
{
    ...
    callback_ptr = DAVT_Callback_Get(sink->transform->source, sink);
...
ATTR_TEXT_IN_IRAM_LEVEL_2 BOOL Sink_Audio_WriteBuffer (SINK sink, U8 *src_addr, U32 length)
{
    ...
    if(ble_dl_gpt_start && transform->source->type == SOURCE_TYPE_N9BLE && transform->source->param.n9ble.context_type == BLE_CONTENT_TYPE_ULL_BLE)
    {
        ...
        if(ble_dl_gpt_duration > ((2*1000) - 100))
        {
