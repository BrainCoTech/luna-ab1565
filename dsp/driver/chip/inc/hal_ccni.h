 ...
 *   The CCNI module had 32 events between any two cores. The driver defines a special name for every event in hal_ccni_config.h for easy use.\n
 *
 *  - Step 1: Decide to use an event with or without a message.\n
 *
 *  - Step 2: Find an event that is not used.\n
 *      (A) Check the event usage status in \\dsp\\driver\\chip\\*\\dsp0\\inc\\hal_ccni_config.h. The event in default status can be used.\n
 *      (B) Or check the ccni_*_to_*_function_table[] in hal_ccni_config.c; The event is not used if the isr is default_isr.\n
 *           For example, if you want to apply an event for MCU to DSP0, please check ccni_cm4_to_dsp0_function_table[] to find an unused event.\n
 ...
 *
 *  - Step 3: Replace the event name in \\dsp\\driver\\chip\\*\\dsp0\\inc\\hal_ccni_config.h for DSP0. \n
 *
 *    We strongly advise that you use a name which implies the event function.
 *    @code
 *    //define EVENT 8 for exception from DSP0 to MCU
 ...
 *        if( pMsg != NULL) {
 *            uint32_t address;
 *            //Suppose DSP0 received an EVENT from MCU with sharing memory and the memory address is in pMsg[0]. \n
 ...
 *    @endcode
 *
 * - Step 5: Register the isr handler to ccni_*_to_*_function_table[] in \\dsp\\driver\\chip\\*\\dsp0\\src\\hal_ccni_config.c for DSP0. \n
 *
 ...
 * - Sample code:
 *    @code
 *       //DSP0 send an event IRQGEN_DSP0_TO_CM4_EVENT9 to MCU.
 *       uint32_t status;
 ...
 *    @code
 *       //DSP0 send an event CCNI_DSP0_TO_CM4_EVENT1 to MCU with const value in message.
 *       uint32_t status;
         //DSP0 send event CCNI_DSP0_TO_CM4_EVENT1 to MCU
 ...
 *       }
 *
 *       //DSP0 send an event CCNI_DSP0_TO_CM4_EVENT2 to MCU and notify send 128 Bytes data with sharing memory.
 *       uint32_t status;
 *       uint32_t sharing_memory_address; 
 *       //DSP0 send event CCNI_DSP0_TO_CM4_EVENT2 to MCU
 ...
 *       //Please mind the receiver should call hal_memview_infrasys_to_mcu(msg_array[0]) to remap the address, otherwise the receiver can not get the right data.
 *       msg_array[1] = 0x80;
 *       //the second parameter should not be NULL.
 *       ret = hal_ccni_set_event(event, msg_array);
 *       if (HAL_CCNI_STATUS_OK != ret) {
