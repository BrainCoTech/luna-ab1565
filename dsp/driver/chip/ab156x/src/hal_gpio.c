...
hal_gpio_status_t hal_gpio_pull_up(hal_gpio_pin_t gpio_pin)
{
  //  uint32_t mask;
    ...
    if (shift != 0xff) {
   //     hal_nvic_save_and_set_interrupt_mask(&mask);
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].pd_reg & 0XFF) + GPIO_CLR_ADDR) = 1 << shift;
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].pu_reg & 0XFF) + GPIO_SET_ADDR) = 1 << shift;
     //   hal_nvic_restore_interrupt_mask(mask);
    } else {
...
hal_gpio_status_t hal_gpio_pull_down(hal_gpio_pin_t gpio_pin)
{
  //  uint32_t mask;
    ...
    if (shift != 0xff) {
       // hal_nvic_save_and_set_interrupt_mask(&mask);
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].pu_reg & 0XFF) + GPIO_CLR_ADDR) = 1 << shift;
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].pd_reg & 0XFF) + GPIO_SET_ADDR) = 1 << shift;
       // hal_nvic_restore_interrupt_mask(mask);
    } else {
#ifdef HAL_GPIO_FEATURE_PUPD
...
hal_gpio_status_t hal_gpio_set_schmitt(hal_gpio_pin_t gpio_pin)
{
   ...
   // hal_nvic_save_and_set_interrupt_mask(&mask);
    ...
    if (shift != 0xff) {
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].smt_reg & 0X0FFF) + GPIO_SET_ADDR) = 1 << shift;
    }
   // hal_nvic_restore_interrupt_mask(mask);
...
hal_gpio_status_t hal_gpio_clear_schmitt(hal_gpio_pin_t gpio_pin)
{
  ...
  //  hal_nvic_save_and_set_interrupt_mask(&mask);
    ...
    shift = gpio_cfg_table[gpio_pin].smt_sr_shift;
    if (shift != 0xff) {
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].smt_reg & 0X0FFF)  + GPIO_CLR_ADDR) = 1 << shift;
    }
   // hal_nvic_restore_interrupt_mask(mask);
...
hal_gpio_status_t hal_gpio_set_slew_rate(hal_gpio_pin_t gpio_pin)
{
  ...
  //  hal_nvic_save_and_set_interrupt_mask(&mask);
    ...
    shift = gpio_cfg_table[gpio_pin].smt_sr_shift;
    if (shift != 0xff) {
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].sr_reg & 0X0FFF) + GPIO_SET_ADDR) = 1 << shift;
    }
  //  hal_nvic_restore_interrupt_mask(mask);
...
hal_gpio_status_t hal_gpio_clear_slew_rate(hal_gpio_pin_t gpio_pin)
{
  ...
  //  hal_nvic_save_and_set_interrupt_mask(&mask);
    ...
    shift = gpio_cfg_table[gpio_pin].smt_sr_shift;
    if (shift != 0xff) {
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].sr_reg & 0X0FFF) + GPIO_CLR_ADDR) = 1 << shift;
    }
  //  hal_nvic_restore_interrupt_mask(mask);
...
hal_gpio_status_t hal_gpio_set_pupd_register(hal_gpio_pin_t gpio_pin, uint8_t gpio_pupd, uint8_t gpio_r0, uint8_t gpio_r1)
{
 ...
 //   hal_nvic_save_and_set_interrupt_mask(&mask);
    ...
    if (gpio_cfg_table[gpio_pin].pupd_r0_r1_shift != 0xff) {
        ...
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].pupd_reg & 0XFF) + shift_addr) = 1 << shift;
        ...
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].r0_reg & 0XFF) + shift_addr) = 1 << shift;
        ...
        GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].r1_reg & 0XFF) + shift_addr) = 1 << shift;
    } else {
    //    hal_nvic_restore_interrupt_mask(mask);
        return HAL_GPIO_STATUS_ERROR_PIN;
    }
...
hal_gpio_status_t hal_gpio_set_high_impedance(hal_gpio_pin_t gpio_pin)
{
    ...
    GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].ies_reg & 0XFF) + GPIO_CLR_ADDR) = 1 << gpio_cfg_table[gpio_pin].ies_shift;
...
hal_gpio_status_t hal_gpio_clear_high_impedance(hal_gpio_pin_t gpio_pin)
{
    ...
    GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].ies_reg & 0XFF) + GPIO_SET_ADDR) = 1 << gpio_cfg_table[gpio_pin].ies_shift;
...
hal_gpio_status_t hal_gpio_set_driving_current(hal_gpio_pin_t gpio_pin, hal_gpio_driving_current_t driving)
{
    ...
    temp = GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].drv_reg & 0XFF));
    temp &= ~(GPIO_REG_TWO_BIT_SET_CLR << shift);
    temp |= (driving << shift);
    GPIO_REG32(IO_CFG_0_BASE + (gpio_cfg_table[gpio_pin].drv_reg & 0XFF)) = temp;
