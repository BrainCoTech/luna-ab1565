...
static int16_t afe_clock_i2s_counter[4];
...
void hal_audio_clock_enable_i2s(afe_i2s_id_t i2s_id, bool enable) {
    HAL_AUDIO_ENTER_CRITICAL();
    if (enable) {
        afe_clock_i2s_counter[i2s_id]++;
        if (afe_clock_i2s_counter[i2s_id] == 1) {
            AFE_SET_REG(AUDIO_TOP_CON1, 0 << i2s_id, 1 << i2s_id);
        }
    } else {
        afe_clock_i2s_counter[i2s_id]--;
        if (afe_clock_i2s_counter[i2s_id] == 0) {
            AFE_SET_REG(AUDIO_TOP_CON1, 1 << i2s_id, 1 << i2s_id);
        } else if (afe_clock_i2s_counter[i2s_id] < 0) {
            afe_clock_i2s_counter[i2s_id] = 0;
        }
    }
    HAL_AUDIO_EXIT_CRITICAL();
}
