...
********************************************************************************************/
void afe_truncate_out_of_range_value(int16_t *truncate_value, int32_t minimum, int32_t maximum)
{
    *truncate_value = *truncate_value < minimum ? minimum : *truncate_value;
    *truncate_value = *truncate_value > maximum ? maximum : *truncate_value;
}

uint32_t afe_to_register_value(int16_t input_db, int32_t max_db_value, int32_t min_db_value, int32_t db_step_value, uint32_t max_reg_value, uint32_t min_reg_value, uint32_t min_db_to_min_reg_value)
{
...
#define AIR_AUDIO_MIXER_GAIN_NUMBER  (3)
#else
#define AIR_AUDIO_MIXER_GAIN_NUMBER  (2)
#endif
...
bool afe_volume_digital_update(afe_hardware_digital_gain_t gain_select)
{
    ...
    if (afe_digital_gain[gain_select].mute) {
        ...
        for (uint32_t i = 0; i < AIR_AUDIO_MIXER_GAIN_NUMBER; i ++) {
            #ifdef AIR_AUDIO_MIXER_GAIN_ENABLE
                if (gain_select == AFE_HW_DIGITAL_GAIN3) break; // there is no need to do hwgain3 cp when AIR_AUDIO_MIXER_GAIN_ENABLE is defined.
            #endif
            if (i == gain_select) continue;
            if (hal_audio_hardware_gain_get_agent_count(i)) { // other hw gain is used, so we should do cp!
                compensation_flag = true;
                break;
            }
        }
        if (compensation_flag) {
            // hw gain = hw gain * cp (percentage)
        ...
        HAL_AUDIO_LOG_INFO("DSP - Hal Audio HW Gain gain select:%d,gain:0x%x,index:%d,cp:%d,cp_flag:%d", 5,
                gain_select,
                gain,
                afe_digital_gain[gain_select].index,
                afe_digital_gain[gain_select].index_compensation,
                compensation_flag
                );
