...
bool hal_memory_set_enable_by_memory_selection(hal_audio_memory_selection_t memory_select, hal_audio_control_status_t control)
{
    ...
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio Memory set enable, memory_select[0x%x], RG_mask:0x%x, on/off [%d]", 3, memory_select, rg_bit_mask, control);
    return false;
}
...
bool hal_memory_set_enable(hal_audio_agent_t agent, hal_audio_control_status_t control)
{
    ...
    if ((AFE_GET_REG(AFE_DAC_CON0) & 0x01) == 0) {
        HAL_AUDIO_ASSERT(false && "DSP - Error, please check running_flag & user conuter!!!");
    }
    AFE_SET_REG(AFE_DAC_CON0, control << offset_bit, 1 << offset_bit);
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio Memory set enable, agent[%d] on/off [%d]", 2, agent, control);
    return false;
}
...
bool hal_memory_set_address(hal_audio_memory_parameter_t *handle, hal_audio_agent_t agent)
{
    ...
    if ((afe_base_register != 0) && (afe_end_register != 0)) {
        ...
        HAL_AUDIO_LOG_INFO("DSP - Hal Audio Memory set address:0x%x length:%d agent %d", 3, handle->buffer_addr, handle->buffer_length, agent);
    }
...
bool hal_memory_set_irq_enable(hal_audio_agent_t agent, hal_audio_control_status_t control)
{
    ...
    if (irq < AFE_AUDIOSYS_IRQ_NUM) {
        ...
        HAL_AUDIO_LOG_INFO("DSP - Hal Audio Memory irq agent:%d, Off/On:%d", 2, agent, control);
...
static const int16_t *afe_ul_biquad_table[9] = {
    afe_ul_biquad_table_100,
    afe_ul_biquad_table_125,
    afe_ul_biquad_table_150,
    afe_ul_biquad_table_175,
    afe_ul_biquad_table_200,
    afe_ul_biquad_table_225,
    afe_ul_biquad_table_250,
    afe_ul_biquad_table_275,
    afe_ul_biquad_table_300
};
...
int32_t hal_src_set_power(afe_mem_asrc_id_t asrc_id, bool on)
{
    ...
    if (asrc_id >= MEM_ASRC_NUM) {
        HAL_AUDIO_LOG_ERROR("DSP - hal_src_set_power Error log:%d, %d !", 2, asrc_id, on);
        return -1;
    }
    if (on) {
...
int32_t afe_power_on_asrc(afe_mem_asrc_id_t asrc_id, bool on)
{
    ...
    if (asrc_id >= MEM_ASRC_NUM) {
        HAL_AUDIO_LOG_ERROR("afe_power_on_asrc() error: invalid asrc[%d]\n", 1, asrc_id);
        return -1;
    }
    if (on) {
        /* asrc_ck select asm_h_ck(270M) */
...
void afe_tracking_asrc_debug(afe_mem_asrc_id_t asrc_id)
{
    uint32_t  addr_offset;
    addr_offset = asrc_id * 0x100;

    HAL_AUDIO_LOG_INFO("HWSRC tracking debug info asrc_id:%d, IBUF WPTR:%x, RPTR:%x, OBUF WPTR:%x, RPTR:%x, tracking_rate:%x, CNFG:%x\r\n", 7,
                                                    asrc_id,
                                                    AFE_READ(ASM_CH01_IBUF_WRPNT + addr_offset),
                                                    AFE_READ(ASM_CH01_IBUF_RDPNT + addr_offset),
                                                    AFE_READ(ASM_CH01_OBUF_WRPNT + addr_offset),
                                                    AFE_READ(ASM_CH01_OBUF_RDPNT + addr_offset),
                                                    AFE_READ(ASM_FREQUENCY_2 + addr_offset),
                                                    AFE_READ(ASM_CH01_CNFG + addr_offset));
}
...
void afe_set_asrc(afe_mem_asrc_id_t asrc_id, afe_src_configuration_p config, bool enable)
{
    ...
    if (enable) {
        ...
        hal_src_set_irq_enable(asrc_id, true);
#else
        hal_src_set_irq_enable(asrc_id, false);
#endif
        ...
        if (config->mode != MEM_ASRC_NO_TRACKING) {
            /* check freq cali status */
            if (AFE_READ(ASM_FREQ_CALI_CTRL + addr_offset)&ASM_FREQ_CALI_CTRL_FREQ_CALC_BUSY_MASK) {
                HAL_AUDIO_LOG_WARNING("warning: freq calibration is busy\r\n", 0);
            }
            if (AFE_READ(ASM_FREQ_CALI_CTRL + addr_offset)&ASM_FREQ_CALI_CTRL_CALI_EN_POS) {
                HAL_AUDIO_LOG_WARNING("warning: freq calibration has been already enabled\r\n", 0);
            }
            /* freq_mode = (denominator/period_mode)*0x800000 */
#if 0
            AFE_WRITE(ASM_FREQ_CALI_CYC + addr_offset, 0x3F00);
            ...
            HAL_AUDIO_LOG_INFO("asrc tracking_mode=%d, tracking_clock=%d\r\n", 2, config->tracking_mode, config->tracking_clock);
            switch (config->tracking_clock) {
                case HAL_AUDIO_SRC_TRACKING_I2S1:
                case HAL_AUDIO_SRC_TRACKING_I2S2:
                case HAL_AUDIO_SRC_TRACKING_I2S3:
            ...
            if (config->mode == MEM_ASRC_TRACKING_MODE_RX) {
                AFE_WRITE(ASM_FREQ_CALI_CYC + addr_offset, 0x3F00);
                AFE_SET_REG(ASM_FREQ_CALI_CTRL + addr_offset, 1 << ASM_FREQ_CALI_CTRL_FREQ_UPDATE_FS2_POS, ASM_FREQ_CALI_CTRL_FREQ_UPDATE_FS2_MASK); /* Rx->FreqMode Bit9=1 */
                AFE_SET_REG(ASM_FREQUENCY_0 + addr_offset, hal_src_get_samplingrate_to_palette(config->output_buffer.rate),  0xFFFFFF);/*FrequencyPalette(output_fs)*/
                AFE_SET_REG(ASM_FREQUENCY_2 + addr_offset, hal_src_get_samplingrate_to_palette(config->input_buffer.rate),  0xFFFFFF);/*FrequencyPalette(input_fs)*/
            ...
            AFE_SET_REG(ASM_FREQUENCY_0 + addr_offset, hal_src_get_samplingrate_to_palette(config->input_buffer.rate),  0xFFFFFF);
            AFE_SET_REG(ASM_FREQUENCY_1 + addr_offset, hal_src_get_samplingrate_to_palette(config->output_buffer.rate), 0xFFFFFF);
            AFE_SET_REG(ASM_CH01_CNFG + addr_offset, 0 << ASM_CH01_CNFG_IFS_POS, ASM_CH01_CNFG_IFS_MASK);
            AFE_SET_REG(ASM_CH01_CNFG + addr_offset, 1 << ASM_CH01_CNFG_OFS_POS, ASM_CH01_CNFG_OFS_MASK);
        }
...
{
    asrc_config->id = asrc_id;
    uint32_t mask;
    hal_nvic_save_and_set_interrupt_mask(&mask);
    if (enable) {
        afe_asrc_clock_on(asrc_id);
        afe_power_on_asrc(asrc_id, true);
        hal_src_set_iir(asrc_id, asrc_config->input_buffer.rate, asrc_config->output_buffer.rate);
...
uint32_t hal_gain_convert_analog_output_cap_value(afe_volume_analog_output_gain_value_t gain)
{
    ...
    switch (gain) {
        ...
        case AFE_VOLUME_ANALOG_OUTPUT_GAIN_CLASSD_POS_6_DB:
            convert_value = (uint32_t)AFE_VOLUME_ANALOG_OUTPUT_CLASSD_CAP_VALUE_NEG_6_DB / (uint32_t)gain;
            break;
        default:
...
void hal_gain_set_analog_output_class_d(afe_volume_analog_output_gain_value_t gain_l, afe_volume_analog_output_gain_value_t gain_r)
{
    ...
    if (hal_audio_afe_get_counter()) {
        ...
        HAL_AUDIO_LOG_INFO("DSP - Hal Audio Gain Output Gain: CON25:0x%x, CON18:0x%x, CON23:0x%x, CON24:0x%x", 4, ANA_GET_REG(AUDDEC_ANA_CON25), ANA_GET_REG(AUDDEC_ANA_CON18), AFE_GET_REG(AUDDEC_ANA_CON23), AFE_GET_REG(AUDDEC_ANA_CON24));
    }
}
    ...
    if (hal_audio_afe_get_counter()) {
        switch (gain_select) {
            ...
            case AFE_HW_ANALOG_GAIN_INPUT3:
                gain_register_l = AUDENC_ANA_CON0 + 0x24 * (gain_select - AFE_HW_ANALOG_GAIN_INPUT1);
                gain_register_r = AUDENC_ANA_CON1 + 0x24 * (gain_select - AFE_HW_ANALOG_GAIN_INPUT1);
                reg_offset = 0x8;
                break;
            default:
                return;
...
{
    afe_i2s_id_t i2s_id = AFE_I2S0;
    if ((agent <= HAL_AUDIO_AGENT_DEVICE_I2S_MASTER_MAX) && (agent >= HAL_AUDIO_AGENT_DEVICE_I2S_MASTER_MIN)) {
        i2s_id = agent - HAL_AUDIO_AGENT_DEVICE_I2S_MASTER_MIN;
    } else if ((agent <= HAL_AUDIO_AGENT_DEVICE_I2S_SLAVE_MAX) && (agent >= HAL_AUDIO_AGENT_DEVICE_I2S_SLAVE_MIN)) {
        i2s_id = agent - HAL_AUDIO_AGENT_DEVICE_I2S_SLAVE_MIN;
    }
    return i2s_id;
}
...
{
    return AFE_I2S0_CON + i2s_id * 0x4;
}
...
bool hal_i2s_master_set_loopback(afe_i2s_id_t i2s_id, bool enable)
{
    ...
    uint32_t register_pos = AFE_I2S_TOP_CON_I2S0_LOOPBACK_POS + 0x1 * AFE_I2S0;
...
void hal_i2s_master_gpio_init(afe_i2s_id_t i2s_id)
{
    ...
    for (uint8_t i = 0; i < 4; i++) {
        hal_gpio_init(gpio_pin + i);
        hal_pinmux_set_function(gpio_pin + i, 4);
    }
}
...
{
    uint32_t i2s_base = AFE_I2S_SLV0_BASE;
    if ((i2s_id < AFE_I2S_NUMBER) && (i2s_id >= 0)) {
        i2s_base += 0x10000 * i2s_id;
    }
    return i2s_base;
}
...
void hal_i2s_slave_gpio_init(afe_i2s_id_t i2s_id)
{
    ...
    for (uint8_t i = 0; i < 4; i++) {
        hal_gpio_init(gpio_pin + i);
        hal_pinmux_set_function(gpio_pin + i, 5);
    }
}
...
bool hal_updown_set_input_rate(afe_updown_sampler_id_t updown_id, uint32_t input_rate)
{
    ...
    if ((updown_id < AFE_UPDOWN_SAMPLER_UP_DOWN_CH_MAX) && (updown_id >= 0)) {
        register_offset = AFE_DEC_INT_CON1_UP_CH01_INPUT_RATE_POS - 0x4 * updown_id;
    } else {
...
bool hal_updown_set_output_rate(afe_updown_sampler_id_t updown_id, uint32_t output_rate)
{
    ...
    if ((updown_id < AFE_UPDOWN_SAMPLER_UP_DOWN_CH_MAX) && (updown_id >= 0)) {
        register_offset = AFE_DEC_INT_CON1_UP_CH01_OUTPUT_RATE_POS - 0x4 * updown_id;
    } else {
...
bool hal_updown_set_enable(afe_updown_sampler_id_t updown_id, bool enable)
{
    uint32_t register_offset;
    if ((updown_id < AFE_UPDOWN_SAMPLER_UP_DOWN_CH_MAX) && (updown_id >= 0)) {
        register_offset = AFE_DEC_INT_CON0_UP_CH01_ENABLE_POS - 0x2 * updown_id;
    } else {
        HAL_AUDIO_LOG_WARNING("DSP - Warning invalid ID:%d  @@", 1, updown_id);
        return true;
    }
...
bool hal_tick_align_set_updown(afe_updown_sampler_id_t updown_id, hal_audio_path_interconnection_tick_source_t tick_source, bool enable)
{
    ...
    if ((tick_source <= HAL_AUDIO_PATH_TICK_SOURCE_I2S2_OUT) && (tick_source > HAL_AUDIO_PATH_TICK_ALIGN_OFF)) {
        tick_select = tick_source - 1;
    } else {
            return true;
    }
    ...
    offset_in = AFE_I2S_SLV_ENGEN_CON3_UP_IN_MODE_POS - 0x4 * updown_id;
    offset_out = AFE_I2S_SLV_ENGEN_CON3_UP_OUT_MODE_POS - 0x4 * updown_id;
...
bool hal_tick_align_set_hw_gain(afe_hardware_digital_gain_t gain_select, hal_audio_path_interconnection_tick_source_t tick_source, bool enable)
{
    ...
    register_offset = AFE_I2S_SLV_ENGEN_CON0_GAIN_MODE_POS + 0x4 * gain_select;
...
bool hal_audio_ul_set_iir(hal_audio_agent_t agent, hal_audio_ul_iir_t iir_filter, bool enable)
{
    ...
    if (iir_filter > HAL_AUDIO_UL_IIR_75HZ_AT_48KHZ) {
        ...
        hal_audio_ul_iir_set_reg((int16_t *)afe_ul_biquad_table[iir_filter - HAL_AUDIO_UL_IIR_100HZ_AT_48KHZ]);
    }
...
bool hal_audio_ul_set_dmic_selection(hal_audio_agent_t agent, hal_audio_dmic_selection_t dmic_selection)
{
    ...
    uint32_t agent_offset = agent - HAL_AUDIO_AGENT_DEVICE_ADDA_UL1;

    if ((agent >= HAL_AUDIO_AGENT_DEVICE_ADDA_UL1) && (agent <= HAL_AUDIO_AGENT_DEVICE_ADDA_MAX)) {
        clock_value = AFE_DMIC_CLOCK_DEVICE_INTERFACE_1 + agent_offset;
        data_offset = TOP_DMIC_DAT_SEL_UL1_POS + 0x2 * agent_offset;
        sel_offset = TOP_DMIC_DAT_SEL_UL1_SEL_POS + 0x1 * agent_offset;
    }

    if (dmic_selection <= HAL_AUDIO_DMIC_ANA_DMIC5) {
        clock_offset = TOP_DMIC_CK_SEL_GPIO_DMIC0_POS + 0x2 * dmic_selection;
    } else {
            return true;
    }
...
bool hal_audio_ul_reset_fifo(hal_audio_agent_t device_agent, bool enable)
{
    ...
    switch (device_agent) {
        ...
        case HAL_AUDIO_AGENT_DEVICE_ADDA_UL2:
        case HAL_AUDIO_AGENT_DEVICE_ADDA_UL3:
            reg_offset = AFE_ADDA_UL_DL_CON0_ADDA_FIFO_RST_POS - 0x8 * (device_agent - HAL_AUDIO_AGENT_DEVICE_ADDA_UL1);
            break;
        default:
            return true;
            break;
...
bool hal_audio_ul_set_swap(hal_audio_agent_t device_agent, bool enable)
{
    ...
    switch (device_agent) {
        ...
        case HAL_AUDIO_AGENT_DEVICE_ADDA_UL2:
        case HAL_AUDIO_AGENT_DEVICE_ADDA_UL3:
            reg_offset = AFE_ADDA_UL_DL_CON0_ADDA6_SWAP_POS - 0x8 * (device_agent - HAL_AUDIO_AGENT_DEVICE_ADDA_UL1);
            break;
        default:
            return true;
            break;
...
uint32_t hal_audio_dl_get_classab_compensation_value(void)
{
    ...
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio dl compensation value_l %d= %d(LP) - %d(LN) value_r %d= %d(RP) - %d(RN)", 6, value_l, avg_p_l, avg_n_l, value_r, avg_p_r, avg_n_r);
...
#endif
        HAL_AUDIO_LOG_INFO("DSP - Hal Audio VOW DMIC DMIC_CLK_SEL_0:0x%x DMIC_CLK_SEL_1:0x%x vow_mode:%d, dmic_selection:%d, ch0:%d, ch1:%d", 6,
                ANA_GET_REG(0xA2010700), ANA_GET_REG(0xA2010704), vow_mode, dmic_selection, vow_enable_mic_ch0, vow_enable_mic_ch1);
    } else {
