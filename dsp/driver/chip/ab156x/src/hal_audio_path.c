...
hal_audio_path_interconnection_input_t hal_audio_path_get_input_interconnection_port(hal_audio_interconn_selection_t interconn_select)
{
    ...
    if (((int32_t)interconn_select >= HAL_AUDIO_INTERCONN_SELECT_INPUT_MIN) && ((int32_t)interconn_select <= HAL_AUDIO_INTERCONN_SELECT_INPUT_MAX)) {
        ...
        HAL_AUDIO_ASSERT(false && "DSP - Error Hal Audio Path Port Input Wrong");
    }
    return interconnection_input;
}
...
hal_audio_path_interconnection_output_t hal_audio_path_get_output_interconnection_port(hal_audio_interconn_selection_t interconn_select)
{
    ...
    if (((int32_t)interconn_select >= (int32_t)HAL_AUDIO_INTERCONN_SELECT_OUTPUT_MIN) && ((int32_t)interconn_select <= (int32_t)HAL_AUDIO_INTERCONN_SELECT_OUTPUT_MAX)) {
        ...
        HAL_AUDIO_ASSERT(false && "DSP - Error Hal Audio Path Port Output Wrong");
    }
    return interconnection_output;
}
...
WITHOUT_DOWNSAMPLER:
        ...
        HAL_AUDIO_LOG_INFO("[interconn] channel %d updown_sampler_use:%d audio_input_rate:%d audio_output_rate:%d hw_gain_use:%d", 5,
            connection_select,
            handle->with_updown_sampler[connection_select],
            handle->audio_input_rate[connection_select],
            handle->audio_output_rate[connection_select],
            handle->with_hw_gain
            );
        ...
        if (input_port_memory_select & (HAL_AUDIO_MEMORY_DL_MASK & (~HAL_AUDIO_MEMORY_DL_DL12))) {
            ...
            hal_audio_path_set_interconnection(connection_state, model_connection_channel, input_port, AUDIO_INTERCONNECTION_OUTPUT_O36 + (input_port % 2));
            afe_updown_configuration_t updown_configuration;
            ...
            updown_configuration.output_rate = 96000; // 96k can convert to 16K/32K/8K
            // AUDIO_ASSERT((!hal_audio_updown_get_agent_count(AFE_UPDOWN_SAMPLER_UP_CH01)) && "SWB mode not support, UP_Sampler is occupied.");
            if (hal_audio_updown_set_agent(&updown_configuration, AFE_UPDOWN_SAMPLER_UP_CH01, control)) {
                hal_audio_updown_set_start(AFE_UPDOWN_SAMPLER_UP_CH01, false);
                hal_audio_path_set_interconnection(connection_state, model_connection_channel, AUDIO_INTERCONNECTION_INPUT_I42, AUDIO_INTERCONNECTION_OUTPUT_O34);
            }
            updown_configuration.input_rate = 96000;
            updown_configuration.tick_align = tick_source;
            updown_configuration.is_echo_configure_input = true;//modify for leo
