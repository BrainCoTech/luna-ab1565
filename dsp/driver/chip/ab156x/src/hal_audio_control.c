...
bool hal_audio_device_get_is_low_jitter(uint32_t device_rate);
...
#endif
void hal_audio_amp_delay_off_handler(void);
...
void hal_audio_control_initialize(void)
{
    ...
    if(!g_audio_device_mutex) {
        OS_ASSERT(FALSE && "DSP - Error create mutex FAIL");
    }
}
...
#endif
        default:
            HAL_AUDIO_LOG_ERROR("DSP - Error Hal Audio hal_audio_control_get_value, command:%d !", 1, command);
            break;
    }
    return get_value;
}
...
hal_audio_status_t hal_audio_control_set_value(hal_audio_set_value_parameter_t *handle, hal_audio_set_value_command_t command)
{
    ...
    switch (command) {
        ...
        default:
            ...
            HAL_AUDIO_LOG_ERROR("DSP - Error Hal Audio hal_audio_control_set_value, command:%d !", 1, command);
            break;
    }
    return audio_status;
}
...
static hal_audio_irq_entry afe_irq_vow_snr_function = NULL;
...
bool hal_audio_device_set_agent(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    if (control == HAL_AUDIO_CONTROL_OFF) {
        handle->common.rate = hal_audio_device_get_rate(agent);
        if (device & (HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_L | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_R)) {
            handle->i2s_master.is_low_jitter = hal_audio_device_get_is_low_jitter(handle->common.rate);
        }
    }
    ...
    if (dsp_vow && (device != HAL_AUDIO_CONTROL_DEVICE_VOW)) { //force vow to switch phase 0 to phase 1
        ...
        if (dsp_vow->mic_interface == handle->analog_mic.mic_interface && afe_analog_control[analog_select].counter) {
            ...
            if (afe_irq_vow_snr_function) {
                HAL_AUDIO_LOG_INFO("force VOW to switch phase 0 to phase 1", 0);
            ...
            if (hal_audio_agent_user_count[HAL_AUDIO_AGENT_DEVICE_ADDA_DL1] == 0) {
                if (amp_control.delay_handle.delay_output_off_time_ms) {
                    ...
                    if (amp_control.delay_handle.agent != HAL_AUDIO_AGENT_ERROR) {
                        ...
                        hal_audio_amp_delay_off_handler();
                    }
                    memcpy(&amp_control.delay_handle.device_parameter, handle, sizeof(hal_audio_device_parameter_t));
...
bool hal_audio_device_distinguish_channel(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    if (device & HAL_AUDIO_CONTROL_DEVICE_ANALOG_MIC_DUAL) {
        ...
        if ((adc_parameter->performance < hal_get_adc_performance_mode(analog_select)) && (afe_analog_control[analog_select].counter)) {
            ...
            HAL_AUDIO_LOG_INFO("switch adc[%d] performance mode,analog_control %d", 2, analog_select, analog_control);
...
bool hal_audio_device_set_i2s_slave(hal_audio_device_parameter_i2s_slave_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio DIG I2S SLAVE:%d rate:%d, word_length:%d, i2s_format:%d, enable %d", 5,
        i2s_id, handle->rate, handle->word_length, handle->i2s_format, control);
    if (control == HAL_AUDIO_CONTROL_ON) {
...
#endif
        hal_audio_clock_enable_i2s(i2s_id, control);
...
bool hal_audio_device_set_vow(hal_audio_device_parameter_vow_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    hal_audio_interface_t mic_interface[2];
    hal_audio_control_t mic_selection[2];
    int8_t i;
    mic_selection[0] = handle->mic_selection;
    mic_interface[0] = handle->mic_interface;
    mic_selection[1] = handle->mic1_selection;
    mic_interface[1] = handle->mic1_interface;
    ...
    hal_vow_reset();
    hal_wow_set_config(handle->vow_with_hpf, handle->snr_threshold, handle->alpha_rise, mic_selection[0], mic_selection[1]);
    hal_wow_set_dma_irq_threshold(handle->dma_irq_threshold);
    memset(&device_handle, 0x0, sizeof(hal_audio_device_parameter_t));
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio VOW mic_selection:0x%x, mic1_selection:0x%x, vow_with_hpf:0x%x, snr_threshold:0x%x, dma_irq_threshold:0x%x on/off:%d", 6, mic_selection[0], mic_selection[1], handle->vow_with_hpf, handle->snr_threshold, handle->dma_irq_threshold, control);
    if ((mic_selection[0] | mic_selection[1])&HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL) {
        /*Either one is DMIC */
        if((mic_selection[0] != HAL_AUDIO_CONTROL_NONE)&&(mic_selection[1] != HAL_AUDIO_CONTROL_NONE)) {
            hal_wow_set_dmic(handle->vow_mode, handle->dmic_selection, HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL, control);
        }else if(mic_selection[0] != HAL_AUDIO_CONTROL_NONE) {
    ...
    } else if ((mic_selection[0] & HAL_AUDIO_CONTROL_DEVICE_ANALOG_MIC_DUAL) || (mic_selection[1] & HAL_AUDIO_CONTROL_DEVICE_ANALOG_MIC_DUAL)) {
        afe_analog_select_t analog_select[2] = {AFE_ANALOG_ADC0, AFE_ANALOG_ADC0};
        for(i = 0; i < 2; i++) {
            if (mic_interface[i] == HAL_AUDIO_INTERFACE_2) {
                analog_select[i] = AFE_ANALOG_ADC1;
            } else if (mic_interface[i] == HAL_AUDIO_INTERFACE_3) {
                analog_select[i] = AFE_ANALOG_ADC2;
        }
#if (HAL_AUDIO_KEEP_ADC_HIGHER_PERFORMANCE_MODE)
            if (afe_analog_control[analog_select[i]].counter) {
                performance = hal_get_adc_performance_mode(analog_select[i]);
                HAL_AUDIO_LOG_INFO("VOW use current ADC%d performance mode%d", 2, analog_select[i], handle->adc_parameter.performance);
        }
#endif
        }
        hal_wow_set_amic(handle->vow_mode, performance, analog_select[0], analog_select[1], mic_selection[0], mic_selection[1], control);
        for(i = 0; i < 2; i++){
            device_handle.analog_mic.audio_device = mic_selection[i];
        ...
        device_handle.analog_mic.adc_parameter.performance = performance;
            device_handle.analog_mic.mic_interface = mic_interface[i];
        hal_audio_device_analog_set_input((hal_audio_device_parameter_t *)&device_handle, device_handle.analog_mic.audio_device, control);
            HAL_AUDIO_LOG_INFO("DSP - Hal Audio VOW analog_select[%d]:0x%x, counter:0x%x, vow_mode:0x%x, performance:0x%x", 5, i, analog_select[i], afe_analog_control[analog_select[i]].counter, handle->vow_mode, handle->adc_parameter.performance);
        }
    }
...
bool hal_audio_device_set_sidetone(hal_audio_device_parameter_sidetone_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    if (control == HAL_AUDIO_CONTROL_ON) {
        ...
        HAL_AUDIO_LOG_INFO("sidetone_gain register_value %d sidetone_gain %d 0x%x", 3, handle->is_sidetone_gain_register_value, handle->sidetone_gain, (uint32_t)handle->sidetone_stop_done_entry);
...
bool hal_audio_device_get_is_low_jitter(uint32_t device_rate) {
    afe_i2s_apll_t apll_source = hal_audio_i2s_get_apll_by_samplerate(device_rate);
    bool device_is_low_jitter;
    if (apll_source == AFE_I2S_APLL1) {
        device_is_low_jitter = !((AFE_GET_REG(AUDIO_TOP_CON0)&AUDIO_TOP_CON0_PDN_22M_MASK) >> AUDIO_TOP_CON0_PDN_22M_POS);
    } else {
        device_is_low_jitter = !((AFE_GET_REG(AUDIO_TOP_CON0)&AUDIO_TOP_CON0_PDN_24M_MASK) >> AUDIO_TOP_CON0_PDN_24M_POS);
    }
    return device_is_low_jitter;
}
...
bool hal_audio_memory_setting(hal_audio_memory_parameter_t *handle, hal_audio_control_t memory_interface, hal_audio_control_status_t control)
{
    ...
    if (control == HAL_AUDIO_CONTROL_ON) {
        ...
        if (agent == HAL_AUDIO_AGENT_MEMORY_AWB2) {
            ...
            //Connect from interconn
            {
                ...
                #if defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE) && defined(AIR_BT_ULL_SWB_ENABLE)
                    // support SWB
                    if (hal_audio_updown_get_agent_count(AFE_UPDOWN_SAMPLER_UP_CH01)) {
                        hal_audio_updown_set_start(AFE_UPDOWN_SAMPLER_UP_CH01, true);
                    }
                #endif
            }
    ...
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio Memory:0x%x, Off/On:%d, sync_status:%d, Rate:%d, IRQ_cnt:%d, format:%d, is_mono:%d", 7,
        handle->memory_select,
        control,
        handle->sync_status,
        handle->audio_path_rate,
        handle->irq_counter,
        handle->pcm_format,
        handle->with_mono_channel
        );
    return false;
}
...
bool hal_audio_src_enable_clock(afe_mem_asrc_id_t asrc_id, hal_audio_control_status_t control)
{
    HAL_AUDIO_ENTER_CRITICAL();
    if (control == HAL_AUDIO_CONTROL_ON) {
...
void hal_audio_src_reset_rate(afe_mem_asrc_id_t asrc_id)
{
    ...
    if (asrc_id == AFE_MEM_ASRC_1) {
        ...
        addr = ASM_FREQUENCY_2;
    } else {
        AFE_SET_REG(ASM2_FREQUENCY_2, 0 , 0xFFFFFF);
        addr = ASM2_FREQUENCY_2;
    }
    HAL_AUDIO_LOG_INFO("[HWSRC]reset hwsrc[%d] tracking rate done ASM_FREQUENCY=0x%x", 2, asrc_id, AFE_READ(addr));
}
...
bool hal_audio_i2s_set_apll(afe_i2s_apll_t apll_source, hal_audio_control_status_t control)
{
    hal_i2s_master_enable_apll(apll_source, (bool)control);
    return false;
}
...
bool hal_audio_slave_set_vdma(hal_audio_slave_vdma_parameter_t *vdma_setting)
{
    ...
    return 0;
ERR_HANDLER:
    DSP_MW_LOG_I("[SLAVE VDMA] set vdma fail, err_bit_mask %d err_status %d", 2, err_bit_mask, i2s_vdma_status);
    return -1;
}
#else
extern vdma_channel_t g_i2s_slave_vdma_channel_infra[];
...
{
    hal_updown_set_enable(updown_id, (bool)control);
    return false;
}
...
bool hal_audio_adda_set_bias_enable(hal_audio_bias_selection_t bias_select, hal_audio_bias_voltage_t *bias_voltage, bool is_low_power, bool bias1_2_with_LDO0, hal_audio_control_status_t control)
{
    ...
    for (bias_search = HAL_AUDIO_BIAS_SELECT_BIAS0 ; bias_search <= HAL_AUDIO_BIAS_SELECT_MAX ; bias_search <<= 1) {
        int32_t bias_control_flag = -1; // -1: do nothing; 0: disabel; 1: enable
        ...
        if (control == HAL_AUDIO_CONTROL_ON) {
            (*cunter_ptr)++;
            if (*cunter_ptr == 1) {
                bias_control_flag = 1; // enable
            }
        } else {
            (*cunter_ptr)--;
            if (*cunter_ptr == 0) {
                bias_control_flag = 0; // disable
            } else if (*cunter_ptr < 0) {
                *cunter_ptr = 0;
            }
        }
        if (bias_control_flag == 0) {
            hal_audio_ana_set_bias_configuration(bias_search, bias_voltage[index - 1], is_low_power, bias1_2_with_LDO0, false);
            hal_audio_adda_set_global_bias_enable(false);
        } else if (bias_control_flag == 1) {
            hal_audio_adda_set_global_bias_enable(true);
            hal_audio_ana_set_bias_configuration(bias_search, bias_voltage[index - 1], is_low_power, bias1_2_with_LDO0, true);
        }
        HAL_AUDIO_LOG_INFO("findPosition:%d,bias_search:0x%x bias_voltage:0x%x control_flag:%d (-1:do nothing)", 4, index, bias_search, bias_voltage[index - 1], bias_control_flag);
    }
...
void hal_audio_amp_delay_off_timer_callback(HAL_AUDIO_TIMER_HANDLE xTimer)
{
    ...
    msg.ccni_message[0] |= MSG_DSP2MCU_AUDIO_AMP << 16;
...
bool hal_audio_dl_set_nle(VOID *DestBuf, U16 CopySize, U32 format, SOURCE_TYPE type)
{
    ...
    HAL_AUDIO_LOG_INFO("[SET NLE] HW_DIGITAL_GAIN_DB = %d, gain_threshold_16:%d, gain_threshold_32:%d, nle_sample_count:%d, data_16:%d, data_32:%d, data_address:0x%x", 7,
            HW_DIGITAL_GAIN_DB,
            gain_threshold_16,
            gain_threshold_32,
            nle_sample_count,
...
#if 1
    HAL_AUDIO_LOG_INFO("VOW noise is_stable=%d stable_noise=0x%x", 2, vow_control.is_stable, vow_control.stable_noise);
...
bool hal_audio_volume_set_digital_gain_fade_time_setting(hal_audio_volume_digital_gain_fade_time_setting_parameter_t *gain_fade_tiem_setting)
{
    ...
    if ((current_target_gain != 0xFFFFFFFF) && (current_gain != 0xFFFFFFFF)) {
        if (1) { //if( (!hal_hw_gain_is_running(AFE_HW_DIGITAL_GAIN1)) && (!hal_hw_gain_is_running(AFE_HW_DIGITAL_GAIN2)) && (!hal_hw_gain_is_running(AFE_HW_DIGITAL_GAIN3))){
            ...
            HAL_AUDIO_LOG_INFO("DSP - HWGAIN gain_select (0~2) %d,gain_index %d,current_gain %d,sample_rate %d,fade_time %d\r\n", 5, gain_select, gain_index, current_gain, sample_rate, fade_time);
            ...
            HAL_AUDIO_LOG_INFO("HW Gain fade step: down_step %d*0.001dB, down_step_val %d, gain_range %d*0.1dB, up_step %d*0.001dB, up_step_val %d, gain_range %d*0.1dB, samples_per_cycle %d ,sample_rate %dHz->%dHz, fade_time %dms",
                               10,
            ...
            HAL_AUDIO_LOG_INFO("DSP - HWGAIN %d fade time %d,target gain 0x%x,current gain 0x%x,fs %d,sample_per_step %d,down_step 0x%x,up_step 0x%x", 8,
                gain_select, fade_time, gain_index, current_gain, sample_rate, sample_per_step, down_step, up_step);
        } else {
            uint32_t i = 0;
            for (i = 0; i < (AFE_HW_DIGITAL_GAIN_NUM); i++) {
                if (hal_hw_gain_is_running(i)) {
                    HAL_AUDIO_LOG_INFO("DSP - Hal Audio HW Gain[%d] is running target 0x%x cur 0x%x, can not set fade time", 3, i, hal_hw_gain_get_target(i), hal_hw_gain_get_current_gain(i));
            }
            }
        }
    } else {
...
*                                           IRQ                                            *
********************************************************************************************/
static hal_audio_irq_entry afe_irq_audiosys_function[AFE_AUDIOSYS_IRQ_NUM] = {0};
static hal_audio_irq_entry afe_irq_src1_function                           = NULL;
static hal_audio_irq_entry afe_irq_src2_function                           = NULL;
static hal_audio_irq_entry afe_irq_anc_function                            = NULL;
static hal_audio_irq_entry afe_irq_vad_function                            = NULL;
#ifdef MTK_TDM_ENABLE
static hal_audio_irq_entry afe_irq_i2s_slave_function[AFE_I2S_SLAVE_IRQ_NUM] = {0};
#else
static hal_audio_irq_entry afe_irq_i2s_slave_function = NULL;
#endif
//static hal_audio_irq_entry afe_irq_vow_snr_function;
static hal_audio_irq_entry afe_irq_vow_fifo_function = NULL;
        ...
        if (hal_audio_agent_user_count[HAL_AUDIO_AGENT_MEMORY_DL1]) {
            ...
            if (afe_get_asrc_irq_is_enabled(AFE_MEM_ASRC_1, ASM_IER_IBUF_EMPTY_INTEN_MASK) && (irq_status & ASM_IFR_IBUF_EMPTY_INT_MASK) && (AFE_READ(ASM_CH01_IBUF_WRPNT) == AFE_READ(ASM_CH01_IBUF_RDPNT))) {
                ...
                if(comp_samples >= input_samples){
                    ...
                    hal_src_clear_irq(0, ASM_IFR_IBUF_EMPTY_INT_MASK);
                    hal_src_set_irq_enable(0, false);
...
void hal_audio_irq_i2S_slave_handler(hal_nvic_irq_t irq)
{
    ...
    if (dma_int & (DMA_GLBSTA_IT1_MASK | DMA_GLBSTA_IT3_MASK | DMA_GLBSTA_IT5_MASK | DMA_GLBSTA_IT7_MASK
        ...
        DSP_MW_LOG_E("[SLAVE VDMA] irq error, dma_int=0x%x", 1, dma_int);
    }
#else
...
void hal_audio_irq_vow_snr_handler(hal_nvic_irq_t irq)
{
    ...
    if (vow_control.first_snr_irq) {
        ...
        if (vow_control.timer_handle != NULL) {
            ...
            HAL_AUDIO_LOG_WARNING("DSP - Hal Audio VOW timer NULL", 0);
        }
        ...
        HAL_AUDIO_LOG_INFO("DSP - Hal Audio VOW snr irq %d occured, ch0_signal:0x%x, ch0_noise:0x%x , ch1_signal:0x%x, ch1_noise:0x%x, Observed_noise:0x%x save vow_pre_ch0_noise_msb:0x%x,vow_pre_ch1_noise_msb:0x%x\n", 8,
            irq, ch0_signal, ch0_noise, ch1_signal, ch1_noise, vow_control.stable_noise, vow_pre_ch0_noise_msb, vow_pre_ch1_noise_msb);
        hal_nvic_disable_irq(VOW_SNR_IRQn);
#endif
        if (afe_irq_vow_snr_function) {
            afe_irq_vow_snr_function();
...
void hal_audio_irq_vow_fifo_handler(hal_nvic_irq_t irq)
{
    ...
    if (vow_control.u4AFE_MEMIF_BUF_WP) {
        ...
        if (AFE_READ(AFE_VOW_TOP_MON0) & 0x2000) {
            HAL_AUDIO_LOG_WARNING("FIFO ERROR 0x%x \r\n", 1, AFE_READ(AFE_VOW_TOP_MON0));
        ...
        if (vow_fifo_cnt > 1) {
            if ((fifo_irq_period > ((VOW_SRAM_COPY_SIZE >> 4) * 1000) + 100) ||
                ...
                HAL_AUDIO_LOG_WARNING("FIFO interrupt %d period wrong : %d \r\n", 2, vow_fifo_cnt, fifo_irq_period);
            }
        }
        vow_fifo_cnt++;
    }
