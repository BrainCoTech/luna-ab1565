...
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ void mux_ringbuffer_move_core(mux_ringbuffer_t * src_rb, uint32_t src_r_head, mux_ringbuffer_t * dst_rb, uint32_t dst_w_head, uint32_t length,  uint16_t *crc)
{
    ...
    #define B2B_SRC_ADDR_MOVE(s_idx)     (src_read_addr += (s_idx))
    #define B2B_DST_ADDR_MOVE(d_idx)     (dst_write_addr += (d_idx))
    ...
    #define B2B_SRC_TYPE_VAL(wrap, type, s_idx)   *(type *)(B2B_SRC_ADDR(wrap, s_idx))
    #define B2B_DST_TYPE_VAL(wrap, type, d_idx)   *(type *)(B2B_DST_ADDR(wrap, d_idx))
    ...
    for (;remain_data_length != 0; ) {
        ...
        //If dst or src addr offset = 1, must reserve 1 byte when data load by word.
        if (dst_offset && (dst_len_2_boundary & 0xFFFFFFFC)) {
            dst_len_2_boundary -= 1;
        } else if (src_offset && (src_len_2_boundary & 0xFFFFFFFC)) {
            src_len_2_boundary -= 1;
        }
        ...
        if (block_n != 0) {
            ...
            #if 0
            LOG_MSGID_W(common,"[%02x] copy block=%3u remain_n=%3u min_block_len=%3u sra=0x%08x dwa=0x%08x i=%3u j=%3u src_step=%3u dst_step=%3u dst_copy_done_len=%3u remain_data_length=%3u",12, \
                dst_rb->uid, block_n, remain_n, min_block_len, src_read_addr, dst_write_addr, i, j, src_step, dst_step, dst_copy_done_len, remain_data_length);
            #endif
        }
        if (remain_n != 0) {
            switch(move_flag) {
            ...
            case MUX_RB_FLAG_DATA_3TOL4:
                ...
                case 2:
                    ...
                    break;
                case 3:
                    B2B_COPY_HALF_WORD(dst_offset, src_offset);
                    B2B_COPY_BYTE(dst_offset + 2 , src_offset + 2);
                    break;
                default:
                    break;
    ...
    #if 0
        LOG_MSGID_W(common,"[%02x] copy remain=%u dst_copy_done_len=%u remain_data_length=%u src_offset=%u dst_offset=%u sra=0x%08x dwa=0x%08x",8, \
            dst_rb->uid, remain_n, dst_copy_done_len, remain_data_length, src_offset, dst_offset, src_read_addr, dst_write_addr);
...
ATTR_TEXT_IN_FAST_MEM uint32_t mux_ringbuffer_write_buffer_st(mux_ringbuffer_t *src_rb, mux_ringbuffer_t *dst_rb, uint32_t data_len, uint16_t *crc)
{
    ...
    case MUX_RB_FLAG_DATA_4TOL3:
        // if (readable_len & 0x3) {
        //     RB_LOG_E("[%02x]:readable_len=%d is not multiple of 4", 2, src_rb->uid, writable_len);
        //     assert(0);
        //     return 0;
        // }
    ...
    case MUX_RB_FLAG_DATA_3TOL4:
        ...
        // if (data_len % 3) {
        //     RB_LOG_E("[%02x]:writable_len=%d is not multiple of 3", 2, src_rb->uid, writable_len);
        //     assert(0);
        //     return 0;
        // }
