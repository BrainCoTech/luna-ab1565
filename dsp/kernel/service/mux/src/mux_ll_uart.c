...
void mux_get_last_time_diff(uint32_t uid, uint32_t *tm_diff, uint32_t *ave_tm_diff, uint32_t *tm_diff_min, uint32_t *tm_diff_max)
{
    static uint32_t last_time[2][9] = {0};
    static uint32_t time_diff_sum[2][9] = {0};
    static uint32_t time_diff_count[2][9] = {0};
    static uint32_t time_diff_min[2][9] = {0};
    static uint32_t time_diff_max[2][9] = {0};
    uint32_t start_count;
    uint32_t time_diff = 0;
    uint32_t is_tx = ((uid&0x80) == 0x80) ? 1: 0;
    uid = uid & 0x7f;

    if (time_diff_min[is_tx][uid] == 0) {
        time_diff_min[is_tx][uid] = 0xFFFFFFFF;
    }

    start_count = mux_get_tick_count();
    if (last_time[is_tx][uid] != 0) {
        time_diff = mux_get_tick_elapse_with_end(last_time[is_tx][uid], start_count);
        if (time_diff < 10000) {
            time_diff_sum[is_tx][uid] += time_diff;
            time_diff_count[is_tx][uid]++;
            // LOG_MSGID_I(common, "start_count=%u last_time=%u is_tx=%u uid=0x%x time_diff=%u\n", 5, start_count, last_time[is_tx][uid], is_tx, uid, time_diff);

            *tm_diff = time_diff;
            *ave_tm_diff = time_diff_sum[is_tx][uid]/time_diff_count[is_tx][uid];

            if (time_diff > 500) {
                if (time_diff < time_diff_min[is_tx][uid]) {
                    time_diff_min[is_tx][uid] = time_diff;
                }

                if (time_diff > time_diff_max[is_tx][uid]) {
                    time_diff_max[is_tx][uid] = time_diff;
                }
            }
        }
    }
    *tm_diff_min = time_diff_min[is_tx][uid];
    *tm_diff_max = time_diff_max[is_tx][uid];
    last_time[is_tx][uid] = start_count;
}
...
#if 1
    uint32_t time_diff = 0, ave_time_diff = 0, time_diff_min, time_diff_max;
    mux_get_last_time_diff(uid, &time_diff, &ave_time_diff, &time_diff_min, &time_diff_max);
    RB_LOG_I("[mux_rx_ll] [%02x] expect size=%d real size=%d, ud_len=%d, vd_len=%d acc_cnt=%u damount=%u td=%u td_ave=%u td_min=%u td_max=%u", 11, \
        rxbuf->uid, buffer->buf_size, *receive_done_data_len, mux_ringbuffer_data_length(&user_config->rxbuf), vd_len, rxbuf->access_count, rxbuf->data_amount, time_diff, \
        ave_time_diff, time_diff_min, time_diff_max);
#else
    RB_LOG_I("[mux_rx_ll] [%02x] expect size=%d real size=%d, ud_len=%d, vd_len=%d acc_cnt=%u damount=%u", 7, rxbuf->uid, buffer->buf_size, *receive_done_data_len,\
        mux_ringbuffer_data_length(&user_config->rxbuf), vd_len, rxbuf->access_count, rxbuf->data_amount);
#endif
    return MUX_STATUS_OK;
}
...
ATTR_TEXT_IN_FAST_MEM mux_status_t mux_tx_ll(mux_handle_t handle, const mux_buffer_t buffers[], uint32_t buffers_counter, uint32_t *send_done_data_len)
{
    ...
    for (i = 0; i < buffers_counter; i++) {
        pbuf = &buffers[i];
        write_len += mux_ringbuffer_write(txbuf, pbuf->p_buf, pbuf->buf_size);
#ifdef MUX_LL_UART_RB_DUMP_ENABLE
        expected_total_size += pbuf->buf_size;
#endif
    }
#ifdef MUX_LL_UART_RB_DUMP_ENABLE
    if (write_len != expected_total_size) {
        mux_ringbuffer_hexdump(txbuf, false);
    }
#endif
    txbuf->access_count++;
    txbuf->data_amount += write_len;
    *send_done_data_len = write_len;

    vdma_get_available_receive_bytes(uart_tx_ch[MUX_LL_PORT_2_UART_PORT(handle_to_port(handle))], &vd_len);

    mux_ll_uart_notify_mcu_ready_to_write(user_config, write_len);

#if 1
    uint32_t time_diff = 0, ave_time_diff = 0, time_diff_min, time_diff_max;
    mux_get_last_time_diff(uid|0x80, &time_diff, &ave_time_diff, &time_diff_min, &time_diff_max);

    RB_LOG_I("[mux_tx_ll] uid=%02x write_len=%d ud_len=%d vd_len=%d acc_cnt=%u damount=%u td=%u td_ave=%u td_min=%u td_max=%u", 10, \
        txbuf->uid, write_len, mux_ringbuffer_data_length(txbuf), vd_len, txbuf->access_count, txbuf->data_amount, time_diff, \
        ave_time_diff, time_diff_min, time_diff_max);
#else
    RB_LOG_I("[mux_tx_ll] uid=%02x write_len=%d ud_len=%d vd_len=%d acc_cnt=%u damount=%u", 6, txbuf->uid, write_len, mux_ringbuffer_data_length(txbuf), vd_len, txbuf->access_count, txbuf->data_amount);
#endif
